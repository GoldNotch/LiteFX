...
{
  for (auto & backend : m_impl->m_backends | std::views::filter(
                                               [type](const auto & b) {
...
void App::use(UniquePtr<IBackend> && backend)
{
  ...
  if (m_impl->m_backends.contains(type))
    throw InvalidArgumentException(
      "backend",
      "Another backend of type {0} already has been registered. An application may only contain one backend of a certain type.",
      type.name());
