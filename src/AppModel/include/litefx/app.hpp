...
public:
  ...
  inline TResult invoke(TArgs... args) const { return m_target(args...); }

  /// <summary>
  /// Returns the unique token of the delegate.
  /// </summary>
  /// <returns>The unique token of the delegate.</returns>
  inline token_type token() const { return m_token; }
...
public:
  ...
  {
    const auto match = std::max_element(m_subscribers.begin(), m_subscribers.end(),
  ...
  {
    const auto last = std::remove_if(m_subscribers.begin(), m_subscribers.end(),
  ...
  /// </summary>
  void clear() noexcept { m_subscribers.clear(); }
  ...
  bool contains(event_token_type token) const noexcept
  {
    return std::find_if(m_subscribers.begin(), m_subscribers.end(),
                        [&token](const auto & d)
                        { return d.token() == token; }) != m_subscribers.end();
  }
  ...
  {
    if (auto match = std::find_if(m_subscribers.begin(), m_subscribers.end(),
                                  [&token](const auto & d) { return d.token() == token; });
        match != m_subscribers.end()) [[likely]]
      return *match;
...
public:
  ...
  explicit operator bool() const noexcept { return !m_subscribers.empty(); }
                                ...
                                [this, callback]()
                                {
                                  ...
                                  if (backend == nullptr)
                                    throw InvalidArgumentException(
                                      "callback", "No backend of type {0} has been registered.",
