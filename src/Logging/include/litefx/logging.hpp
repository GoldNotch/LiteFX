...
public:
  RollingFileSink(const String & fileName, LogLevel level = LogLevel::Info,
                  const String & pattern = "%+", bool truncate = false, int maxFiles = 0);
...
#define LITEFX_WARNING(log, format, ...)                                                           \
  LiteFX::Logging::Logger::get(log).warning(format, __VA_ARGS__)
#define LITEFX_ERROR(log, format, ...) LiteFX::Logging::Logger::get(log).error(format, __VA_ARGS__)
#define LITEFX_FATAL_ERROR(log, format, ...)                                                       \
  LiteFX::Logging::Logger::get(log).fatal(format, __VA_ARGS__)
