...
public:
  ...
  constexpr Vector(T val) noexcept { std::fill(std::begin(m_elements), std::end(m_elements), val); }
  ...
  constexpr Vector(vec_type && _other) noexcept { m_elements = std::move(_other.m_elements); }
  ...
  constexpr explicit Vector(std::ranges::input_range auto && input) noexcept
    requires std::is_nothrow_convertible_v<std::ranges::range_value_t<decltype(input)>, T>
  {
...
public:
  ...
  /// </summary>
  constexpr operator std::array<T, DIM>() const noexcept { return m_elements; }
  ...
  constexpr int size() const noexcept { return vec_size; }
...
Vector<T, DIM> FromGLMVector(const glm::vec<DIM, T> & v);


template<typename T, size_t DIM>
glm::vec<DIM, T> ToGLMVector(const Vector<T, DIM> & v);
#endif

#if defined(LITEFX_BUILD_WITH_DIRECTX_MATH)
// TODO: Write for XMVECTOR, XMFLOAT_N, XMUINT_N, XMINT_N
template<typename T, size_t DIM, typename DXVecT>
Vector<T, DIM> FromDXVector(const DXVecT & v);


template<typename T, size_t DIM, typename DXVecT>
DXVecT ToDXVector(const Vector<T, DIM> & v);
#endif
