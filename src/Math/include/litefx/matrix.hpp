...
public:
  ...
  constexpr Matrix(T val) noexcept { std::fill(std::begin(m_elements), std::end(m_elements), val); }
  ...
  constexpr Matrix(mat_type && _other) noexcept { m_elements = std::move(_other.m_elements); }
...
public:
  ...
  constexpr const scalar_type * elements() const noexcept { return m_elements.data(); }

  /// <summary>
  /// Returns a pointer to the raw data of the matrix.
  /// </summary>
  /// <returns>A pointer to the raw data of the matrix.</returns>
  constexpr scalar_type * elements() noexcept { return m_elements.data(); }

  /// <summary>
  /// Returns the number of elements of the matrix.
  /// </summary>
  /// <returns>The number of elements of the matrix.</returns>
  consteval size_t size() const noexcept { return mat_rows * mat_cols; }
  ...
  constexpr std::array<scalar_type, mat_cols> column(size_t col) const noexcept
  {
    ...
    return m_elements | std::views::drop(col) | std::views::stride(mat_cols) |
           std::ranges::to<std::array<scalar_type, mat_cols>>();
  }
  ...
  /// </summary>
  constexpr operator std::mdspan<const scalar_type, std::extents<std::size_t, mat_rows, mat_cols>>()
    const noexcept
  {
    return std::mdspan<const scalar_type, std::extents<std::size_t, mat_rows, mat_cols>>(
      m_elements.data());
  }
  ...
  /// Converts the matrix into a multi-dimensional view over the elements.
  /// </summary>
  constexpr operator std::mdspan<scalar_type,
                                 std::extents<std::size_t, mat_rows, mat_cols>>() noexcept
  {
    return std::mdspan<scalar_type, std::extents<std::size_t, mat_rows, mat_cols>>(
      m_elements.data());
  }
#endif
  ...
  /// </summary>
  constexpr operator std::array<T, mat_rows * mat_cols>() const noexcept { return m_elements; }
  ...
  consteval bool symmetric() const noexcept { return ROWS == COLS; }
...
public:
  ...
  template<unsigned rows, unsigned cols>
  constexpr operator glm::mat<cols, rows, scalar_type>() const noexcept
    requires(mat_rows >= rows && mat_cols >= cols)
  {
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<2, 2, scalar_type>() const noexcept
    requires(mat_rows >= 2 && mat_cols >= 2)
  {
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<2, 3, scalar_type>() const noexcept
    requires(mat_rows >= 3 && mat_cols >= 2)
  {
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<2, 4, scalar_type>() const noexcept
    requires(mat_rows >= 4 && mat_cols >= 2)
  {
    return glm::mat<2, 4, scalar_type>(at(0, 0), at(1, 0), at(2, 0), at(3, 0), at(0, 1), at(1, 1),
                                       at(2, 1), at(3, 1));
  }
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<3, 2, scalar_type>() const noexcept
    requires(mat_rows >= 2 && mat_cols >= 3)
  {
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<4, 2, scalar_type>() const noexcept
    requires(mat_rows >= 2 && mat_cols >= 4)
  {
    return glm::mat<4, 2, scalar_type>(at(0, 0), at(1, 0), at(0, 1), at(1, 1), at(0, 2), at(1, 2),
                                       at(0, 3), at(1, 3));
  }
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<3, 3, scalar_type>() const noexcept
    requires(mat_rows >= 3 && mat_cols >= 3)
  {
    return glm::mat<3, 3, scalar_type>(at(0, 0), at(1, 0), at(2, 0), at(0, 1), at(1, 1), at(2, 1),
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<3, 4, scalar_type>() const noexcept
    requires(mat_rows >= 4 && mat_cols >= 3)
  {
    return glm::mat<3, 4, scalar_type>(at(0, 0), at(1, 0), at(2, 0), at(3, 0), at(0, 1), at(1, 1),
                                       at(2, 1), at(3, 1), at(0, 2), at(1, 2), at(2, 2), at(3, 2));
  }
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<4, 3, scalar_type>() const noexcept
    requires(mat_rows >= 3 && mat_cols >= 4)
  {
    return glm::mat<4, 3, scalar_type>(at(0, 0), at(1, 0), at(2, 0), at(0, 1), at(1, 1), at(2, 1),
                                       at(0, 2), at(1, 2), at(2, 2), at(0, 3), at(1, 3), at(2, 3));
  }
  ...
  /// </summary>
  /// <returns>The glm matrix instance.</returns>
  constexpr operator glm::mat<4, 4, scalar_type>() const noexcept
    requires(mat_rows >= 4 && mat_cols >= 4)
  {
    return glm::mat<4, 4, scalar_type>(at(0, 0), at(1, 0), at(2, 0), at(3, 0), at(0, 1), at(1, 1),
                                       at(2, 1), at(3, 1), at(0, 2), at(1, 2), at(2, 2), at(3, 2),
...
public:
  ...
  constexpr operator DirectX::XMMATRIX() const noexcept
    requires((mat_rows == 3 || mat_rows == 4) && (mat_cols == 3 || mat_cols == 4) &&
             std::convertible_to<scalar_type, float>)
  {
  ...
  /// </summary>
  /// <returns>The DirectX matrix instance.</returns>
  constexpr operator DirectX::XMFLOAT3X3() const noexcept
    requires(mat_rows >= 3 && mat_cols >= 3 && std::convertible_to<scalar_type, float>)
  {
    return DirectX::XMFLOAT3X3(at(0, 0), at(0, 1), at(0, 2), at(1, 0), at(1, 2), at(2, 0), at(2, 1),
                               at(2, 2));
  }
  ...
  /// </summary>
  /// <returns>The DirectX matrix instance.</returns>
  constexpr operator DirectX::XMFLOAT4X3() const noexcept
    requires(mat_rows >= 4 && mat_cols >= 3 && std::convertible_to<scalar_type, float>)
  {
    return DirectX::XMFLOAT4X3(at(0, 0), at(0, 1), at(0, 2), at(1, 0), at(1, 2), at(2, 0), at(2, 1),
                               at(2, 2), at(3, 0), at(3, 1), at(3, 2));
  }
  ...
  /// </summary>
  /// <returns>The DirectX matrix instance.</returns>
  constexpr operator DirectX::XMFLOAT3X4() const noexcept
    requires(mat_rows >= 3 && mat_cols >= 4 && std::convertible_to<scalar_type, float>)
  {
    return DirectX::XMFLOAT3X4(at(0, 0), at(0, 1), at(0, 2), at(0, 3), at(1, 0), at(1, 2), at(1, 3),
  ...
  /// </summary>
  /// <returns>The DirectX matrix instance.</returns>
  constexpr operator DirectX::XMFLOAT4X4() const noexcept
    requires(mat_rows >= 4 && mat_cols >= 4 && std::convertible_to<scalar_type, float>)
  {
    return DirectX::XMFLOAT4X4(at(0, 0), at(0, 1), at(0, 2), at(0, 3), at(1, 0), at(1, 2), at(1, 3),
                               at(2, 0), at(2, 1), at(2, 2), at(2, 3), at(3, 0), at(3, 1), at(3, 2),
                               at(3, 3));
  }
