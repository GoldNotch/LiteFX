...
size_t LiteFX::Rendering::getSize(Format format)
{
  switch (format)
  {
    ...
    default: throw InvalidArgumentException("format", "Unsupported format: {0}.", format);
  }
}

bool LiteFX::Rendering::hasDepth(Format format)
{
  const Array<Format> depthFormats = {Format::D16_UNORM,         Format::D32_SFLOAT,
                                      Format::X8_D24_UNORM,      Format::D16_UNORM_S8_UINT,
                                      Format::D24_UNORM_S8_UINT, Format::D32_SFLOAT_S8_UINT};

  return std::any_of(std::begin(depthFormats), std::end(depthFormats),
                     [&](Format f) { return f == format; });
}

bool LiteFX::Rendering::hasStencil(Format format)
{
  const Array<Format> stencilFormats = {Format::D16_UNORM_S8_UINT, Format::D24_UNORM_S8_UINT,
                                        Format::D32_SFLOAT_S8_UINT, Format::S8_UINT};

  return std::any_of(std::begin(stencilFormats), std::end(stencilFormats),
                     [&](Format f) { return f == format; });
}
