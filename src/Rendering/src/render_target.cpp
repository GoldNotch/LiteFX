...
public:
  RenderTargetImpl(RenderTarget * parent, StringView name, UInt64 uid, UInt32 location,
                   RenderTargetType type, Format format, RenderTargetFlags flags,
                   const Vector4f & clearValues, const BlendState & blendState)
    : base(parent)
    , m_identifier(uid)
    , m_name(name)
    , m_location(location)
...
RenderTarget::RenderTarget(UInt64 uid, UInt32 location, RenderTargetType type, Format format,
                           RenderTargetFlags flags, const Vector4f & clearValues,
                           const BlendState & blendState)
  : m_impl(makePimpl<RenderTargetImpl>(this, "", uid, location, type, format, flags, clearValues,
                                       blendState))
{
}

RenderTarget::RenderTarget(StringView name, UInt32 location, RenderTargetType type, Format format,
                           RenderTargetFlags flags, const Vector4f & clearValues,
                           const BlendState & blendState)
  : m_impl(makePimpl<RenderTargetImpl>(this, name, hash(name), location, type, format, flags,
                                       clearValues, blendState))
{
}

RenderTarget::RenderTarget(const RenderTarget & _other) noexcept
  : m_impl(makePimpl<RenderTargetImpl>(this, _other.name(), _other.identifier(), _other.location(),
                                       _other.type(), _other.format(), _other.flags(),
                                       _other.clearValues(), _other.blendState()))
{
}

RenderTarget::RenderTarget(RenderTarget && _other) noexcept
  : m_impl(makePimpl<RenderTargetImpl>(this, std::move(_other.m_impl->m_name),
                                       std::move(_other.m_impl->m_identifier),
                                       std::move(_other.m_impl->m_location),
                                       std::move(_other.m_impl->m_type),
                                       std::move(_other.m_impl->m_format),
                                       std::move(_other.m_impl->m_flags),
                                       std::move(_other.m_impl->m_clearValues),
                                       std::move(_other.m_impl->m_blendState)))
{
}
...
UInt64 RenderTarget::identifier() const noexcept { return m_impl->m_identifier; }

const String & RenderTarget::name() const noexcept { return m_impl->m_name; }

UInt32 RenderTarget::location() const noexcept { return m_impl->m_location; }

RenderTargetType RenderTarget::type() const noexcept { return m_impl->m_type; }

Format RenderTarget::format() const noexcept { return m_impl->m_format; }

RenderTargetFlags RenderTarget::flags() const noexcept { return m_impl->m_flags; }
...
const Vector4f & RenderTarget::clearValues() const noexcept { return m_impl->m_clearValues; }
