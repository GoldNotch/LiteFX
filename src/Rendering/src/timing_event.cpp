...
String TimingEvent::name() const noexcept { return m_impl->m_name; }

UInt64 TimingEvent::readTimestamp() const noexcept
{
  return m_impl->m_swapChain.readTimingEvent(shared_from_this());
}

UInt32 TimingEvent::queryId() const
{
  return m_impl->m_swapChain.resolveQueryId(shared_from_this());
}
