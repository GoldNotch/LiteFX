...
public:
  RenderPassDependencyImpl(RenderPassDependency * parent, const RenderTarget & renderTarget,
...
RenderPassDependency::RenderPassDependency(
  const RenderTarget & renderTarget, const DescriptorBindingPoint & descriptorBinding) noexcept
...
RenderPassDependency::RenderPassDependency(const RenderTarget & renderTarget,
                                           UInt32 descriptorRegister, UInt32 space) noexcept
  : RenderPassDependency(renderTarget,
                         DescriptorBindingPoint{.Register = descriptorRegister, .Space = space})
{
}
...
RenderPassDependency::RenderPassDependency(RenderPassDependency && _other) noexcept
  : m_impl(makePimpl<RenderPassDependencyImpl>(this, std::move(_other.m_impl->m_renderTarget),
                                               std::move(_other.m_impl->m_descriptorBinding)))
{
}
