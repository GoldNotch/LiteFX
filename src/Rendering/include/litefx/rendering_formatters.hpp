...
struct LITEFX_RENDERING_API std::formatter<ResourceUsage> : std::formatter<std::string_view>
{
  auto format(ResourceUsage t, std::format_context & ctx) const
  {
    ...
    else
    {
      ...
      if ((t & ResourceUsage::AccelerationStructureBuildInput) ==
          ResourceUsage::AccelerationStructureBuildInput)
...
struct LITEFX_RENDERING_API std::formatter<BufferFormat> : std::formatter<std::string_view>
{
  auto format(BufferFormat t, std::format_context & ctx) const
  {
    ...
    {
      case 1: names.push_back("X"); break;
      case 2: names.push_back("XY"); break;
      case 3: names.push_back("XYZ"); break;
      case 4: names.push_back("XYZW"); break;
      default: return formatter<string_view>::format("Invalid", ctx);
    }

    switch (::getBufferFormatChannelSize(t))
    {
      case 8: names.push_back("8"); break;
      case 16: names.push_back("16"); break;
      case 32: names.push_back("32"); break;
      case 64: names.push_back("64"); break;
      default: return formatter<string_view>::format("Invalid", ctx);
    }

    switch (::getBufferFormatType(t))
    {
      case 0x01: names.push_back("F"); break;
      case 0x02: names.push_back("I"); break;
      case 0x04: names.push_back("S"); break;
      default: return formatter<string_view>::format("Invalid", ctx);
    }
...
template<>
struct LITEFX_RENDERING_API std::formatter<AccelerationStructureFlags>
  : std::formatter<std::string_view>
{
  auto format(AccelerationStructureFlags t, std::format_context & ctx) const
  {
    ...
    else
    {
      ...
      if ((t & AccelerationStructureFlags::AllowCompaction) ==
          AccelerationStructureFlags::AllowCompaction)
        names.push_back("AllowCompaction");
      if ((t & AccelerationStructureFlags::PreferFastTrace) ==
          AccelerationStructureFlags::AllowCompaction)
        names.push_back("PreferFastTrace");
      if ((t & AccelerationStructureFlags::PreferFastBuild) ==
          AccelerationStructureFlags::PreferFastBuild)
        names.push_back("PreferFastBuild");
      if ((t & AccelerationStructureFlags::MinimizeMemory) ==
          AccelerationStructureFlags::MinimizeMemory)
