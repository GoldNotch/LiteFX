...
enum class LITEFX_RENDERING_API ShaderStage : UInt32
{
  ...
  /// </summary>
  RasterizationPipeline = Vertex | Geometry | TessellationControl | TessellationEvaluation |
                          Fragment,
  ...
  /// </summary>
  Any = Vertex | TessellationControl | TessellationEvaluation | Geometry | Fragment | Compute |
        Task | Mesh | RayGeneration | AnyHit | ClosestHit | Miss | Intersection | Callable,
...
public:
  ...
  explicit RenderTarget(UInt64 uid, UInt32 location, RenderTargetType type, Format format,
  ...
  /// <param name="clearValues">The values with which the render target gets cleared.</param>
  /// <param name="blendState">The render target blend state.</param>
  explicit RenderTarget(StringView name, UInt32 location, RenderTargetType type, Format format,
                        RenderTargetFlags flags = RenderTargetFlags::None,
                        const Vector4f & clearValues = {0.f, 0.f, 0.f, 0.f},
                        const BlendState & blendState = {});
...
public:
  ...
  RenderPassDependency(const RenderTarget & renderTarget,
                       const DescriptorBindingPoint & descriptorBinding) noexcept;
  ...
  RenderPassDependency(const RenderTarget & renderTarget, UInt32 bindingRegister,
...
public:
  ...
  explicit DepthStencilState(const DepthState & depthState, const DepthBias & depthBias,
                             const StencilState & stencilState) noexcept;
...
public:
  ...
  explicit Rasterizer(PolygonMode polygonMode, CullMode cullMode, CullOrder cullOrder,
                      Float lineWidth = 1.f,
...
public:
  ...
  inline std::shared_ptr<TimingEvent> getptr() { return shared_from_this(); }
...
public:
  ...
  BufferAttribute(UInt32 location, UInt32 offset, BufferFormat format, AttributeSemantic semantic,
...
public:
  ...
  /// <param name="firstElement">The first element of the array to map.</param>
  /// <param name="write">If `true`, <paramref name="data" /> is copied into the internal memory. If `false` the internal memory is copied into <paramref name="data" />.</param>
  virtual void map(Span<void *> data, size_t elementSize, UInt32 firstElement = 0,
                   bool write = true) = 0;
};
...
/// </summary>
class LITEFX_RENDERING_API IBuffer : public virtual IDeviceMemory,
                                     public virtual IMappable,
                                     public virtual IStateResource
{
public:
...
public:
  ...
  /// <seealso cref="update" />
  inline void build(const ICommandBuffer & commandBuffer,
                    SharedPtr<const IBuffer> scratchBuffer = nullptr,
  ...
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if <paramref name="buffer" /> is not `nullptr` and the range provided by <paramref name="offset" /> and <paramref name="maxSize" /> is not fully contained by the buffer.</exception>
  /// <seealso cref="build" />
  inline void update(const ICommandBuffer & commandBuffer,
                     SharedPtr<const IBuffer> scratchBuffer = nullptr,
                     SharedPtr<const IBuffer> buffer = nullptr, UInt64 offset = 0,
                     UInt64 maxSize = 0)
  {
...
private:
  ...
  virtual void doBuild(const ICommandBuffer & commandBuffer, SharedPtr<const IBuffer> scratchBuffer,
                       SharedPtr<const IBuffer> buffer, UInt64 offset, UInt64 maxSize) = 0;
  virtual void doUpdate(const ICommandBuffer & commandBuffer,
                        SharedPtr<const IBuffer> scratchBuffer, SharedPtr<const IBuffer> buffer,
                        UInt64 offset, UInt64 maxSize) = 0;
};
...
public:
  ...
  public:
    ...
    TriangleMesh(SharedPtr<const IVertexBuffer> vertexBuffer,
                 SharedPtr<const IIndexBuffer> indexBuffer = nullptr,
      ...
      , Flags(flags)
    {
      if (vertexBuffer == nullptr) [[unlikely]]
        throw ArgumentNotInitializedException("vertexBuffer",
...
public:
  ...
  /// <param name="flags">The flags that control how the primitives in the geometry behaves during ray-tracing.</param>
  /// <exception cref="RuntimeException">Thrown, if the acceleration structure already contains bounding boxes.</exception>
  inline void addTriangleMesh(SharedPtr<const IVertexBuffer> vertexBuffer,
                              SharedPtr<const IIndexBuffer> indexBuffer = nullptr,
                              SharedPtr<const IBuffer> transformBuffer = nullptr,
                              GeometryFlags flags = GeometryFlags::None)
  {
  ...
  inline void copy(const ICommandBuffer & commandBuffer,
                   IBottomLevelAccelerationStructure & destination, bool compress = false,
                   SharedPtr<const IBuffer> buffer = nullptr, UInt64 offset = 0,
...
public:
  ...
  /// <exception cref="RuntimeException">Thrown, if the acceleration structure already contains bounding boxes.</exception>
  template<typename TSelf>
  inline auto withTriangleMesh(this TSelf && self, SharedPtr<const IVertexBuffer> vertexBuffer,
                               SharedPtr<const IIndexBuffer> indexBuffer = nullptr,
                               SharedPtr<const IBuffer> transformBuffer = nullptr,
                               GeometryFlags flags = GeometryFlags::None) -> TSelf &&
  {
  ...
  /// <exception cref="RuntimeException">Thrown, if the acceleration structure already contains triangle meshes.</exception>
  template<typename TSelf>
  inline auto withBoundingBox(this TSelf && self, SharedPtr<const IBuffer> buffer,
                              GeometryFlags flags = GeometryFlags::None) -> TSelf &&
  {
...
private:
  virtual void doCopy(const ICommandBuffer & commandBuffer,
                      IBottomLevelAccelerationStructure & destination, bool compress,
                      SharedPtr<const IBuffer> buffer, UInt64 offset, bool copyBuildInfo) const = 0;
};
...
public:
  ...
  inline void addInstance(SharedPtr<const IBottomLevelAccelerationStructure> blas, UInt32 id,
  ...
  /// <param name="mask">A user defined mask value that can be used to include or exclude the instance during a ray-tracing pass.</param>
  /// <param name="flags">The flags that control the behavior of the instance.</param>
  inline void addInstance(SharedPtr<const IBottomLevelAccelerationStructure> blas,
  ...
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if <paramref name="buffer" /> is not `nullptr` and does not fully contain the required memory to store the copy, starting at <paramref name="offset" />.</exception>
  inline void copy(const ICommandBuffer & commandBuffer,
                   ITopLevelAccelerationStructure & destination, bool compress = false,
                   SharedPtr<const IBuffer> buffer = nullptr, UInt64 offset = 0,
                   bool copyBuildInfo = true) const
  {
    doCopy(commandBuffer, destination, compress, buffer, offset, copyBuildInfo);
...
public:
  ...
  /// <returns>A reference to the current TLAS.</returns>
  template<typename TSelf>
  inline auto withInstance(this TSelf && self,
                           SharedPtr<const IBottomLevelAccelerationStructure> blas, UInt32 id,
                           UInt32 hitGroupOffset = 0, Byte mask = 0xFF,
                           InstanceFlags flags = InstanceFlags::None) noexcept -> TSelf &&
  {
    self.addInstance(Instance{.BottomLevelAccelerationStructure = blas,
                              .Id = id,
                              .Mask = mask,
                              .HitGroupOffset = hitGroupOffset,
                              .Flags = flags});
  ...
  template<typename TSelf>
  inline auto withInstance(this TSelf && self,
                           SharedPtr<const IBottomLevelAccelerationStructure> blas,
                           const TMatrix3x4<Float> & transform, UInt32 id,
                           UInt32 hitGroupOffset = 0, Byte mask = 0xFF,
                           InstanceFlags flags = InstanceFlags::None) noexcept -> TSelf &&
  {
    self.addInstance(Instance{.BottomLevelAccelerationStructure = blas,
...
private:
  virtual void doCopy(const ICommandBuffer & commandBuffer,
                      ITopLevelAccelerationStructure & destination, bool compress,
                      SharedPtr<const IBuffer> buffer, UInt64 offset, bool copyBuildInfo) const = 0;
};
...
public:
  ...
  /// <param name="accessAfter">The access types that subsequent commands continue with.</param>
  /// <param name="layout">The image layout to transition into.</param>
  constexpr void transition(const IImage & image, UInt32 level, UInt32 levels, UInt32 layer,
...
private:
  constexpr virtual void doTransition(const IBuffer & buffer, ResourceAccess accessBefore,
                                      ResourceAccess accessAfter) = 0;
  constexpr virtual void doTransition(const IBuffer & buffer, UInt32 element,
                                      ResourceAccess accessBefore, ResourceAccess accessAfter) = 0;
  constexpr virtual void doTransition(const IImage & image, ResourceAccess accessBefore,
                                      ResourceAccess accessAfter, ImageLayout layout) = 0;
  constexpr virtual void doTransition(const IImage & image, ResourceAccess accessBefore,
                                      ResourceAccess accessAfter, ImageLayout fromLayout,
                                      ImageLayout toLayout) = 0;
  constexpr virtual void doTransition(const IImage & image, UInt32 level, UInt32 levels,
                                      UInt32 layer, UInt32 layers, UInt32 plane,
                                      ResourceAccess accessBefore, ResourceAccess accessAfter,
                                      ImageLayout layout) = 0;
  constexpr virtual void doTransition(const IImage & image, UInt32 level, UInt32 levels,
                                      UInt32 layer, UInt32 layers, UInt32 plane,
                                      ResourceAccess accessBefore, ResourceAccess accessAfter,
                                      ImageLayout fromLayout, ImageLayout toLayout) = 0;
};
...
private:
  virtual void doUpdate(UInt32 binding, const IBuffer & buffer, UInt32 bufferElement,
                        UInt32 elements, UInt32 firstDescriptor) const = 0;
  virtual void doUpdate(UInt32 binding, const IImage & texture, UInt32 descriptor,
                        UInt32 firstLevel, UInt32 levels, UInt32 firstLayer,
                        UInt32 layers) const = 0;
  virtual void doUpdate(UInt32 binding, const ISampler & sampler, UInt32 descriptor) const = 0;
  virtual void doUpdate(UInt32 binding, const IAccelerationStructure & accelerationStructure,
                        UInt32 descriptor) const = 0;
};
...
{
public:
  using resource_container = Variant<std::monostate, Ref<const IBuffer>, Ref<const IImage>,
                                     Ref<const ISampler>, Ref<const IAccelerationStructure>>;
...
private:
  ...
  virtual UniquePtr<IDescriptorSet> getDescriptorSet(
    UInt32 descriptors, const Enumerable<DescriptorBinding> & bindings = {}) const = 0;
  virtual Enumerable<UniquePtr<IDescriptorSet>> getDescriptorSets(
    UInt32 descriptorSets, UInt32 descriptors,
    const Enumerable<Enumerable<DescriptorBinding>> & bindings = {}) const = 0;
  virtual Enumerable<UniquePtr<IDescriptorSet>> getDescriptorSets(
    UInt32 descriptorSets, UInt32 descriptors,
    std::function<Enumerable<DescriptorBinding>(UInt32)> bindingFactory) const = 0;
...
public:
  /// <inheritdoc />
  const shader_group_type & shaderGroup() const noexcept override { return m_shaderGroup; }
  ...
  /// <inheritdoc />
  UInt64 localDataSize() const noexcept override { return sizeof(TLocalData); }
...
public:
  ...
  /// <inheritdoc />
  const void * localData() const noexcept override { return nullptr; }

  /// <inheritdoc />
  UInt64 localDataSize() const noexcept override { return 0_ui64; }
...
public:
  ...
  inline SharedPtr<const IShaderProgram> program() const noexcept { return m_program; }
...
public:
  ...
  inline void addShaderRecord(StringView shaderName)
  {
    ...
    if (shaderModule == nullptr) [[unlikely]]
      throw InvalidArgumentException(
        "shaderName", "The parent shader program does not contain a shader named \"{}\".",
        shaderName);
  ...
  inline void addMeshGeometryShaderHitGroupRecord(std::optional<StringView> anyHitShaderName,
                                                  std::optional<StringView> closestHitShaderName)
  {
...
public:
  ...
  /// </summary>
  /// <returns>A pointer with shared ownership to the current instance.</returns>
  inline std::shared_ptr<const IShaderProgram> getptr() const { return shared_from_this(); }

  /// <summary>
  /// Returns a pointer with shared ownership to the current instance.
  /// </summary>
  /// <returns>A pointer with shared ownership to the current instance.</returns>
  inline std::shared_ptr<IShaderProgram> getptr() { return shared_from_this(); }
...
public:
  ...
  inline const IShaderModule * operator[](StringView name) const noexcept
  {
    ...
    if (auto match = std::ranges::find_if(modules,
                                          [name](auto module) {
                                            return std::strcmp(module->fileName().c_str(),
                                                               name.data()) == 0;
                                          });
        match != modules.end())
      return *match;
...
public:
  ...
  /// <param name="elements">The number of elements to copy from the source buffer into the target buffer.</param>
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if the number of either the source buffer or the target buffer has not enough elements for the specified <paramref name="elements" /> parameter.</exception>
  inline void transfer(SharedPtr<const IBuffer> source, const IBuffer & target,
  ...
  /// <param name="elements">The number of elements to copy from the source buffer into the target image sub-resources.</param>
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if the number of either the source buffer or the target buffer has not enough elements for the specified <paramref name="elements" /> parameter.</exception>
  inline void transfer(SharedPtr<const IBuffer> source, const IImage & target,
  ...
  /// <param name="subresources">The number of sub-resources to copy between the images.</param>
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if the number of either the source buffer or the target buffer has not enough elements for the specified <paramref name="elements" /> parameter.</exception>
  inline void transfer(SharedPtr<const IImage> source, const IImage & target,
  ...
  /// <param name="subresources">The number of sub-resources to copy.</param>
  /// <exception cref="ArgumentOutOfRangeException">Thrown, if the number of either the source buffer or the target buffer has not enough elements for the specified <paramref name="elements" /> parameter.</exception>
  inline void transfer(SharedPtr<const IImage> source, const IBuffer & target,
  ...
  inline void bind(this const TSelf & self, std::initializer_list<const T *> descriptorSets)
  ...
  /// <exception cref="RuntimeException">Thrown, if no pipeline has been used before attempting to bind the descriptor set.</exception>
  template<typename TSelf>
  inline void bind(this const TSelf & self, std::ranges::input_range auto && descriptorSets)
    requires std::derived_from<
      std::remove_cv_t<std::remove_pointer_t<std::iter_value_t<std::ranges::iterator_t<
        std::remove_cv_t<std::remove_reference_t<decltype(descriptorSets)>>>>>>,
      IDescriptorSet>
  {
  ...
  template<typename TSelf, typename T>
  inline void bind(this const TSelf & self, std::initializer_list<const T *> descriptorSets,
                   const typename TSelf::pipeline_type & pipeline) noexcept
  {
    // NOTE: In the future we might be able to remove this method, if P2447R4 is added to the language.
    Array<const T *> sets = descriptorSets;
  ...
  /// <seealso cref="dispatch" />
  inline void dispatchMeshIndirect(
  ...
  inline void traceRays(UInt32 width, UInt32 height, UInt32 depth,
  ...
  /// <param name="hitShaderBindingTable">The shader binding table that contains the hit shaders.</param>
  /// <param name="callableShaderBindingTable">The shader binding table that contains the callable shaders.</param>
  inline void traceRays(const Vector3u & dimensions, const ShaderBindingTableOffsets & offsets,
                        const IBuffer & rayGenerationShaderBindingTable,
                        const IBuffer * missShaderBindingTable = nullptr,
                        const IBuffer * hitShaderBindingTable = nullptr,
                        const IBuffer * callableShaderBindingTable = nullptr) const noexcept
  ...
  virtual void draw(UInt32 vertices, UInt32 instances = 1, UInt32 firstVertex = 0,
  ...
  /// <param name="maxBatches">The maximum number of batches executed, even if there are more batches in <paramref name="countBuffer"/>.</param>
  /// <seealso cref="draw" />
  inline void drawIndirect(const IBuffer & batchBuffer, const IBuffer & countBuffer,
  ...
  virtual void drawIndexed(UInt32 indices, UInt32 instances = 1, UInt32 firstIndex = 0,
                           Int32 vertexOffset = 0, UInt32 firstInstance = 0) const noexcept = 0;
  ...
  /// <param name="vertexOffset">The offset added to each index to find the corresponding vertex.</param>
  /// <param name="firstInstance">The index of the first instance to draw.</param>
  inline void drawIndexed(const IIndexBuffer & indexBuffer, UInt32 instances = 1,
  ...
  /// <param name="vertexOffset">The offset added to each index to find the corresponding vertex.</param>
  /// <param name="firstInstance">The index of the first instance to draw.</param>
  inline void drawIndexed(const IVertexBuffer & vertexBuffer, const IIndexBuffer & indexBuffer,
  ...
  inline void buildAccelerationStructure(IBottomLevelAccelerationStructure & blas,
  ...
  /// <exception cref="ArgumentNotInitializedException">Thrown, if the provided <paramref name="scratchBuffer" /> is not initialized.</exception>
  /// <seealso cref="IAccelerationStructure::build" />
  inline void buildAccelerationStructure(ITopLevelAccelerationStructure & tlas,
                                         const SharedPtr<const IBuffer> scratchBuffer,
                                         const IBuffer & buffer, UInt64 offset = 0) const
  {
  ...
  /// <exception cref="ArgumentNotInitializedException">Thrown, if the provided <paramref name="scratchBuffer" /> is not initialized.</exception>
  /// <seealso cref="IAccelerationStructure::build" />
  inline void updateAccelerationStructure(IBottomLevelAccelerationStructure & blas,
                                          const SharedPtr<const IBuffer> scratchBuffer,
                                          const IBuffer & buffer, UInt64 offset = 0) const
  {
  ...
  /// <exception cref="ArgumentNotInitializedException">Thrown, if the provided <paramref name="scratchBuffer" /> is not initialized.</exception>
  /// <seealso cref="IAccelerationStructure::build" />
  inline void updateAccelerationStructure(ITopLevelAccelerationStructure & tlas,
                                          const SharedPtr<const IBuffer> scratchBuffer,
                                          const IBuffer & buffer, UInt64 offset = 0) const
  {
  ...
  inline void copyAccelerationStructure(const IBottomLevelAccelerationStructure & from,
  ...
  /// <param name="to">The destination acceleration structure to copy to.</param>
  /// <param name="compress">If set to `true`, the acceleration structure will be compressed.</param>
  inline void copyAccelerationStructure(const ITopLevelAccelerationStructure & from,
...
private:
  virtual UniquePtr<IBarrier> getBarrier(PipelineStage syncBefore,
                                         PipelineStage syncAfter) const noexcept = 0;
  virtual void cmdBarrier(const IBarrier & barrier) const noexcept = 0;
  virtual void cmdGenerateMipMaps(IImage & image) noexcept = 0;
  virtual void cmdTransfer(const IBuffer & source, const IBuffer & target, UInt32 sourceElement,
                           UInt32 targetElement, UInt32 elements) const = 0;
  virtual void cmdTransfer(const IBuffer & source, const IImage & target, UInt32 sourceElement,
                           UInt32 firstSubresource, UInt32 elements) const = 0;
  virtual void cmdTransfer(const IImage & source, const IImage & target, UInt32 sourceSubresource,
                           UInt32 targetSubresource, UInt32 subresources) const = 0;
  virtual void cmdTransfer(const IImage & source, const IBuffer & target, UInt32 firstSubresource,
                           UInt32 targetElement, UInt32 subresources) const = 0;
  virtual void cmdTransfer(SharedPtr<const IBuffer> source, const IBuffer & target,
                           UInt32 sourceElement, UInt32 targetElement, UInt32 elements) const = 0;
  virtual void cmdTransfer(SharedPtr<const IBuffer> source, const IImage & target,
                           UInt32 sourceElement, UInt32 firstSubresource,
                           UInt32 elements) const = 0;
  virtual void cmdTransfer(SharedPtr<const IImage> source, const IImage & target,
                           UInt32 sourceSubresource, UInt32 targetSubresource,
                           UInt32 subresources) const = 0;
  virtual void cmdTransfer(SharedPtr<const IImage> source, const IBuffer & target,
                           UInt32 firstSubresource, UInt32 targetElement,
                           UInt32 subresources) const = 0;
  virtual void cmdTransfer(const void * const data, size_t size, const IBuffer & target,
                           UInt32 targetElement, UInt32 elements) const = 0;
  virtual void cmdTransfer(Span<const void * const> data, size_t elementSize,
                           const IBuffer & target, UInt32 targetElement) const = 0;
  virtual void cmdTransfer(const void * const data, size_t size, const IImage & target,
                           UInt32 subresource) const = 0;
  virtual void cmdTransfer(Span<const void * const> data, size_t elementSize, const IImage & target,
                           UInt32 firstSubresource, UInt32 elements) const = 0;
  ...
  virtual void cmdBind(const IDescriptorSet & descriptorSet,
                       const IPipeline & pipeline) const noexcept = 0;
  virtual void cmdBind(Span<const IDescriptorSet *> descriptorSets,
                       const IPipeline & pipeline) const noexcept = 0;
  ...
  virtual void cmdPushConstants(const IPushConstantsLayout & layout,
                                const void * const memory) const noexcept = 0;
  virtual void cmdDispatchIndirect(const IBuffer & batchBuffer, UInt32 batchCount,
                                   UInt64 offset) const noexcept = 0;
  virtual void cmdDispatchMeshIndirect(const IBuffer & batchBuffer, UInt32 batchCount,
                                       UInt64 offset) const noexcept = 0;
  virtual void cmdDispatchMeshIndirect(const IBuffer & batchBuffer, const IBuffer & countBuffer,
                                       UInt64 offset, UInt64 countOffset,
                                       UInt32 maxBatches) const noexcept = 0;
  virtual void cmdDraw(const IVertexBuffer & vertexBuffer, UInt32 instances, UInt32 firstVertex,
                       UInt32 firstInstance) const = 0;
  virtual void cmdDrawIndirect(const IBuffer & batchBuffer, UInt32 batchCount,
                               UInt64 offset) const noexcept = 0;
  virtual void cmdDrawIndirect(const IBuffer & batchBuffer, const IBuffer & countBuffer,
                               UInt64 offset, UInt64 countOffset,
                               UInt32 maxBatches) const noexcept = 0;
  virtual void cmdDrawIndexed(const IIndexBuffer & indexBuffer, UInt32 instances, UInt32 firstIndex,
                              Int32 vertexOffset, UInt32 firstInstance) const = 0;
  virtual void cmdDrawIndexed(const IVertexBuffer & vertexBuffer, const IIndexBuffer & indexBuffer,
                              UInt32 instances, UInt32 firstIndex, Int32 vertexOffset,
                              UInt32 firstInstance) const = 0;
  virtual void cmdDrawIndexedIndirect(const IBuffer & batchBuffer, UInt32 batchCount,
                                      UInt64 offset) const noexcept = 0;
  virtual void cmdDrawIndexedIndirect(const IBuffer & batchBuffer, const IBuffer & countBuffer,
                                      UInt64 offset, UInt64 countOffset,
                                      UInt32 maxBatches) const noexcept = 0;
  ...
  virtual void cmdBuildAccelerationStructure(IBottomLevelAccelerationStructure & blas,
                                             const SharedPtr<const IBuffer> scratchBuffer,
                                             const IBuffer & buffer, UInt64 offset) const = 0;
  virtual void cmdBuildAccelerationStructure(ITopLevelAccelerationStructure & tlas,
                                             const SharedPtr<const IBuffer> scratchBuffer,
                                             const IBuffer & buffer, UInt64 offset) const = 0;
  virtual void cmdUpdateAccelerationStructure(IBottomLevelAccelerationStructure & blas,
                                              const SharedPtr<const IBuffer> scratchBuffer,
                                              const IBuffer & buffer, UInt64 offset) const = 0;
  virtual void cmdUpdateAccelerationStructure(ITopLevelAccelerationStructure & tlas,
                                              const SharedPtr<const IBuffer> scratchBuffer,
                                              const IBuffer & buffer, UInt64 offset) const = 0;
  virtual void cmdCopyAccelerationStructure(const IBottomLevelAccelerationStructure & from,
                                            const IBottomLevelAccelerationStructure & to,
                                            bool compress) const noexcept = 0;
  virtual void cmdCopyAccelerationStructure(const ITopLevelAccelerationStructure & from,
                                            const ITopLevelAccelerationStructure & to,
                                            bool compress) const noexcept = 0;
  virtual void cmdTraceRays(UInt32 width, UInt32 height, UInt32 depth,
                            const ShaderBindingTableOffsets & offsets,
                            const IBuffer & rayGenerationShaderBindingTable,
                            const IBuffer * missShaderBindingTable,
                            const IBuffer * hitShaderBindingTable,
                            const IBuffer * callableShaderBindingTable) const noexcept = 0;
};
...
public:
  ...
  inline UniquePtr<IBuffer> allocateShaderBindingTable(
...
private:
  virtual UniquePtr<IBuffer> getShaderBindingTable(ShaderBindingTableOffsets & offsets,
                                                   ShaderBindingGroup groups) const noexcept = 0;
};
...
public:
  ...
  public:
    ...
    inline const Size2d & newSize() const noexcept { return m_newSize; }
  };
...
public:
  ...
  /// </summary>
  virtual inline ~IFrameBuffer() noexcept { released.invoke(this, {}); }
...
public:
  ...
  template<typename TSelf>
  inline auto addImage(this TSelf && self, Format format,
  ...
  /// <param name="usage">The desired resource usage flags for the image.</param>
  template<typename TSelf>
  inline auto addImage(this TSelf && self, StringView name, Format format,
                       MultiSamplingLevel samples = MultiSamplingLevel::x1,
                       ResourceUsage usage = ResourceUsage::FrameBufferImage) -> TSelf &&
  {
  ...
  virtual void addImage(const String & name, Format format,
                        MultiSamplingLevel samples = MultiSamplingLevel::x1,
                        ResourceUsage usage = ResourceUsage::FrameBufferImage) = 0;
  ...
  /// <param name="usage">The desired resource usage flags for the image.</param>
  template<typename TSelf>
  inline auto addImage(this TSelf && self, const RenderTarget & renderTarget,
                       MultiSamplingLevel samples = MultiSamplingLevel::x1,
                       ResourceUsage usage = ResourceUsage::FrameBufferImage) -> TSelf &&
  {
  ...
  /// <param name="samples">The number of samples of the image.</param>
  /// <param name="usage">The desired resource usage flags for the image.</param>
  inline void addImage(const RenderTarget & renderTarget,
                       MultiSamplingLevel samples = MultiSamplingLevel::x1,
                       ResourceUsage usage = ResourceUsage::FrameBufferImage)
  {
  ...
  /// <param name="usage">The desired resource usage flags for the image.</param>
  template<typename TSelf>
  inline auto addImage(this TSelf && self, StringView name, const RenderTarget & renderTarget,
                       MultiSamplingLevel samples = MultiSamplingLevel::x1,
                       ResourceUsage usage = ResourceUsage::FrameBufferImage) -> TSelf &&
  {
  ...
  /// <param name="samples">The number of samples of the image.</param>
  /// <param name="usage">The desired resource usage flags for the image.</param>
  virtual void addImage(const String & name, const RenderTarget & renderTarget,
                        MultiSamplingLevel samples = MultiSamplingLevel::x1,
                        ResourceUsage usage = ResourceUsage::FrameBufferImage) = 0;
  ...
  /// <param name="usage">The desired resource usage flags for the image.</param>
  template<typename TSelf>
  inline auto addImages(this TSelf && self, Span<const RenderTarget> renderTargets,
                        MultiSamplingLevel samples = MultiSamplingLevel::x1,
                        ResourceUsage usage = ResourceUsage::FrameBufferImage) -> TSelf &&
  {
    std::ranges::for_each(renderTargets, [&](auto & renderTarget)
                          { self.addImage(renderTarget.name(), renderTarget, samples, usage); });
    return std::forward<TSelf>(self);
  }
...
public:
  ...
  public:
    ...
    inline const IFrameBuffer & frameBuffer() const noexcept { return m_frameBuffer; }
  };
...
public:
  ...
  virtual const Optional<DescriptorBindingPoint> & inputAttachmentSamplerBinding()
    const noexcept = 0;
...
public:
  ...
  public:
    ...
    inline Format surfaceFormat() const noexcept { return m_surfaceFormat; }
    ...
    inline const Size2d & renderArea() const noexcept { return m_renderArea; }

    /// <summary>
    /// Gets the number of back-buffers in the swap chain.
    /// </summary>
    /// <returns>The number of back-buffers in the swap chain.</returns>
    inline UInt32 buffers() const noexcept { return m_buffers; }
    ...
    inline bool enableVsync() const noexcept { return m_vsync; }
  };
...
public:
  ...
  [[nodiscard]] inline std::shared_ptr<TimingEvent> registerTimingEvent(
...
public:
  ...
  virtual void reset(Format surfaceFormat, const Size2d & renderArea, UInt32 buffers,
                     bool enableVsync = false) = 0;
...
public:
  ...
  public:
    ...
    inline UInt64 fence() const noexcept { return m_fence; }
  };
...
public:
  ...
  virtual void beginDebugRegion(const String & label,
                                const Vectors::ByteVector3 & color = {128_b, 128_b,
                                                                      128_b}) const noexcept {};
  ...
  virtual void setDebugMarker(const String & label,
                              const Vectors::ByteVector3 & color = {128_b, 128_b,
                                                                    128_b}) const noexcept {};
...
private:
  ...
  virtual UInt64 submitCommandBuffers(
    const Enumerable<SharedPtr<const ICommandBuffer>> & commandBuffers) const = 0;
...
public:
  ...
  inline UniquePtr<IBuffer> createBuffer(BufferType type, ResourceHeap heap, size_t elementSize,
                                         UInt32 elements = 1,
  ...
  /// <param name="usage">The intended usage for the buffer.</param>
  /// <returns>The instance of the buffer.</returns>
  inline UniquePtr<IBuffer> createBuffer(const IDescriptorSetLayout & descriptorSet, UInt32 binding,
  ...
  /// <param name="usage">The intended usage for the buffer.</param>
  /// <returns>The instance of the buffer.</returns>
  inline UniquePtr<IBuffer> createBuffer(const IPipeline & pipeline, UInt32 space, UInt32 binding,
                                         ResourceHeap heap, UInt32 elementSize, UInt32 elements,
                                         ResourceUsage usage = ResourceUsage::Default) const
  {
  ...
  /// <param name="usage">The intended usage for the buffer.</param>
  /// <returns>The instance of the buffer.</returns>
  inline UniquePtr<IBuffer> createBuffer(const String & name, BufferType type, ResourceHeap heap,
  ...
  /// <param name="usage">The intended usage for the buffer.</param>
  /// <returns>The instance of the buffer.</returns>
  inline UniquePtr<IBuffer> createBuffer(const String & name,
                                         const IDescriptorSetLayout & descriptorSet, UInt32 binding,
                                         ResourceHeap heap, UInt32 elements = 1,
                                         ResourceUsage usage = ResourceUsage::Default) const
  {
    auto & descriptor = descriptorSet.descriptor(binding);
  ...
  inline UniquePtr<IBuffer> createBuffer(const String & name,
                                         const IDescriptorSetLayout & descriptorSet, UInt32 binding,
                                         ResourceHeap heap, size_t elementSize, UInt32 elements,
                                         ResourceUsage usage = ResourceUsage::Default) const
  {
    auto & descriptor = descriptorSet.descriptor(binding);
  ...
  /// <param name="usage">The intended usage for the buffer.</param>
  /// <returns>The instance of the buffer.</returns>
  inline UniquePtr<IBuffer> createBuffer(const String & name, const IPipeline & pipeline,
                                         UInt32 space, UInt32 binding, ResourceHeap heap,
                                         UInt32 elements = 1,
                                         ResourceUsage usage = ResourceUsage::Default) const
  {
  ...
  inline UniquePtr<IVertexBuffer> createVertexBuffer(
  ...
  inline UniquePtr<IIndexBuffer> createIndexBuffer(
  ...
  inline UniquePtr<IImage> createTexture(Format format, const Size3d & size,
                                         ImageDimensions dimension = ImageDimensions::DIM_2,
                                         UInt32 levels = 1, UInt32 layers = 1,
                                         MultiSamplingLevel samples = MultiSamplingLevel::x1,
                                         ResourceUsage usage = ResourceUsage::Default) const
  {
  ...
  /// <returns>The instance of the texture.</returns>
  /// <seealso cref="createTextures" />
  inline UniquePtr<IImage> createTexture(const String & name, Format format, const Size3d & size,
                                         ImageDimensions dimension = ImageDimensions::DIM_2,
                                         UInt32 levels = 1, UInt32 layers = 1,
                                         MultiSamplingLevel samples = MultiSamplingLevel::x1,
                                         ResourceUsage usage = ResourceUsage::Default) const
  ...
  inline Enumerable<UniquePtr<IImage>> createTextures(
    UInt32 elements, Format format, const Size3d & size,
    ImageDimensions dimension = ImageDimensions::DIM_2, UInt32 layers = 1, UInt32 levels = 1,
    MultiSamplingLevel samples = MultiSamplingLevel::x1,
    ResourceUsage usage = ResourceUsage::Default) const
  {
  ...
  inline UniquePtr<ISampler> createSampler(
  ...
  inline Enumerable<UniquePtr<ISampler>> createSamplers(
...
private:
  virtual UniquePtr<IBuffer> getBuffer(BufferType type, ResourceHeap heap, size_t elementSize,
                                       UInt32 elements, ResourceUsage usage) const = 0;
  virtual UniquePtr<IBuffer> getBuffer(const String & name, BufferType type, ResourceHeap heap,
                                       size_t elementSize, UInt32 elements,
                                       ResourceUsage usage) const = 0;
  virtual UniquePtr<IVertexBuffer> getVertexBuffer(const IVertexBufferLayout & layout,
                                                   ResourceHeap heap, UInt32 elements,
                                                   ResourceUsage usage) const = 0;
  virtual UniquePtr<IVertexBuffer> getVertexBuffer(const String & name,
                                                   const IVertexBufferLayout & layout,
                                                   ResourceHeap heap, UInt32 elements,
                                                   ResourceUsage usage) const = 0;
  virtual UniquePtr<IIndexBuffer> getIndexBuffer(const IIndexBufferLayout & layout,
                                                 ResourceHeap heap, UInt32 elements,
                                                 ResourceUsage usage) const = 0;
  virtual UniquePtr<IIndexBuffer> getIndexBuffer(const String & name,
                                                 const IIndexBufferLayout & layout,
                                                 ResourceHeap heap, UInt32 elements,
                                                 ResourceUsage usage) const = 0;
  virtual UniquePtr<IImage> getTexture(Format format, const Size3d & size,
                                       ImageDimensions dimension, UInt32 levels, UInt32 layers,
                                       MultiSamplingLevel samples, ResourceUsage usage) const = 0;
  virtual UniquePtr<IImage> getTexture(const String & name, Format format, const Size3d & size,
                                       ImageDimensions dimension, UInt32 levels, UInt32 layers,
                                       MultiSamplingLevel samples, ResourceUsage usage) const = 0;
  virtual Enumerable<UniquePtr<IImage>> getTextures(UInt32 elements, Format format,
                                                    const Size3d & size, ImageDimensions dimension,
                                                    UInt32 layers, UInt32 levels,
                                                    MultiSamplingLevel samples,
                                                    ResourceUsage usage) const = 0;
  virtual UniquePtr<ISampler> getSampler(FilterMode magFilter, FilterMode minFilter,
                                         BorderMode borderU, BorderMode borderV, BorderMode borderW,
                                         MipMapMode mipMapMode, Float mipMapBias, Float maxLod,
                                         Float minLod, Float anisotropy) const = 0;
  virtual UniquePtr<ISampler> getSampler(const String & name, FilterMode magFilter,
                                         FilterMode minFilter, BorderMode borderU,
                                         BorderMode borderV, BorderMode borderW,
                                         MipMapMode mipMapMode, Float mipMapBias, Float maxLod,
                                         Float minLod, Float anisotropy) const = 0;
  virtual Enumerable<UniquePtr<ISampler>> getSamplers(UInt32 elements, FilterMode magFilter,
                                                      FilterMode minFilter, BorderMode borderU,
                                                      BorderMode borderV, BorderMode borderW,
                                                      MipMapMode mipMapMode, Float mipMapBias,
                                                      Float maxLod, Float minLod,
                                                      Float anisotropy) const = 0;
  virtual UniquePtr<IBottomLevelAccelerationStructure> getBlas(
    StringView name, AccelerationStructureFlags flags) const = 0;
  virtual UniquePtr<ITopLevelAccelerationStructure> getTlas(
    StringView name, AccelerationStructureFlags flags) const = 0;
};
...
public:
  ...
  inline void computeAccelerationStructureSizes(const IBottomLevelAccelerationStructure & blas,
  ...
  /// <param name="scratchSize">The size of the scratch memory buffer.</param>
  /// <param name="forUpdate">If set to `true`, <paramref name="scratchSize" /> will contain the amount of scratch memory required for an update.</param>
  inline void computeAccelerationStructureSizes(const ITopLevelAccelerationStructure & tlas,
                                                UInt64 & bufferSize, UInt64 & scratchSize,
                                                bool forUpdate = false) const
  {
...
private:
  virtual void getAccelerationStructureSizes(const IBottomLevelAccelerationStructure & blas,
                                             UInt64 & bufferSize, UInt64 & scratchSize,
                                             bool forUpdate) const = 0;
  virtual void getAccelerationStructureSizes(const ITopLevelAccelerationStructure & tlas,
                                             UInt64 & bufferSize, UInt64 & scratchSize,
                                             bool forUpdate) const = 0;
...
private:
  virtual UniquePtr<IBarrier> getNewBarrier(PipelineStage syncBefore,
                                            PipelineStage syncAfter) const noexcept = 0;
  virtual UniquePtr<IFrameBuffer> getNewFrameBuffer(StringView name,
                                                    const Size2d & renderArea) const noexcept = 0;
...
public:
  ...
  virtual const IGraphicsAdapter * findAdapter(
    const Optional<UInt64> & adapterId = std::nullopt) const = 0;
