...
#define LITEFX_DEFINE_FLAGS(T)                                                                     \
  ...
  {                                                                                                \
    using _base_t = std::underlying_type_t<T>;                                                     \
    return static_cast<T>(static_cast<_base_t>(lhs) | static_cast<_base_t>(rhs));                  \
  }                                                                                                \
...
#define LITEFX_FLAG_IS_SET(val, flag)                                                              \
  static_cast<bool>((std::to_underlying(val) & std::to_underlying(flag)) ==                        \
...
public:
  ...
  constexpr Enumerable(std::ranges::input_range auto && input) noexcept
    requires std::convertible_to<std::ranges::range_value_t<decltype(input)>, T>
  {
  ...
  constexpr Enumerable()
    requires std::default_initializable<array_type>
  = default;
...
public:
  ...
  constexpr iterator begin() noexcept { return m_elements.begin(); }

  /// <summary>
  /// Returns the iterator that points to the beginning of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the beginning of the `Enumerable`.</returns>
  constexpr const_iterator begin() const noexcept { return m_elements.begin(); }

  /// <summary>
  /// Returns the iterator that points to the beginning of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the beginning of the `Enumerable`.</returns>
  constexpr const_iterator cbegin() noexcept { return m_elements.cbegin(); }

  /// <summary>
  /// Returns the iterator that points to the beginning of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the beginning of the `Enumerable`.</returns>
  constexpr const_iterator cbegin() const noexcept { return m_elements.cbegin(); }
  ...
  constexpr iterator end() noexcept { return m_elements.end(); }

  /// <summary>
  /// Returns the iterator that points to the ending of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the ending of the `Enumerable`.</returns>
  constexpr const_iterator end() const noexcept { return m_elements.end(); }

  /// <summary>
  /// Returns the iterator that points to the ending of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the ending of the `Enumerable`.</returns>
  constexpr const_iterator cend() noexcept { return m_elements.cend(); }

  /// <summary>
  /// Returns the iterator that points to the ending of the `Enumerable`.
  /// </summary>
  /// <returns>The iterator that points to the ending of the `Enumerable`.</returns>
  constexpr const_iterator cend() const noexcept { return m_elements.cend(); }
  ...
  constexpr decltype(auto) front() { return m_elements.front(); }

  /// <summary>
  /// Returns the first element of the `Enumerable`, if it is not empty.
  /// </summary>
  /// <returns>The first element of the `Enumerable`, if it is not empty.</returns>
  constexpr decltype(auto) front() const { return m_elements.front(); }
};

#if (defined(BUILD_LITEFX_PIMPL) && BUILD_LITEFX_PIMPL) ||                                         \
  (!defined(BUILD_LITEFX_PIMPL)) && !defined(LITEFX_IMPLEMENTATION)
/// <summary>
...
public:
  ...
  constexpr PimplPtr(const PimplPtr & src) noexcept
  ...
  constexpr PimplPtr & operator=(const PimplPtr & src) noexcept
  {
    m_ptr.reset(new pImpl(*src.m_ptr));
    return *this;
  }
...
private:
  ...
  constexpr PimplPtr(pImpl * pimpl) noexcept
    : m_ptr(pimpl)
  {
  }
...
public:
  ...
  constexpr Implement(TInterface * parent)
    : m_parent(parent)
  {
    if (parent == nullptr)
...
protected:
  ...
  explicit Resource(const THandle handle) noexcept
    : m_handle(handle)
  {
  }
...
public:
  ...
  constexpr explicit Builder(TPointer && instance) noexcept
  ...
  constexpr Builder(Builder && _other) noexcept
    : m_instance(std::move(_other.m_instance))
  {
  }
...
public:
  ...
  /// <param name="instance">The instance of the object to build.</param>
  constexpr explicit Builder(TParent & parent, TPointer && instance) noexcept
    : m_parent(parent)
    , m_instance(std::move(instance))
  {
  }
