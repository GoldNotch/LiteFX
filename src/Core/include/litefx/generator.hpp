...
private:
  ...
  static void * _Allocate(_Alloc _Al, const size_t _Size)
  {
    ...
    else
    {
      ...
      const auto _Al_address = (reinterpret_cast<uintptr_t>(_Ptr) + _Size + alignof(_Alloc) - 1) &
                               ~(alignof(_Alloc) - 1);
...
public:
  ...
  static void * operator new(const size_t _Size, allocator_arg_t, const _Alloc2 & _Al,
                             const _Args &...)
  {
...
public:
  ...
  static void * operator new(const size_t _Size, allocator_arg_t, const _Alloc & _Al,
                             const _Args &...)
  {
  ...
  static void * operator new(const size_t _Size, const _This &, allocator_arg_t, const _Alloc & _Al,
                             const _Args &...)
  {
    static_assert(_Has_real_pointers<_Alloc>, "coroutine allocators must use true pointers");
    return _Allocate(_Al, _Size);
...
public:
  ...
  _NODISCARD auto final_suspend() noexcept { return _Final_awaiter{}; }
...
private:
  struct _Element_awaiter
  {
    ...
    constexpr void await_suspend(coroutine_handle<_Promise> _Handle) noexcept
    {
      ...
      _STL_INTERNAL_STATIC_ASSERT(
        is_pointer_interconvertible_base_of_v<_Gen_promise_base, _Promise>);
  ...
  {
    _NODISCARD bool await_ready() noexcept { return false; }
  ...
  struct _Nested_awaitable
  {
    ...
    explicit _Nested_awaitable(generator<_Rty, _Vty, _Alloc> && _Gen_) noexcept
      : _Gen(_STD move(_Gen_))
    {
    }

    _NODISCARD bool await_ready() noexcept { return !_Gen._Coro; }
...
public:
  ...
  _NODISCARD bool operator==(default_sentinel_t) const noexcept { return _Coro.done(); }
...
public:
  ...
  _NODISCARD _Gen_iter<_Value, _Ref> begin()
  {
    ...
    _Coro.resume();
    return _Gen_iter<_Value,
                     _Ref>{_Gen_secret_tag{},
                           coroutine_handle<_Gen_promise_base<_Gen_yield_t<_Ref>>>::from_address(
                             _Coro.address())};
  }

  _NODISCARD default_sentinel_t end() const noexcept { return default_sentinel; }
