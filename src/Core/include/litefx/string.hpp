...
constexpr auto Join(std::ranges::input_range auto && elements, StringView delimiter = ""sv) noexcept
  requires std::convertible_to<std::ranges::range_value_t<decltype(elements)>, String>
{
  return std::ranges::fold_left(elements | std::views::join_with(delimiter), String{},
                                std::plus<>{});
}

constexpr auto WJoin(std::ranges::input_range auto && elements,
                     WStringView delimiter = L""sv) noexcept
  requires std::convertible_to<std::ranges::range_value_t<decltype(elements)>, String>
{
  return std::ranges::fold_left(elements | std::views::join_with(delimiter), WString{},
                                std::plus<>{});
}
...
inline String Narrow(WStringView utf16)
{
  ...
  const auto size = ::WideCharToMultiByte(CP_UTF8, 0, utf16.data(), static_cast<int>(utf16.size()),
