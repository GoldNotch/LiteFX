...
#elif (defined(LiteFX_Backends_Vulkan_EXPORTS) || defined(__APPLE__)) && defined __GNUC__ &&       \
  __GNUC__ >= 4
...
/// </summary>
class LITEFX_VULKAN_API VulkanGraphicsAdapter final : public IGraphicsAdapter,
                                                      public Resource<VkPhysicalDevice>
{
...
/// </summary>
class LITEFX_VULKAN_API VulkanSurface final : public ISurface,
...
public:
  ...
  explicit VulkanPlatformException(VkResult result) noexcept
    : m_code(result)
    , RuntimeException("Operation returned {0}.", result)
  {
  }
  ...
  explicit VulkanPlatformException(VkResult result, StringView message) noexcept
    : m_code(result)
    , RuntimeException("{1} Operation returned {0}.", result, message)
  {
  }
...
public:
  ...
  VkResult code() const noexcept { return m_code; }
};
