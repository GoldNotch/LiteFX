...
using ImageBarrier =
  Tuple<ResourceAccess, ResourceAccess, const IVulkanImage &, Optional<ImageLayout>, ImageLayout,
...
PipelineStage VulkanBarrier::syncBefore() const noexcept { return m_impl->m_syncBefore; }

PipelineStage & VulkanBarrier::syncBefore() noexcept { return m_impl->m_syncBefore; }

PipelineStage VulkanBarrier::syncAfter() const noexcept { return m_impl->m_syncAfter; }

PipelineStage & VulkanBarrier::syncAfter() noexcept { return m_impl->m_syncAfter; }
...
void VulkanBarrier::transition(const IVulkanBuffer & buffer, ResourceAccess accessBefore,
                               ResourceAccess accessAfter)
{
  m_impl->m_bufferBarriers.push_back(
    {accessBefore, accessAfter, buffer, std::numeric_limits<UInt32>::max()});
}

void VulkanBarrier::transition(const IVulkanBuffer & buffer, UInt32 element,
...
void VulkanBarrier::transition(const IVulkanImage & image, ResourceAccess accessBefore,
...
void VulkanBarrier::transition(const IVulkanImage & image, UInt32 level, UInt32 levels,
...
void VulkanBarrier::execute(const VulkanCommandBuffer & commandBuffer) const noexcept
{
  ...
  auto globalBarriers =
    ...
    std::views::transform(
      [syncBefore, syncAfter](auto & barrier)
      {
  ...
  auto imageBarriers =
    ...
    std::views::transform(
      [syncBefore, syncAfter](auto & barrier)
      {
        ...
        auto currentLayout =
          Vk::getImageLayout(std::get<3>(barrier).value_or(ImageLayout::Undefined));
        ...
        return VkImageMemoryBarrier2{.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
                                     ...
                                     .subresourceRange =
                                       VkImageSubresourceRange{.aspectMask = image.aspectMask(
                                                                 std::get<9>(barrier)),
  ...
  {
    VkDependencyInfo barriers = {.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
