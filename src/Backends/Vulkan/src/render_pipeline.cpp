...
public:
  VulkanRenderPipelineImpl(VulkanRenderPipeline * parent, const VulkanRenderPass & renderPass,
                           bool alphaToCoverage, SharedPtr<VulkanPipelineLayout> layout,
...
public:
  VkPipeline initialize(MultiSamplingLevel samples)
  {
    ...
    // Setup dynamic state.
    Array<VkDynamicState> dynamicStates{VkDynamicState::VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT,
  ...
  VkPipeline initializeGraphicsPipeline(
    const VkPipelineDynamicStateCreateInfo & dynamicState,
    const LiteFX::Array<VkPipelineShaderStageCreateInfo> & shaderStages)
  {
    LITEFX_TRACE(VULKAN_LOG, "Creating render pipeline \"{1}\" for layout {0}...",
    ...
    rasterizerState.frontFace = rasterizer.cullOrder() == CullOrder::ClockWise
    ...
    LITEFX_TRACE(VULKAN_LOG, "Input assembler state: {{ PrimitiveTopology: {0} }}",
    ...
    std::ranges::for_each(
      vertexLayouts,
      [&, l = 0](const IVertexBufferLayout * layout) mutable
      {
        ...
        Array<VkVertexInputAttributeDescription> currentAttributes =
          bufferAttributes |
          std::views::transform(
            [&bufferAttributes, &bindingPoint, i = 0](const BufferAttribute * attribute) mutable
            {
              LITEFX_TRACE(VULKAN_LOG,
                           "\tAttribute {0}/{1}: {{ Location: {2}, Offset: {3}, Format: {4} }}",
                           ++i, bufferAttributes.size(), attribute->location(), attribute->offset(),
                           attribute->format());
        ...
        vertexInputAttributes.insert(std::end(vertexInputAttributes), std::begin(currentAttributes),
                                     std::end(currentAttributes));
    ...
    std::ranges::for_each(m_renderPass.renderTargets(),
                          [&colorBlendAttachments](const RenderTarget & renderTarget)
                          {
                            ...
                            colorBlendAttachments.push_back(VkPipelineColorBlendAttachmentState{
                              ...
                              .srcColorBlendFactor =
                                Vk::getBlendFactor(renderTarget.blendState().SourceColor),
                              .dstColorBlendFactor =
                                Vk::getBlendFactor(renderTarget.blendState().DestinationColor),
                              .colorBlendOp =
                                Vk::getBlendOperation(renderTarget.blendState().ColorOperation),
                              .srcAlphaBlendFactor =
                                Vk::getBlendFactor(renderTarget.blendState().SourceAlpha),
                              .dstAlphaBlendFactor =
                                Vk::getBlendFactor(renderTarget.blendState().DestinationAlpha),
                              .alphaBlendOp =
                                Vk::getBlendOperation(renderTarget.blendState().AlphaOperation),
                              .colorWriteMask = static_cast<VkColorComponentFlags>(
                                renderTarget.blendState().WriteMask)});
                          });
    ...
    depthStencilState.depthCompareOp =
      Vk::getCompareOp(rasterizer.depthStencilState().depthState().Operation);
    ...
    depthStencilState.front.compareOp =
      Vk::getCompareOp(rasterizer.depthStencilState().stencilState().FrontFace.Operation);
    depthStencilState.front.failOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().FrontFace.StencilFailOp);
    depthStencilState.front.passOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().FrontFace.StencilPassOp);
    depthStencilState.front.depthFailOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().FrontFace.DepthFailOp);
    ...
    depthStencilState.back.compareOp =
      Vk::getCompareOp(rasterizer.depthStencilState().stencilState().BackFace.Operation);
    depthStencilState.back.failOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().BackFace.StencilFailOp);
    depthStencilState.back.passOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().BackFace.StencilPassOp);
    depthStencilState.back.depthFailOp =
      Vk::getStencilOp(rasterizer.depthStencilState().stencilState().BackFace.DepthFailOp);
    ...
    VkPipeline pipeline;
    raiseIfFailed(::vkCreateGraphicsPipelines(m_renderPass.device().handle(), VK_NULL_HANDLE, 1,
    ...
    requires std::is_convertible_v<std::ranges::range_value_t<decltype(descriptorSets)>, UInt32>
  {
    ...
    m_frameBufferResizeTokens[interfacePointer] =
      frameBuffer.resized.add(std::bind(&VulkanRenderPipelineImpl::onFrameBufferResize, this,
                                        std::placeholders::_1, std::placeholders::_2));
    m_frameBufferReleaseTokens[interfacePointer] =
      frameBuffer.released.add(std::bind(&VulkanRenderPipelineImpl::onFrameBufferRelease, this,
                                         std::placeholders::_1, std::placeholders::_2));
  }
  ...
  void bindInputAttachments(const VulkanCommandBuffer & commandBuffer)
  {
    ...
    auto descriptorSets = m_inputAttachmentBindings.at(interfacePointer) |
...
bool VulkanRenderPipeline::alphaToCoverage() const noexcept { return m_impl->m_alphaToCoverage; }

MultiSamplingLevel VulkanRenderPipeline::samples() const noexcept { return m_impl->m_samples; }
...
void VulkanRenderPipeline::bind(const VulkanCommandBuffer & commandBuffer,
...
VulkanRenderPipelineBuilder::VulkanRenderPipelineBuilder(const VulkanRenderPass & renderPass,
                                                         const String & name)
