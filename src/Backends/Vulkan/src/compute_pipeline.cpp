...
public:
  ...
  {
    LITEFX_TRACE(VULKAN_LOG, "Creating compute pipeline (\"{1}\") for layout {0}...",
    ...
    VkPipeline pipeline;
    raiseIfFailed(::vkCreateComputePipelines(m_device.handle(), VK_NULL_HANDLE, 1, &pipelineInfo,
                                             nullptr, &pipeline),
...
VulkanComputePipeline::VulkanComputePipeline(const VulkanDevice & device) noexcept
  : m_impl(makePimpl<VulkanComputePipelineImpl>(this, device))
  , VulkanPipelineState(VK_NULL_HANDLE)
{
}
                                 ...
                                 Span<const VulkanDescriptorSet *> descriptorSets) const noexcept
{
  ...
  if (sets.empty()) [[unlikely]]
    ...
    ::vkCmdBindDescriptorSets(commandBuffer.handle(), VK_PIPELINE_BIND_POINT_COMPUTE,
...
VulkanComputePipelineBuilder::VulkanComputePipelineBuilder(const VulkanDevice & device,
                                                           const String & name)
