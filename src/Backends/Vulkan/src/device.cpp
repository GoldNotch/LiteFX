...
private:
  ...
  public:
      ...
      , m_type(type)
    {
      ...
      auto high = std::views::repeat(static_cast<Float>(QueuePriority::High) / 100.0f) |
                  std::views::take(chunkSize);
      auto low = std::views::repeat(static_cast<Float>(QueuePriority::Normal) / 100.0f) |
                 std::views::take(chunkSize + remainder);
    ...
    ~QueueFamily() noexcept { m_queues.clear(); }
  ...
  public:
    VulkanQueue * createQueue(const VulkanDevice & device, QueuePriority priority)
    {
      ...
      auto left = std::ranges::lower_bound(m_queuePriorities, static_cast<Float>(priority) / 100.0f,
                                           std::greater<float>{});
      auto right = std::ranges::upper_bound(m_queuePriorities,
                                            static_cast<Float>(priority) / 100.0f,
                                            std::greater<float>{});
      ...
      if (left == std::end(m_queuePriorities)) [[unlikely]]
      {
        ...
        switch (priority)
        {
          ...
          case High:
            nextPriority = QueuePriority::Realtime;
            break; // This will find the default queue
          case Realtime:
            [[unlikely]] // Should never actually happen, as there should be at least one queue in the family.
            default
              : throw RuntimeException(
                  "No queue found in this priority. Check if the queue family contains any queues.");
        }
      ...
      auto indices =
        std::views::iota(std::ranges::distance(std::cbegin(m_queuePriorities), left),
                         std::ranges::distance(std::cbegin(m_queuePriorities), right)) |
        std::views::transform(
          [this](auto i)
          {
            return std::make_tuple(i, std::ranges::count_if(m_queues, [i](const auto & q)
                                                            { return q->queueId() == i; }));
          });
      auto [queueId, refCount] =
        *std::ranges::min_element(indices, {}, [](const auto & i) { return std::get<1>(i); });

      LITEFX_DEBUG(VULKAN_LOG, "Creating queue with id {0} of type {2} (referenced {1} times).",
                   queueId, refCount, m_type);

      // Create a queue instance with the queue id.
      auto queue =
        makeUnique<VulkanQueue>(device, m_type, priority, m_id, static_cast<UInt32>(queueId));
...
public:
  VulkanDeviceImpl(VulkanDevice * parent, const VulkanGraphicsAdapter & adapter,
                   UniquePtr<VulkanSurface> && surface, const GraphicsDeviceFeatures & features,
...
private:
  void defineMandatoryExtensions(const GraphicsDeviceFeatures & features) noexcept
  {
    ...
    if (auto match =
          std::ranges::find_if(availableExtensions, [](const String & extension)
                               { return extension == VK_EXT_DEBUG_MARKER_EXTENSION_NAME; });
        match != availableExtensions.end())
...
public:
  void loadQueueFamilies()
  {
    ...
    ::vkGetPhysicalDeviceQueueFamilyProperties(m_adapter.handle(), &queueFamilies,
    ...
    auto families =
      ...
      std::views::transform(
        [i = 0](
          const VkQueueFamilyProperties & familyProperty) mutable -> Tuple<int, UInt32, QueueType>
        {
    ...
    std::ranges::sort(families, std::less<>{},
                      [](const auto & family) -> int
                      { return std::popcount(std::to_underlying(std::get<2>(family))); });
    ...
    m_families =
      ...
      std::views::transform(
        [](const auto & family)
        { return QueueFamily(std::get<0>(family), std::get<1>(family), std::get<2>(family)); }) |
  ...
  VkDevice initialize(const GraphicsDeviceFeatures & features)
  {
    if (!m_adapter.validateDeviceExtensions(m_extensions))
      throw InvalidArgumentException(
        "extensions", "Some required device extensions are not supported by the system.");

    auto const requiredExtensions =
      m_extensions |
      std::views::transform([](const auto & extension) { return extension.c_str(); }) |
      std::ranges::to<Array<const char *>>();
    ...
    auto maxQueueCount = std::ranges::max(
      m_families | std::views::transform([](auto & family) { return family.total(); }));
    Array<Float> priorities(maxQueueCount);
    std::ranges::generate(priorities,
                          [i = 0]() mutable {
                            return static_cast<Float>(i++ == 0 ? QueuePriority::High
    ...
    VkPhysicalDeviceVulkan12Features deviceFeatures12 =
      {.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES,
       ...
       .descriptorBindingUniformBufferUpdateAfterBind =
         true, // On NVidia cards this requires Turing or later.
    ...
    VkDeviceCreateInfo createInfo = {.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO,
                                     ...
                                     .enabledExtensionCount =
                                       static_cast<UInt32>(requiredExtensions.size()),
                                     .ppEnabledExtensionNames = requiredExtensions.data()};

    // Create the device.
    VkDevice device;
    raiseIfFailed(::vkCreateDevice(m_adapter.handle(), &createInfo, nullptr, &device),
                  "Unable to create Vulkan device.");

    // Load extension methods.
#ifndef NDEBUG
    debugMarkerSetObjectName = reinterpret_cast<PFN_vkDebugMarkerSetObjectNameEXT>(
    ...
    if (features.MeshShaders)
    {
      if (vkCmdDrawMeshTasks == nullptr)
        vkCmdDrawMeshTasks = reinterpret_cast<PFN_vkCmdDrawMeshTasksEXT>(
          ::vkGetDeviceProcAddr(device, "vkCmdDrawMeshTasksEXT"));

      if (vkCmdDrawMeshTasksIndirect)
        vkCmdDrawMeshTasksIndirect = reinterpret_cast<PFN_vkCmdDrawMeshTasksIndirectEXT>(
    ...
    if (features.RayTracing)
    {
      ...
      if (vkCmdCopyAccelerationStructure == nullptr)
        vkCmdCopyAccelerationStructure = reinterpret_cast<PFN_vkCmdCopyAccelerationStructureKHR>(
      ...
      if (vkCreateRayTracingPipelines == nullptr)
        vkCreateRayTracingPipelines = reinterpret_cast<PFN_vkCreateRayTracingPipelinesKHR>(
      ...
      if (vkCmdTraceRays == nullptr)
        vkCmdTraceRays = reinterpret_cast<PFN_vkCmdTraceRaysKHR>(
  ...
  void initializeDefaultQueues()
  {
    ...
    if (m_graphicsQueue == nullptr)
      throw RuntimeException(
        "Unable to find a fitting command queue to present the specified surface.");

    if (m_transferQueue == nullptr)
    {
      LITEFX_INFO(
        VULKAN_LOG,
        "Unable to find dedicated transfer queue for device-device transfer. Using default graphics queue instead.");
      m_transferQueue = m_graphicsQueue;
    }

    if (m_computeQueue == nullptr)
    {
      LITEFX_INFO(VULKAN_LOG,
                  "Unable to find dedicated compute queue. Using default graphics queue instead.");
      m_computeQueue = m_graphicsQueue;
    }
  }

  void createFactory() { m_factory = makeUnique<VulkanGraphicsFactory>(*m_parent); }

  void createSwapChain(Format format, const Size2d & renderArea, UInt32 backBuffers,
                       bool enableVsync)
  {
    m_swapChain =
      makeUnique<VulkanSwapChain>(*m_parent, format, renderArea, backBuffers, enableVsync);
  }
...
public:
                            ...
                            const VkSurfaceKHR & surface = VK_NULL_HANDLE)
  {
    ...
    auto match = std::ranges::find_if(m_families,
                                      [&](const auto & family)
                                      {
                                        ...
                                        if (surface != VK_NULL_HANDLE) [[unlikely]]
                                        {
                                          ...
                                          ::vkGetPhysicalDeviceSurfaceSupportKHR(m_adapter.handle(),
                                                                                 family.id(),
                                                                                 surface,
                                                                                 &canPresent);
...
VulkanDevice::VulkanDevice(const VulkanBackend & backend, const VulkanGraphicsAdapter & adapter,
                           UniquePtr<VulkanSurface> && surface, GraphicsDeviceFeatures features,
                           Span<String> extensions)
  : VulkanDevice(backend, adapter, std::move(surface), Format::B8G8R8A8_SRGB, {800, 600}, 3, false,
                 features, extensions)
{
}
...
VulkanDevice::VulkanDevice(const VulkanBackend & /*backend*/, const VulkanGraphicsAdapter & adapter,
  ...
  : Resource<VkDevice>(nullptr)
  , m_impl(makePimpl<VulkanDeviceImpl>(this, adapter, std::move(surface), features, extensions))
{
  ...
  LITEFX_DEBUG(VULKAN_LOG, "Available extensions: {0}",
               Join(adapter.getAvailableDeviceExtensions(), ", "));
...
Enumerable<UInt32> VulkanDevice::queueFamilyIndices(QueueType type) const noexcept
{
  return m_impl->m_families |
         std::views::filter(
           [type](const auto & family)
           { return type == QueueType::None || LITEFX_FLAG_IS_SET(family.type(), type); }) |
         std::views::transform([](const auto & family) { return family.id(); }) |
         std::ranges::to<Enumerable<UInt32>>();
}

VulkanSwapChain & VulkanDevice::swapChain() noexcept { return *m_impl->m_swapChain; }
...
VulkanRenderPassBuilder VulkanDevice::buildRenderPass(const String & name,
                                                      UInt32 commandBuffers) const
{
  return VulkanRenderPassBuilder(*this, commandBuffers, name);
}

VulkanRenderPipelineBuilder VulkanDevice::buildRenderPipeline(const VulkanRenderPass & renderPass,
...
VulkanRasterizerBuilder VulkanDevice::buildRasterizer() const { return VulkanRasterizerBuilder(); }
...
VulkanBarrierBuilder VulkanDevice::buildBarrier() const { return VulkanBarrierBuilder(); }
#endif // defined(LITEFX_BUILD_DEFINE_BUILDERS)

DeviceState & VulkanDevice::state() const noexcept { return m_impl->m_deviceState; }

const VulkanSwapChain & VulkanDevice::swapChain() const noexcept { return *m_impl->m_swapChain; }

const VulkanSurface & VulkanDevice::surface() const noexcept { return *m_impl->m_surface; }

const VulkanGraphicsAdapter & VulkanDevice::adapter() const noexcept { return m_impl->m_adapter; }

const VulkanGraphicsFactory & VulkanDevice::factory() const noexcept { return *m_impl->m_factory; }
...
UniquePtr<VulkanBarrier> VulkanDevice::makeBarrier(PipelineStage syncBefore,
                                                   PipelineStage syncAfter) const noexcept
{
  return makeUnique<VulkanBarrier>(syncBefore, syncAfter);
}

UniquePtr<VulkanFrameBuffer> VulkanDevice::makeFrameBuffer(StringView name,
...
void VulkanDevice::computeAccelerationStructureSizes(
  const VulkanBottomLevelAccelerationStructure & blas, UInt64 & bufferSize, UInt64 & scratchSize,
...
void VulkanDevice::computeAccelerationStructureSizes(
  const VulkanTopLevelAccelerationStructure & tlas, UInt64 & bufferSize, UInt64 & scratchSize,
  bool forUpdate) const
{
