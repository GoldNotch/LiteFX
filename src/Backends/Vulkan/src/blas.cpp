...
class VulkanBottomLevelAccelerationStructure::VulkanBottomLevelAccelerationStructureImpl
  : public Implement<VulkanBottomLevelAccelerationStructure>
{
public:
...
public:
  VulkanBottomLevelAccelerationStructureImpl(VulkanBottomLevelAccelerationStructure * parent,
                                             AccelerationStructureFlags flags)
    : base(parent)
    , m_flags(flags)
  {
    if (LITEFX_FLAG_IS_SET(flags, AccelerationStructureFlags::PreferFastBuild) &&
        LITEFX_FLAG_IS_SET(flags, AccelerationStructureFlags::PreferFastTrace)) [[unlikely]]
      throw InvalidArgumentException(
        "flags",
        "Cannot combine acceleration structure flags `PreferFastBuild` and `PreferFastTrace`.");
  }
    ...
    return [this]() -> std::generator<std::pair<UInt32, VkAccelerationStructureGeometryKHR>>
    {
      ...
      for (auto & mesh : m_triangleMeshes)
      {
        ...
        auto positionAttribute =
          std::ranges::find_if(attributes, [](const BufferAttribute * attribute)
                               { return attribute->semantic() == AttributeSemantic::Position; });

        if (positionAttribute == attributes.end()) [[unlikely]]
          throw RuntimeException(
            "A vertex buffer must contain a position attribute to be used in a bottom-level acceleration structure.");

        if ((*positionAttribute)->offset() != 0) [[unlikely]]
          throw RuntimeException(
            "The position attribute must not have a non-zero offset in the vertex buffer layout.");

        UInt32 primitiveCount = mesh.IndexBuffer == nullptr ? mesh.VertexBuffer->elements() / 3
                                                            : mesh.IndexBuffer->elements() / 3;

        co_yield std::
          make_pair(primitiveCount,
                           ...
                           {.sType =
                            ...
                            .indexType =
                              mesh.IndexBuffer == nullptr
                                ...
                                : (mesh.IndexBuffer->layout().indexType() == IndexType::UInt16
                                     ? VK_INDEX_TYPE_UINT16
                                     : VK_INDEX_TYPE_UINT32),
                            .indexData =
                              mesh.IndexBuffer == nullptr ? 0 : mesh.IndexBuffer->virtualAddress(),
                            .transformData = mesh.TransformBuffer == nullptr
                                             ? 0
                                             : mesh.TransformBuffer->virtualAddress()}},
      ...
      for (auto & bb : m_boundingBoxes)
      {
        if (bb.Buffer == nullptr) [[unlikely]]
          throw RuntimeException(
            "Cannot build bottom-level acceleration structure from uninitialized bounding boxes.");
...
VulkanBottomLevelAccelerationStructure::VulkanBottomLevelAccelerationStructure(
  AccelerationStructureFlags flags, StringView name)
...
UInt64 VulkanBottomLevelAccelerationStructure::offset() const noexcept { return m_impl->m_offset; }
...
UInt64 VulkanBottomLevelAccelerationStructure::size() const noexcept
{
  ...
  if (LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowCompaction) &&
      m_impl->m_queryPool != VK_NULL_HANDLE)
    res = ::vkGetQueryPoolResults(m_impl->m_device->handle(), m_impl->m_queryPool, 0, 1,
                                  sizeof(UInt64), &size, 0,
                                  VkQueryResultFlagBits::VK_QUERY_RESULT_64_BIT);

  return size;
}

void VulkanBottomLevelAccelerationStructure::build(const VulkanCommandBuffer & commandBuffer,
                                                   SharedPtr<const IVulkanBuffer> scratchBuffer,
                                                   SharedPtr<const IVulkanBuffer> buffer,
                                                   UInt64 offset, UInt64 maxSize)
{
  ...
  if (scratchBuffer != nullptr && scratchBuffer->size() < requiredScratchMemory)
    throw InvalidArgumentException(
      "scratchBuffer",
      "The provided scratch buffer does not contain enough memory to build the acceleration structure (contained memory: {0} bytes, required memory: {1} bytes).",
      scratchBuffer->size(), requiredScratchMemory);
  else if (scratchBuffer == nullptr)
    scratchBuffer = device.factory().createBuffer(BufferType::Storage, ResourceHeap::Resource,
                                                  requiredScratchMemory, 1,
                                                  ResourceUsage::AllowWrite);

  if (buffer == nullptr)
    buffer = m_impl->m_buffer && m_impl->m_buffer->size() >= requiredMemory
             ? m_impl->m_buffer
             : device.factory().createBuffer(BufferType::AccelerationStructure,
                                             ResourceHeap::Resource, requiredMemory, 1,
                                             ResourceUsage::AllowWrite);
  else if (maxSize < requiredMemory) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "maxSize", 0ull, maxSize, requiredMemory,
      "The maximum available size is not sufficient to contain the acceleration structure.");
  else if (buffer->size() < offset + requiredMemory) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "buffer", 0ull, (UInt64)buffer->size(), offset + requiredMemory,
      "The buffer does not contain enough memory after offset {0} to fully contain the acceleration structure.",
      offset);
  ...
  if (LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowCompaction))
  {
    if (m_impl->m_queryPool == VK_NULL_HANDLE)
    {
      ...
      raiseIfFailed(::vkCreateQueryPool(device.handle(), &queryPoolInfo, nullptr,
                                        &m_impl->m_queryPool),
                    "Unable to create query pool for compaction size queries.");
    }
  ...
  if (LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowCompaction))
  {
    auto barrier = device.makeBarrier(PipelineStage::AccelerationStructureBuild,
                                      PipelineStage::AccelerationStructureBuild);
    barrier->transition(*m_impl->m_buffer, ResourceAccess::AccelerationStructureWrite,
...
void VulkanBottomLevelAccelerationStructure::update(const VulkanCommandBuffer & commandBuffer,
                                                    SharedPtr<const IVulkanBuffer> scratchBuffer,
                                                    SharedPtr<const IVulkanBuffer> buffer,
                                                    UInt64 offset, UInt64 maxSize)
{
  ...
  if (m_impl->m_buffer == nullptr) [[unlikely]]
    throw RuntimeException(
      "The acceleration structure must have been built before it can be updated.");

  if (!LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowUpdate)) [[unlikely]]
    throw RuntimeException(
      "The acceleration structure does not allow updates. Specify `AccelerationStructureFlags::AllowUpdate` during creation.");
  ...
  if (scratchBuffer != nullptr && scratchBuffer->size() < requiredScratchMemory)
    throw InvalidArgumentException(
      "scratchBuffer",
      "The provided scratch buffer does not contain enough memory to update the acceleration structure (contained memory: {0} bytes, required memory: {1} bytes).",
      scratchBuffer->size(), requiredScratchMemory);
  else if (scratchBuffer == nullptr)
    scratchBuffer = device.factory().createBuffer(BufferType::Storage, ResourceHeap::Resource,
                                                  requiredScratchMemory, 1,
  ...
  if (buffer == nullptr)
    buffer = m_impl->m_buffer->size() >= requiredMemory
             ? m_impl->m_buffer
             : device.factory().createBuffer(BufferType::AccelerationStructure,
                                             ResourceHeap::Resource, requiredMemory, 1,
                                             ResourceUsage::AllowWrite);
...
void VulkanBottomLevelAccelerationStructure::copy(
  const VulkanCommandBuffer & commandBuffer, VulkanBottomLevelAccelerationStructure & destination,
  bool compress, SharedPtr<const IVulkanBuffer> buffer, UInt64 offset, bool copyBuildInfo) const
{
  // Validate the state.
  if (m_impl->m_buffer == nullptr) [[unlikely]]
    throw RuntimeException(
  ...
  if (compress && !LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowCompaction))
    [[unlikely]]
    throw RuntimeException(
      "The acceleration structure does not allow compaction. Specify `AccelerationStructureFlags::AllowCompaction` during creation.");
  ...
  if (!LITEFX_FLAG_IS_SET(m_impl->m_flags, AccelerationStructureFlags::AllowCompaction))
    ...
    raiseIfFailed(::vkGetQueryPoolResults(device.handle(), m_impl->m_queryPool, 0, 1,
                                          sizeof(UInt64), &requiredMemory, 0,
                                          VkQueryResultFlagBits::VK_QUERY_RESULT_64_BIT),
                  "Unable to query for compressed acceleration structure size.");

  // Validate the input arguments.
  if (buffer == nullptr)
    buffer = destination.m_impl->m_buffer->size() >= requiredMemory[0]
             ? destination.m_impl->m_buffer
             : device.factory().createBuffer(BufferType::AccelerationStructure,
                                             ResourceHeap::Resource, requiredMemory[0], 1,
                                             ResourceUsage::AllowWrite);
  else if (buffer->size() < offset + requiredMemory[0]) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "buffer", 0ull, (UInt64)buffer->size(), offset + requiredMemory[0],
      "The buffer does not contain enough memory after offset {0} to fully contain the acceleration structure.",
      offset);
  ...
  if (LITEFX_FLAG_IS_SET(destination.flags(), AccelerationStructureFlags::AllowCompaction))
  {
    ...
    if (destination.m_impl->m_queryPool == VK_NULL_HANDLE)
    {
      ...
      raiseIfFailed(::vkCreateQueryPool(device.handle(), &queryPoolInfo, nullptr,
                                        &destination.m_impl->m_queryPool),
                    "Unable to create query pool for compaction size queries.");
    }
  ...
  raiseIfFailed(::vkCreateAccelerationStructure(device.handle(), &info, nullptr,
                                                &destination.handle()),
                "Unable to update destination acceleration structure handle.");
  ...
  if (LITEFX_FLAG_IS_SET(destination.flags(), AccelerationStructureFlags::AllowCompaction))
  {
    auto barrier = device.makeBarrier(PipelineStage::AccelerationStructureCopy,
                                      PipelineStage::AccelerationStructureCopy);
    barrier->transition(*destination.m_impl->m_buffer, ResourceAccess::AccelerationStructureWrite,
                        ResourceAccess::AccelerationStructureRead);
    commandBuffer.barrier(*barrier);
    ::vkCmdWriteAccelerationStructuresProperties(
      commandBuffer.handle(), 1, &destination.handle(),
      VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR, destination.m_impl->m_queryPool, 0);
  }
...
void VulkanBottomLevelAccelerationStructure::addTriangleMesh(const TriangleMesh & mesh)
{
  if (m_impl->m_buffer != nullptr) [[unlikely]]
    throw RuntimeException(
      "An acceleration structure cannot be modified after buffers for it have been created.");

  if (!m_impl->m_boundingBoxes.empty()) [[unlikely]]
    throw RuntimeException(
      "A bottom-level acceleration structure can only contain either bounding boxes or triangle meshes, but not both at the same time.");
...
{
  if (auto match = std::ranges::find_if(m_impl->m_triangleMeshes, [&mesh](const auto & e)
                                        { return std::addressof(e) == std::addressof(mesh); });
      match != m_impl->m_triangleMeshes.end())
  {
...
{
  if (auto match = std::ranges::find_if(m_impl->m_boundingBoxes, [&aabb](const auto & e)
                                        { return std::addressof(e) == std::addressof(aabb); });
      match != m_impl->m_boundingBoxes.end())
  {
...
Array<std::pair<UInt32, VkAccelerationStructureGeometryKHR>>
VulkanBottomLevelAccelerationStructure::buildInfo() const
{
  return m_impl->build();
}

void VulkanBottomLevelAccelerationStructure::updateState(
  const VulkanDevice * device, VkAccelerationStructureKHR handle_) noexcept
{
  if (handle() != VK_NULL_HANDLE)
...
void VulkanBottomLevelAccelerationStructure::doBuild(const ICommandBuffer & commandBuffer,
                                                     SharedPtr<const IBuffer> scratchBuffer,
...
void VulkanBottomLevelAccelerationStructure::doUpdate(const ICommandBuffer & commandBuffer,
                                                      SharedPtr<const IBuffer> scratchBuffer,
...
void VulkanBottomLevelAccelerationStructure::doCopy(const ICommandBuffer & commandBuffer,
                                                    ...
                                                    UInt64 offset, bool copyBuildInfo) const
{
  copy(dynamic_cast<const VulkanCommandBuffer &>(commandBuffer),
       dynamic_cast<VulkanBottomLevelAccelerationStructure &>(destination), compress,
