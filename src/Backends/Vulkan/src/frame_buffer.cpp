...
public:
  VulkanFrameBufferImpl(VulkanFrameBuffer * parent, const VulkanDevice & device,
...
public:
  ...
  void initialize()
  {
    ...
    auto getImageView =
      [&](const UniquePtr<IVulkanImage> & image) -> std::pair<const IVulkanImage *, VkImageView>
    {
      ...
      raiseIfFailed(::vkCreateImageView(m_device.handle(), &createInfo, nullptr, &imageView),
    ...
    m_renderTargetHandles = m_images | std::views::transform(getImageView) |
                            std::ranges::to<Dictionary<const IVulkanImage *, VkImageView>>();

#ifndef NDEBUG
    // Set debug names.
    std::ranges::for_each(m_images,
                          [this](auto & image)
                          {
  ...
  void resize(const Size2d & renderArea)
  {
    ...
    auto images = m_images |
                  std::views::transform(
                    [&](const UniquePtr<IVulkanImage> & image)
                    {
                      auto format = image->format();
                      auto newImage =
                        m_device.factory().createTexture(image->name(), format, renderArea,
                      ...
                      if (::hasDepth(format) || ::hasStencil(format))
                        barrier->transition(*newImage, ResourceAccess::None, ResourceAccess::None,
...
const Size2d & VulkanFrameBuffer::size() const noexcept { return m_impl->m_size; }

size_t VulkanFrameBuffer::getWidth() const noexcept { return m_impl->m_size.width(); }

size_t VulkanFrameBuffer::getHeight() const noexcept { return m_impl->m_size.height(); }
...
void VulkanFrameBuffer::addImage(const String & name, Format format, MultiSamplingLevel samples,
                                 ResourceUsage usage)
{
  ...
  auto barrier = commandBuffer->makeBarrier(PipelineStage::None, PipelineStage::None);
  if (::hasDepth(format) || ::hasStencil(format))
    barrier->transition(*m_impl->m_images.back(), ResourceAccess::None, ResourceAccess::None,
                        ImageLayout::DepthRead);
  else
    barrier->transition(*m_impl->m_images.back(), ResourceAccess::None, ResourceAccess::None,
                        ImageLayout::ShaderResource);
...
void VulkanFrameBuffer::addImage(const String & name, const RenderTarget & renderTarget,
