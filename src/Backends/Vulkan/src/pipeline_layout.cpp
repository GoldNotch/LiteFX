...
public:
  VulkanPipelineLayoutImpl(VulkanPipelineLayout * parent, const VulkanDevice & device,
                           Enumerable<UniquePtr<VulkanDescriptorSetLayout>> && descriptorLayouts,
...
public:
  VkPipelineLayout initialize()
  {
    ...
    std::ranges::sort(m_descriptorSetLayouts, [](const UniquePtr<VulkanDescriptorSetLayout> & a,
                                                 const UniquePtr<VulkanDescriptorSetLayout> & b)
    ...
    for (Tuple spaces :
         m_descriptorSetLayouts |
           std::views::transform([](const UniquePtr<VulkanDescriptorSetLayout> & layout)
                                 { return layout->space(); }) |
    ...
    if (!emptySets.empty())
    {
      for (auto s : emptySets)
        m_descriptorSetLayouts.push_back(UniquePtr<
                                         VulkanDescriptorSetLayout>{new VulkanDescriptorSetLayout(
          m_device, {}, s,
          ShaderStage::Any)}); // No descriptor can ever be allocated from an empty descriptor set.
    ...
    auto descriptorSetLayouts =
      m_descriptorSetLayouts |
      std::views::transform([](const UniquePtr<VulkanDescriptorSetLayout> & layout)
                            { return std::as_const(*layout.get()).handle(); }) |
      std::ranges::to<Array<VkDescriptorSetLayout>>();

    // Query for push constant ranges.
    auto ranges = m_pushConstantsLayout == nullptr ? Enumerable<const VulkanPushConstantsRange *>{}
                                                   : m_pushConstantsLayout->ranges();
    auto rangeHandles =
      ranges |
      std::views::transform(
        [](const VulkanPushConstantsRange * range)
        {
          return VkPushConstantRange{.stageFlags = static_cast<VkShaderStageFlags>(
    ...
    LITEFX_TRACE(
      VULKAN_LOG,
      "Creating pipeline layout {0} {{ Descriptor Sets: {1}, Push Constant Ranges: {2} }}...",
      reinterpret_cast<void *>(m_parent), descriptorSetLayouts.size(), rangeHandles.size());
    ...
    raiseIfFailed(::vkCreatePipelineLayout(m_device.handle(), &pipelineLayoutInfo, nullptr,
                                           &layout),
...
VulkanPipelineLayout::VulkanPipelineLayout(
  ...
  UniquePtr<VulkanPushConstantsLayout> && pushConstantsLayout)
  : m_impl(makePimpl<VulkanPipelineLayoutImpl>(this, device, std::move(descriptorSetLayouts),
...
VulkanPipelineLayout::VulkanPipelineLayout(const VulkanDevice & device) noexcept
  : m_impl(makePimpl<VulkanPipelineLayoutImpl>(this, device))
  , Resource<VkPipelineLayout>(VK_NULL_HANDLE)
{
}
...
const VulkanDescriptorSetLayout & VulkanPipelineLayout::descriptorSet(UInt32 space) const
{
  if (auto match =
        std::ranges::find_if(m_impl->m_descriptorSetLayouts,
                             [&space](const UniquePtr<VulkanDescriptorSetLayout> & layout)
                             { return layout->space() == space; });
      match != m_impl->m_descriptorSetLayouts.end())
    return *match->get();

  throw InvalidArgumentException("space", "No descriptor set layout uses the provided space {0}.",
                                 space);
}
...
class VulkanPipelineLayoutBuilder::VulkanPipelineLayoutBuilderImpl
  : public Implement<VulkanPipelineLayoutBuilder>
{
public:
  friend class VulkanPipelineLayoutBuilder;
  friend class VulkanPipelineLayout;
...
VulkanPipelineLayoutBuilder::VulkanPipelineLayoutBuilder(const VulkanDevice & parent)
  : m_impl(makePimpl<VulkanPipelineLayoutBuilderImpl>(this, parent))
