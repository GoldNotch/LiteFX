...
public:
  VulkanBackendImpl(VulkanBackend * parent, const App & app, Span<String> extensions,
...
private:
    ...
    const VkDebugUtilsMessengerCallbackDataEXT * callbackData, void * userData)
  {
    ...
    switch (severity)
    {
      case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
        LITEFX_WARNING(VULKAN_LOG, "{1}: {0}", callbackData->pMessage, t);
        break;
      case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
        LITEFX_ERROR(VULKAN_LOG, "{1}: {0}", callbackData->pMessage, t);
        break;
      case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
        LITEFX_DEBUG(VULKAN_LOG, "{1}: {0}", callbackData->pMessage, t);
        break;
      default:
      case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
        LITEFX_TRACE(VULKAN_LOG, "{1}: {0}", callbackData->pMessage, t);
        break;
    }
  ...
  static VKAPI_ATTR VkBool32 VKAPI_CALL onDebugBreak(
    VkDebugUtilsMessageSeverityFlagBitsEXT severity, VkDebugUtilsMessageTypeFlagsEXT type,
    const VkDebugUtilsMessengerCallbackDataEXT * callbackData, void * userData)
  {
  ...
  VkInstance initialize()
  {
    ...
    if (!VulkanBackend::validateInstanceExtensions(m_extensions))
      throw InvalidArgumentException(
        "extensions", "Some required Vulkan extensions are not supported by the system.");
    ...
    if (!VulkanBackend::validateInstanceLayers(m_layers))
      throw InvalidArgumentException(
        "validationLayers", "Some required Vulkan layers are not supported by the system.");

    auto enabledLayers =
      m_layers | std::views::transform([this](const auto & layer) { return layer.c_str(); }) |
      std::ranges::to<Array<const char *>>();
    ...
    appInfo.applicationVersion =
      VK_MAKE_VERSION(m_app.version().major(), m_app.version().minor(), m_app.version().patch());
...
#endif
    VkInstance instance;
    raiseIfFailed(::vkCreateInstance(&createInfo, nullptr, &instance),
    ...
    vkQueueBeginDebugUtilsLabel =
      (PFN_vkQueueBeginDebugUtilsLabelEXT)vkGetInstanceProcAddr(instance,
                                                                "vkQueueBeginDebugUtilsLabelEXT");
    vkQueueEndDebugUtilsLabel =
      (PFN_vkQueueEndDebugUtilsLabelEXT)vkGetInstanceProcAddr(instance,
                                                              "vkQueueEndDebugUtilsLabelEXT");
    vkQueueInsertDebugUtilsLabel =
      (PFN_vkQueueInsertDebugUtilsLabelEXT)vkGetInstanceProcAddr(instance,
                                                                 "vkQueueInsertDebugUtilsLabelEXT");

    if (vkCreateDebugUtilsMessenger == nullptr)
      LITEFX_WARNING(
        VULKAN_LOG,
        "The debug messenger factory \"vkCreateDebugUtilsMessengerEXT\" could not be loaded. Debug utilities will not be enabled.");
    else if (vkDestroyDebugUtilsMessenger == nullptr)
      LITEFX_WARNING(
        VULKAN_LOG,
        "The debug messenger factory \"vkDestroyDebugUtilsMessengerEXT\" could not be loaded. Debug utilities will not be enabled.");
    else
    {
      auto result = vkCreateDebugUtilsMessenger(instance, &debugMessageCallbackInfo, nullptr,
                                                &m_debugMessenger);

      if (result == VK_ERROR_EXTENSION_NOT_PRESENT)
        LITEFX_WARNING(VULKAN_LOG,
                       "The extension \"{0}\" is not present. Debug utilities will not be enabled.",
      ...
      raiseIfFailed(vkCreateDebugUtilsMessenger(instance, &debugBreakCallbackInfo, nullptr,
                                                &m_debugBreaker),
                    "Unable to initialize debug break callback.");
  ...
  void loadAdapters() noexcept
  {
    ...
    m_adapters = handles |
                 std::views::transform([this](const auto & handle)
                                       { return makeUnique<VulkanGraphicsAdapter>(handle); }) |
...
VulkanBackend::VulkanBackend(const App & app, Span<String> extensions,
                             Span<String> validationLayers)
...
Enumerable<const VulkanGraphicsAdapter *> VulkanBackend::listAdapters() const
{
  return m_impl->m_adapters |
         std::views::transform([](const UniquePtr<VulkanGraphicsAdapter> & adapter)
                               { return adapter.get(); });
}

const VulkanGraphicsAdapter * VulkanBackend::findAdapter(const Optional<UInt64> & adapterId) const
{
  if (auto match = std::ranges::find_if(m_impl->m_adapters,
                                        [&adapterId](const auto & adapter) {
                                          return !adapterId.has_value() ||
                                                 adapter->uniqueId() == adapterId;
                                        });
      match != m_impl->m_adapters.end()) [[likely]]
    return match->get();
...
void VulkanBackend::registerDevice(String name, UniquePtr<VulkanDevice> && device)
{
  if (m_impl->m_devices.contains(name))
    throw InvalidArgumentException("name",
                                   "The backend already contains a device with the name \"{0}\".",
                                   name);

#ifndef NDEBUG
  device->setDebugName(*reinterpret_cast<const UInt64 *>(&std::as_const(*device).handle()),
  ...
  return std::ranges::all_of(extensions,
                             [&availableExtensions](const auto & extension)
                             {
                               ...
                               if (match == availableExtensions.end())
                                 LITEFX_ERROR(VULKAN_LOG,
                                              "Extension {0} is not supported by this instance.",
                                              extension);
...
Enumerable<String> VulkanBackend::getAvailableInstanceExtensions() noexcept
{
  ...
  return availableExtensions | std::views::transform([](const VkExtensionProperties & extension)
                                                     { return String(extension.extensionName); });
}
...
bool VulkanBackend::validateInstanceLayers(Span<const String> layers) noexcept
{
  ...
  return std::ranges::
    all_of(layers,
           [&availableLayers](const auto & layer)
           {
             auto match = std::ranges::find_if(availableLayers,
             ...
             if (match == availableLayers.end())
               LITEFX_ERROR(VULKAN_LOG, "Validation layer {0} is not supported by this instance.",
                            layer);
...
Enumerable<String> VulkanBackend::getInstanceValidationLayers() noexcept
{
  ...
  return availableLayers | std::views::transform([](const VkLayerProperties & layer)
