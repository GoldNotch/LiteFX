...
public:
    ...
    : base(parent)
  {
    ...
    {
      case VK_PHYSICAL_DEVICE_TYPE_CPU: m_type = GraphicsAdapterType::CPU; break;
      case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
      case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
      case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU: m_type = GraphicsAdapterType::GPU; break;
      default: m_type = GraphicsAdapterType::Other; break;
    }
    ...
    ::vkEnumerateDeviceExtensionProperties(m_parent->handle(), nullptr, &extensions,
                                           availableExtensions.data());
    m_deviceExtensions = availableExtensions |
                         std::views::transform([](const VkExtensionProperties & extension)
    ...
    m_deviceLayers = availableLayers |
...
VulkanGraphicsAdapter::VulkanGraphicsAdapter(VkPhysicalDevice adapter)
  : Resource<VkPhysicalDevice>(adapter)
  , m_impl(makePimpl<VulkanGraphicsAdapterImpl>(this))
{
}

VulkanGraphicsAdapter::~VulkanGraphicsAdapter() noexcept = default;

String VulkanGraphicsAdapter::name() const noexcept { return m_impl->m_name; }

UInt64 VulkanGraphicsAdapter::uniqueId() const noexcept { return m_impl->m_luid; }

UInt32 VulkanGraphicsAdapter::vendorId() const noexcept { return m_impl->m_vendorId; }

UInt32 VulkanGraphicsAdapter::deviceId() const noexcept { return m_impl->m_deviceId; }

GraphicsAdapterType VulkanGraphicsAdapter::type() const noexcept { return m_impl->m_type; }

UInt64 VulkanGraphicsAdapter::driverVersion() const noexcept { return m_impl->m_driverVersion; }

UInt32 VulkanGraphicsAdapter::apiVersion() const noexcept { return m_impl->m_apiVersion; }

VkPhysicalDeviceLimits VulkanGraphicsAdapter::limits() const noexcept { return m_impl->m_limits; }
                             ...
                             [&availableExtensions](const auto & extension)
                             {
                               ...
                               if (match == availableExtensions.end())
                                 LITEFX_ERROR(VULKAN_LOG,
                                              "Extension {0} is not supported by this adapter.",
                                              extension);
    ...
    all_of(layers,
           [&availableLayers](const auto & layer)
           {
             ...
             if (match == availableLayers.end())
               LITEFX_ERROR(VULKAN_LOG, "Validation layer {0} is not supported by this adapter.",
                            layer);
