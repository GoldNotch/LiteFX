...
class VulkanTopLevelAccelerationStructure::VulkanTopLevelAccelerationStructureImpl
  : public Implement<VulkanTopLevelAccelerationStructure>
{
public:
...
public:
  VulkanTopLevelAccelerationStructureImpl(VulkanTopLevelAccelerationStructure * parent,
    ...
    return m_instances |
           std::views::transform(
             [](const Instance & instance)
             {
               ...
               auto desc =
                 VkAccelerationStructureInstanceKHR{.instanceCustomIndex = instance.Id,
                                                    .mask = instance.Mask,
                                                    .instanceShaderBindingTableRecordOffset =
                                                      instance.HitGroupOffset,
                                                    .flags =
                                                      std::bit_cast<VkGeometryInstanceFlagsKHR>(
                                                        instance.Flags),
                                                    .accelerationStructureReference =
                                                      blasBuffer == nullptr
                                                        ? 0ull
                                                        : blasBuffer->virtualAddress() +
                                                            instance
                                                              .BottomLevelAccelerationStructure
                                                              ->offset()};

               std::memcpy(desc.transform.matrix, instance.Transform.elements(),
                           sizeof(Float) * 12);
               return desc;
             }) |
...
UInt64 VulkanTopLevelAccelerationStructure::offset() const noexcept { return m_impl->m_offset; }
...
void VulkanTopLevelAccelerationStructure::build(const VulkanCommandBuffer & commandBuffer,
                                                ...
                                                UInt64 offset, UInt64 maxSize)
{
  ...
  if (buffer == nullptr)
    buffer = m_impl->m_buffer != nullptr && m_impl->m_buffer->size() >= requiredMemory
             ? m_impl->m_buffer
...
void VulkanTopLevelAccelerationStructure::update(const VulkanCommandBuffer & commandBuffer,
                                                 SharedPtr<const IVulkanBuffer> scratchBuffer,
                                                 SharedPtr<const IVulkanBuffer> buffer,
                                                 UInt64 offset, UInt64 maxSize)
{
...
void VulkanTopLevelAccelerationStructure::copy(const VulkanCommandBuffer & commandBuffer,
                                               VulkanTopLevelAccelerationStructure & destination,
                                               bool compress, SharedPtr<const IVulkanBuffer> buffer,
...
void VulkanTopLevelAccelerationStructure::clear() noexcept { m_impl->m_instances.clear(); }

bool VulkanTopLevelAccelerationStructure::remove(const Instance & instance) noexcept
{
  if (auto match = std::ranges::find_if(m_impl->m_instances, [&instance](const auto & e)
                                        { return std::addressof(e) == std::addressof(instance); });
      match != m_impl->m_instances.end())
  {
...
Array<VkAccelerationStructureInstanceKHR> VulkanTopLevelAccelerationStructure::buildInfo()
  const noexcept
{
...
void VulkanTopLevelAccelerationStructure::doBuild(const ICommandBuffer & commandBuffer,
...
void VulkanTopLevelAccelerationStructure::doCopy(const ICommandBuffer & commandBuffer,
                                                 ITopLevelAccelerationStructure & destination,
