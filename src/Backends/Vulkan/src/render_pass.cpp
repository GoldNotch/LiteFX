...
public:
  void mapRenderTargets(Span<RenderTarget> renderTargets)
  {
    ...
    if (m_presentTarget != nullptr)
      m_swapChainTokens.push_back(
        m_swapChain.reseted.add(std::bind(&VulkanRenderPassImpl::onSwapChainReset, this,
  ...
  void registerFrameBuffer(const VulkanFrameBuffer & frameBuffer)
  {
    ...
    if (!m_frameBufferTokens.contains(interfacePointer)) [[unlikely]]
    {
      m_frameBufferTokens[interfacePointer] =
        frameBuffer.released.add(std::bind(&VulkanRenderPassImpl::onFrameBufferRelease, this,
                                           std::placeholders::_1, std::placeholders::_2));
...
#ifndef NDEBUG
        m_device.setDebugName(*reinterpret_cast<const UInt64 *>(
                                &std::as_const(*commandBuffer).handle()),
                              VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
                              std::format("{0} Primary Commands {1}", m_parent->name(),
...
#ifndef NDEBUG
            m_device
              .setDebugName(*reinterpret_cast<const UInt64 *>(
                              &std::as_const(*commandBuffer).handle()),
                            VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
  ...
  Array<VkRenderingAttachmentInfo> colorTargetContext(const VulkanFrameBuffer & frameBuffer)
  {
    return m_renderTargets |
           std::views::filter([](const RenderTarget & renderTarget)
                              { return renderTarget.type() != RenderTargetType::DepthStencil; }) |
           std::views::transform(
             [this, &frameBuffer](const RenderTarget & renderTarget)
             {
               ...
               VkRenderingAttachmentInfo attachmentInfo =
                 {.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                  ...
                  .loadOp = renderTarget.clearBuffer() ? VK_ATTACHMENT_LOAD_OP_CLEAR
                                                       : VK_ATTACHMENT_LOAD_OP_LOAD,
                  .storeOp = renderTarget.isVolatile() ? VK_ATTACHMENT_STORE_OP_NONE
                                                       : VK_ATTACHMENT_STORE_OP_STORE,
                  .clearValue = {renderTarget.clearValues().x(), renderTarget.clearValues().y(),
               ...
               if (renderTarget.type() == RenderTargetType::Present &&
                   frameBuffer[renderTarget].samples() > MultiSamplingLevel::x1)
               {
                 auto getImageView = [&](const IVulkanImage & image) -> VkImageView
                 {
                   ...
                   raiseIfFailed(::vkCreateImageView(m_device.handle(), &createInfo, nullptr,
                                                     &imageView),
                                 "Unable to create image view for swap chain back buffer.");
                   return imageView;
                 };
                 ...
                 attachmentInfo.resolveImageLayout =
                   Vk::getImageLayout(ImageLayout::ResolveDestination);
               }
  ...
  Optional<VkRenderingAttachmentInfo> depthTargetContext(const VulkanFrameBuffer & frameBuffer)
  {
    ...
    return VkRenderingAttachmentInfo{.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                                     ...
                                     .loadOp = m_depthStencilTarget->clearBuffer()
                                               ? VK_ATTACHMENT_LOAD_OP_CLEAR
                                               : VK_ATTACHMENT_LOAD_OP_LOAD,
                                     .storeOp = m_depthStencilTarget->isVolatile()
                                                ? VK_ATTACHMENT_STORE_OP_NONE
                                                : VK_ATTACHMENT_STORE_OP_STORE,
                                     .clearValue = {m_depthStencilTarget->clearValues().x(),
  ...
  Optional<VkRenderingAttachmentInfo> stencilTargetContext(const VulkanFrameBuffer & frameBuffer)
  {
    ...
    return VkRenderingAttachmentInfo{.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                                     ...
                                     .imageLayout =
                                       VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                                     .loadOp = m_depthStencilTarget->clearStencil()
                                               ? VK_ATTACHMENT_LOAD_OP_CLEAR
                                               : VK_ATTACHMENT_LOAD_OP_LOAD,
                                     .storeOp = m_depthStencilTarget->isVolatile()
                                                ? VK_ATTACHMENT_STORE_OP_NONE
                                                : VK_ATTACHMENT_STORE_OP_STORE,
                                     .clearValue = {m_depthStencilTarget->clearValues().y(),
                                                    m_depthStencilTarget->clearValues().y(),
                                                    m_depthStencilTarget->clearValues().y(),
                                                    m_depthStencilTarget->clearValues().y()}};
  }

  inline SharedPtr<VulkanCommandBuffer> getPrimaryCommandBuffer(
    const VulkanFrameBuffer & frameBuffer)
  {
    return m_primaryCommandBuffers.at(static_cast<const IFrameBuffer *>(&frameBuffer));
  }

  inline Array<SharedPtr<VulkanCommandBuffer>> & getSecondaryCommandBuffers(
    const VulkanFrameBuffer & frameBuffer)
  {
...
VulkanRenderPass::VulkanRenderPass(const VulkanDevice & device, Span<RenderTarget> renderTargets,
                                   Span<RenderPassDependency> inputAttachments,
                                   Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding,
                                   UInt32 secondaryCommandBuffers)
  : VulkanRenderPass(device, device.defaultQueue(QueueType::Graphics), renderTargets,
...
VulkanRenderPass::VulkanRenderPass(const VulkanDevice & device, const String & name,
                                   ...
                                   Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding,
                                   UInt32 secondaryCommandBuffers)
  : VulkanRenderPass(device, name, device.defaultQueue(QueueType::Graphics), renderTargets,
                     inputAttachments, inputAttachmentSamplerBinding, secondaryCommandBuffers)
{
}

VulkanRenderPass::VulkanRenderPass(const VulkanDevice & device, const VulkanQueue & queue,
                                   Span<RenderTarget> renderTargets,
                                   Span<RenderPassDependency> inputAttachments,
                                   Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding,
                                   UInt32 secondaryCommandBuffers)
  : m_impl(makePimpl<VulkanRenderPassImpl>(this, device, queue, renderTargets, inputAttachments,
...
const VulkanDevice & VulkanRenderPass::device() const noexcept { return m_impl->m_device; }
...
const VulkanQueue & VulkanRenderPass::commandQueue() const noexcept { return *m_impl->m_queue; }
...
const RenderTarget & VulkanRenderPass::renderTarget(UInt32 location) const
{
  if (auto match = std::ranges::find_if(m_impl->m_renderTargets,
      ...
      match != m_impl->m_renderTargets.end()) [[likely]]
    return *match;
...
{
  return std::ranges::any_of(m_impl->m_renderTargets, [](const auto & renderTarget)
                             { return renderTarget.type() == RenderTargetType::Present; });
}
...
const Optional<DescriptorBindingPoint> & VulkanRenderPass::inputAttachmentSamplerBinding()
  const noexcept
{
...
void VulkanRenderPass::begin(const VulkanFrameBuffer & frameBuffer) const
{
  ...
  VkRect2D renderArea = {0, 0, static_cast<UInt32>(frameBuffer.size().width()),
                         static_cast<UInt32>(frameBuffer.size().height())};
  ...
  std::ranges::for_each(m_impl->m_renderTargets,
                        [&renderTargetBarrier, &depthStencilBarrier,
                         &frameBuffer](const RenderTarget & renderTarget)
                        {
                          ...
                          if (renderTarget.type() == RenderTargetType::DepthStencil)
                            ...
                            renderTargetBarrier.transition(image, ResourceAccess::None,
  ...
  if (requiresResolve)
  {
    ...
    resolveBarrier.transition(backBufferImage, ResourceAccess::None, ResourceAccess::ResolveWrite,
                              ImageLayout::Undefined, ImageLayout::ResolveDestination);
...
UInt64 VulkanRenderPass::end() const
{
  ...
  auto secondaryHandles = m_impl->getSecondaryCommandBuffers(frameBuffer) |
                          std::views::transform(
                            ...
                            {
                              commandBuffer->end();
                              return std::as_const(*commandBuffer).handle();
                            }) |
                          std::ranges::to<Array<VkCommandBuffer>>();
  ::vkCmdExecuteCommands(std::as_const(*primaryCommandBuffer).handle(),
                         static_cast<UInt32>(secondaryHandles.size()), secondaryHandles.data());
                        ...
                        [&](const RenderTarget & renderTarget)
                        {
                          switch (renderTarget.type())
                          {
                            ...
                            case RenderTargetType::Color:
                              renderTargetBarrier.transition(frameBuffer[renderTarget],
                                                             ResourceAccess::RenderTarget,
                                                             ResourceAccess::None,
                                                             ImageLayout::RenderTarget,
                                                             ImageLayout::ShaderResource);
                            ...
                            case RenderTargetType::Present:
                              if (requiresResolve)
                                resolveBarrier.transition(frameBuffer[renderTarget],
                                                          ResourceAccess::RenderTarget,
                                                          ResourceAccess::None,
                                                          ImageLayout::RenderTarget,
                                                          ImageLayout::ShaderResource);
                              else
                                presentBarrier.transition(frameBuffer[renderTarget],
                                                          ResourceAccess::RenderTarget,
  ...
  if (requiresResolve)
  {
    ...
    presentBarrier.transition(backBufferImage, ResourceAccess::ResolveWrite, ResourceAccess::None,
  ...
  else if (this->hasPresentTarget())
  {
    ...
    endPresentBarrier.transition(presentTarget, ResourceAccess::TransferRead, ResourceAccess::None,
                                 ImageLayout::CopySource, ImageLayout::ShaderResource);
...
VulkanRenderPassBuilder::VulkanRenderPassBuilder(const VulkanDevice & device,
                                                 const String & name) noexcept
  : VulkanRenderPassBuilder(device, 1, name)
{
}

VulkanRenderPassBuilder::VulkanRenderPassBuilder(const VulkanDevice & device, UInt32 commandBuffers,
                                                 const String & name) noexcept
