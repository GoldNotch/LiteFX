...
public:
    ...
    , m_device(device)
  {
    ...
    raiseIfFailed(::vmaCreateAllocator(&allocatorInfo, &m_allocator),
                  "Unable to create Vulkan memory allocator.");
  }
...
UniquePtr<IVulkanBuffer> VulkanGraphicsFactory::createBuffer(BufferType type, ResourceHeap heap,
...
UniquePtr<IVulkanBuffer> VulkanGraphicsFactory::createBuffer(const String & name, BufferType type,
                                                             ResourceHeap heap, size_t elementSize,
                                                             UInt32 elements,
                                                             ResourceUsage usage) const
{
  // Validate inputs.
  ...
  buffer = VulkanBuffer::allocate(name, type, elements, elementSize, alignment, usage,
                                  m_impl->m_device, m_impl->m_allocator, bufferInfo, allocInfo);

#ifndef NDEBUG
  if (!name.empty())
    m_impl->m_device.setDebugName(*reinterpret_cast<const UInt64 *>(
                                    &std::as_const(*buffer).handle()),
...
UniquePtr<IVulkanVertexBuffer> VulkanGraphicsFactory::createVertexBuffer(
  const String & name, const VulkanVertexBufferLayout & layout, ResourceHeap heap, UInt32 elements,
  ResourceUsage usage) const
{
  // Validate usage.
  ...
  buffer = VulkanVertexBuffer::allocate(name, layout, elements, usage, m_impl->m_device,
                                        m_impl->m_allocator, bufferInfo, allocInfo);
  ...
  ResourceUsage usage) const
{
  ...
  buffer = VulkanIndexBuffer::allocate(name, layout, elements, usage, m_impl->m_device,
                                       m_impl->m_allocator, bufferInfo, allocInfo);
...
UniquePtr<IVulkanImage> VulkanGraphicsFactory::createTexture(Format format, const Size3d & size,
                                                             ImageDimensions dimension,
                                                             UInt32 levels, UInt32 layers,
  ...
  UInt32 layers, MultiSamplingLevel samples, ResourceUsage usage) const
{
  ...
  auto image = VulkanImage::allocate(name, m_impl->m_device, {width, height, depth}, format,
                                     dimension, levels, layers, samples, usage, m_impl->m_allocator,
                                     imageInfo, allocInfo);

#ifndef NDEBUG
  if (!name.empty())
    m_impl->m_device.setDebugName(*reinterpret_cast<const UInt64 *>(
                                    &std::as_const(*image).handle()),
...
Enumerable<UniquePtr<IVulkanImage>> VulkanGraphicsFactory::createTextures(
...
UniquePtr<IVulkanSampler> VulkanGraphicsFactory::createSampler(
...
{
  return makeUnique<VulkanSampler>(m_impl->m_device, magFilter, minFilter, borderU, borderV,
...
{
  auto sampler = makeUnique<VulkanSampler>(m_impl->m_device, magFilter, minFilter, borderU, borderV,
...
Enumerable<UniquePtr<IVulkanSampler>> VulkanGraphicsFactory::createSamplers(
...
UniquePtr<VulkanBottomLevelAccelerationStructure> VulkanGraphicsFactory::
...
UniquePtr<VulkanTopLevelAccelerationStructure> VulkanGraphicsFactory::
