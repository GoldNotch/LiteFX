  ...
  auto format(SpvReflectResult t, std::format_context & ctx)
  {
    ...
    switch (t)
    {
      ...
      case SPV_REFLECT_RESULT_ERROR_PARSE_FAILED:
        name = "SPV_REFLECT_RESULT_ERROR_PARSE_FAILED";
        break;
      case SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED:
        name = "SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED";
        break;
      case SPV_REFLECT_RESULT_ERROR_RANGE_EXCEEDED:
        name = "SPV_REFLECT_RESULT_ERROR_RANGE_EXCEEDED";
        break;
      case SPV_REFLECT_RESULT_ERROR_NULL_POINTER:
        name = "SPV_REFLECT_RESULT_ERROR_NULL_POINTER";
        break;
      case SPV_REFLECT_RESULT_ERROR_INTERNAL_ERROR:
        name = "SPV_REFLECT_RESULT_ERROR_INTERNAL_ERROR";
        break;
      case SPV_REFLECT_RESULT_ERROR_COUNT_MISMATCH:
        name = "SPV_REFLECT_RESULT_ERROR_COUNT_MISMATCH";
        break;
      case SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND:
        name = "SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_CODE_SIZE:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_CODE_SIZE";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_MAGIC_NUMBER:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_MAGIC_NUMBER";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_EOF:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_EOF";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_SET_NUMBER_OVERFLOW:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_SET_NUMBER_OVERFLOW";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_STORAGE_CLASS:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_STORAGE_CLASS";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_RECURSION:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_RECURSION";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_INSTRUCTION:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_INSTRUCTION";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_BLOCK_DATA:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_BLOCK_DATA";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_BLOCK_MEMBER_REFERENCE:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_BLOCK_MEMBER_REFERENCE";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ENTRY_POINT:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ENTRY_POINT";
        break;
      case SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_EXECUTION_MODE:
        name = "SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_EXECUTION_MODE";
        break;
  ...
  SharedPtr<VulkanPipelineLayout> reflectPipelineLayout()
  {
    ...
    std::ranges::for_each(
      ...
      [this, &descriptorSetLayouts,
       &pushConstantRanges](UniquePtr<VulkanShaderModule> & shaderModule)
      {
        ...
        if (result != SPV_REFLECT_RESULT_SUCCESS) [[unlikely]]
          throw RuntimeException("Unable to reflect shader module (Error {0:x}).",
                                 static_cast<UInt32>(reflection.GetResult()));

        // Get the number of descriptor sets and push constants.
        UInt32 descriptorSetCount, pushConstantCount;

        if ((result = reflection.EnumerateDescriptorSets(&descriptorSetCount, nullptr)) !=
            SPV_REFLECT_RESULT_SUCCESS) [[unlikely]]
          throw RuntimeException("Unable to get descriptor set count (Error {0:x}).",
                                 static_cast<UInt32>(result));

        if ((result = reflection.EnumeratePushConstants(&pushConstantCount, nullptr)) !=
            SPV_REFLECT_RESULT_SUCCESS) [[unlikely]]
          throw RuntimeException("Unable to get push constants count (Error {0:x}).",
                                 static_cast<UInt32>(result));
        ...
        if ((result =
               reflection.EnumerateDescriptorSets(&descriptorSetCount, descriptorSets.data())) !=
            SPV_REFLECT_RESULT_SUCCESS) [[unlikely]]
          throw RuntimeException("Unable to enumerate descriptor sets (Error {0:x}).",
                                 static_cast<UInt32>(result));

        if ((result =
               reflection.EnumeratePushConstants(&pushConstantCount, pushConstants.data())) !=
            SPV_REFLECT_RESULT_SUCCESS) [[unlikely]]
          throw RuntimeException("Unable to enumerate push constants (Error {0:x}).",
                                 static_cast<UInt32>(result));

        // Parse the descriptor sets.
        std::ranges::for_each(
          descriptorSets,
          [this, &shaderModule, &reflection,
           &descriptorSetLayouts](const SpvReflectDescriptorSet * descriptorSet)
          {
            ...
            std::ranges::generate(
              ...
              [&descriptorSet, i = 0]() mutable
              {
                ...
                switch (descriptor->descriptor_type)
                {
                  ...
                  case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
                    throw RuntimeException(
                      "The shader exposes a combined image samplers, which is currently not supported.");
                  case SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
                  case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
                    throw RuntimeException(
                      "The shader exposes a dynamic buffer, which is currently not supported.");
                  case SPV_REFLECT_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
                    type = DescriptorType::InputAttachment;
                    inputAttachmentIndex = descriptor->input_attachment_index;
                    break;
                  ...
                  case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER:
                  {
                    ...
                    if ((descriptor->type_description->members[0].type_flags &
                         SPV_REFLECT_TYPE_FLAG_STRUCT) == SPV_REFLECT_TYPE_FLAG_STRUCT)
                      type = (descriptor->resource_type & SPV_REFLECT_RESOURCE_FLAG_SRV) ==
                ...
                return DescriptorInfo{.location = descriptor->binding,
            ...
            if (!descriptorSetLayouts.contains(descriptorSet->set))
              descriptorSetLayouts.insert(
                std::make_pair(descriptorSet->set, DescriptorSetInfo{.space = descriptorSet->set,
            ...
            else
            {
              ...
              for (auto & descriptor : descriptors)
              {
                ...
                if (auto match =
                      std::ranges::find_if(layout.descriptors,
                                           [&descriptor](const DescriptorInfo & element)
                                           { return element.location == descriptor.location; });
                    match == layout.descriptors.end())
                  layout.descriptors.push_back(descriptor);
                else if (!descriptor.equals(*match))
                  LITEFX_WARNING(
                    VULKAN_LOG,
                    "Mismatching descriptors detected: the descriptor at location {0} ({3} elements with size of {4} bytes) of the descriptor set {1} in shader stage {2} conflicts with a descriptor from at least one other shader stage and will be dropped (conflicts with descriptor of type {8} in stage/s {5} with {6} elements of {7} bytes).",
                    descriptor.location, descriptorSet->set, shaderModule->type(),
                    descriptor.elements, descriptor.elementSize, layout.stage, match->elements,
                    match->elementSize, match->type);
              }
        ...
        if (pushConstantCount > 1)
          LITEFX_WARNING(
            VULKAN_LOG,
            "More than one push constant range detected for shader stage {0}. If you have multiple entry points, you may be able to split them up into different shader files.",
            shaderModule->type());

        std::ranges::for_each(pushConstants,
                              [this, &shaderModule, &reflection,
                               &pushConstantRanges](const SpvReflectBlockVariable * pushConstant)
                              {
                                pushConstantRanges.push_back(
                                  PushConstantRangeInfo{.stage = shaderModule->type(),
                                                        .offset = pushConstant->absolute_offset,
                                                        .size = pushConstant->padded_size});
                              });
      });

    // Create the descriptor set layouts.
    auto descriptorSets = [this, &descriptorSetLayouts]()
      -> std::generator<UniquePtr<VulkanDescriptorSetLayout>>
    {
      for (auto it = descriptorSetLayouts.begin(); it != descriptorSetLayouts.end(); ++it)
      {
        ...
        auto descriptorLayouts =
          [&descriptorSet]() -> std::generator<UniquePtr<VulkanDescriptorLayout>>
        {
          for (auto descriptor = descriptorSet.descriptors.begin();
            ...
            co_yield descriptor->type == DescriptorType::InputAttachment
              ? makeUnique<VulkanDescriptorLayout>(descriptor->type, descriptor->location,
                                                   descriptor->inputAttachmentIndex)
              : makeUnique<VulkanDescriptorLayout>(descriptor->type, descriptor->location,
                                                   descriptor->elementSize, descriptor->elements);
        }() | std::views::as_rvalue;

        co_yield makeUnique<VulkanDescriptorSetLayout>(m_device, std::move(descriptorLayouts),
                                                       descriptorSet.space, descriptorSet.stage);
      }
    }() | std::views::as_rvalue |
           std::ranges::to<Enumerable<UniquePtr<VulkanDescriptorSetLayout>>>();

    // Create the push constants layout.
    auto pushConstants = [&pushConstantRanges]()
      -> std::generator<UniquePtr<VulkanPushConstantsRange>>
    {
      for (auto it = pushConstantRanges.begin(); it != pushConstantRanges.end(); ++it)
        co_yield makeUnique<
          VulkanPushConstantsRange>(it->stage, it->offset, it->size, 0,
                                    0); // No space or binding for Vulkan push constants.
    }() | std::views::as_rvalue |
    ...
    auto pushConstantsLayout =
      makeUnique<VulkanPushConstantsLayout>(std::move(pushConstants), overallSize);

    // Return the pipeline layout.
    return makeShared<VulkanPipelineLayout>(m_device, std::move(descriptorSets),
...
SharedPtr<VulkanShaderProgram> VulkanShaderProgram::create(
...
{
  return m_impl->m_modules | std::views::transform([](const UniquePtr<VulkanShaderModule> & shader)
                                                   { return shader.get(); });
}
...
class VulkanShaderProgramBuilder::VulkanShaderProgramBuilderImpl
  : public Implement<VulkanShaderProgramBuilder>
{
public:
  friend class VulkanShaderProgramBuilder;
...
VulkanShaderProgramBuilder::VulkanShaderProgramBuilder(const VulkanDevice & device)
  : m_impl(makePimpl<VulkanShaderProgramBuilderImpl>(this, device))
