...
public:
    ...
    , m_device(device)
  {
    ...
    if (!m_supportsTiming)
      LITEFX_WARNING(
        VULKAN_LOG,
        "Timestamp queries are not supported and will be disabled. Reading timestamps will always return 0.");
  }
...
public:
  void initialize(Format format, const Size2d & renderArea, UInt32 buffers, bool vsync)
  {
    ...
    if (auto match = std::ranges::find_if(surfaceFormats, [format](Format surfaceFormat)
        ...
        match != surfaceFormats.end()) [[likely]]
      selectedFormat = *match;
    else [[unlikely]]
      throw InvalidArgumentException("format",
                                     "The requested format is not supported by this device.");
    ...
    createInfo.imageExtent.height =
      std::max<UInt32>(1u, std::clamp(static_cast<UInt32>(renderArea.height()),
                                      deviceCaps.minImageExtent.height,
                                      deviceCaps.maxImageExtent.height));
    createInfo.imageExtent.width =
      std::max<UInt32>(1u, std::clamp(static_cast<UInt32>(renderArea.width()),
                                      deviceCaps.minImageExtent.width,
                                      deviceCaps.maxImageExtent.width));
    ...
    LITEFX_TRACE(
      VULKAN_LOG,
      "Creating swap chain for device {0} {{ Images: {1}, Extent: {2}x{3} Px, Format: {4}, VSync: {5} }}...",
      reinterpret_cast<const void *>(&m_device), images, createInfo.imageExtent.width,
      createInfo.imageExtent.height, selectedFormat, vsync);

    // Log if something needed to be changed.
    [[unlikely]] if (selectedFormat != format)
      LITEFX_INFO(VULKAN_LOG, "The format {0} has been changed to the compatible format {1}.",
                  format, selectedFormat);

    [[unlikely]] if (createInfo.imageExtent.height != static_cast<UInt32>(renderArea.height()) ||
                     createInfo.imageExtent.width != static_cast<UInt32>(renderArea.width()))
      LITEFX_INFO(VULKAN_LOG, "The render area has been adjusted to {0}x{1} Px (was {2}x{3} Px).",
                  createInfo.imageExtent.height, createInfo.imageExtent.width,
                  static_cast<UInt32>(renderArea.height()),
                  static_cast<UInt32>(renderArea.width()));

    [[unlikely]] if (images != buffers)
      LITEFX_INFO(VULKAN_LOG, "The number of buffers has been adjusted from {0} to {1}.", buffers,
                  images);

    // Create the swap chain instance.
    VkSwapchainKHR swapChain;
    raiseIfFailed(::vkCreateSwapchainKHR(m_device.handle(), &createInfo, nullptr, &swapChain),
                  "Swap chain could not be created.");

    // Initialize the fences used to wait for image access.
    VkFenceCreateInfo fenceInfo{.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
    raiseIfFailed(::vkCreateFence(m_device.handle(), &fenceInfo, nullptr, &m_waitForImage),
                  "Unable to create image acquisition fence.");
    ...
    std::ranges::generate(m_waitForWorkload,
                          [&]()
                          {
                            ...
                            raiseIfFailed(::vkCreateSemaphore(m_device.handle(), &semaphoreInfo,
                                                              nullptr, &semaphore),
                                          "Unable to workload synchronization semaphore.");
                            return semaphore;
                          });

    // Create the swap chain images.
    auto actualRenderArea = Size2d(static_cast<size_t>(createInfo.imageExtent.width),
                                   static_cast<size_t>(createInfo.imageExtent.height));
    ...
    m_presentImages = imageChain |
                      std::views::transform(
                        [this, &actualRenderArea, &selectedFormat](const VkImage & image)
                        {
                          return makeUnique<VulkanImage>(m_device, image,
                                                         Size3d{actualRenderArea.width(),
                                                                actualRenderArea.height(), 1},
                                                         selectedFormat, ImageDimensions::DIM_2, 1,
                                                         1, MultiSamplingLevel::x1,
  ...
  void resetQueryPools(const Array<SharedPtr<TimingEvent>> & timingEvents)
  {
    ...
    std::ranges::generate(m_timingQueryPools,
                          [this, &timingEvents]()
                          {
                            ...
                            VkQueryPool pool;
                            raiseIfFailed(::vkCreateQueryPool(m_device.handle(), &poolInfo, nullptr,
                                                              &pool),
                                          "Unable to allocate timestamp query pool.");
  ...
  void cleanup()
  {
    ...
    std::ranges::for_each(m_waitForWorkload, [&](VkSemaphore semaphore)
  ...
  UInt32 swapBackBuffer()
  {
    ...
    raiseIfFailed(
      ::vkAcquireNextImageKHR(m_device.handle(), m_handle, UINT64_MAX, VK_NULL_HANDLE,
                              m_waitForImage, &m_currentImage),
      "Unable to swap front buffer. Make sure that all previously acquired images are actually presented before acquiring another image.");
    raiseIfFailed(::vkWaitForFences(m_device.handle(), 1, &m_waitForImage, VK_TRUE, UINT64_MAX),
                  "Unable to wait for image acquisition.");
    raiseIfFailed(::vkResetFences(m_device.handle(), 1, &m_waitForImage),
                  "Unable to reset image acquisition fence.");
    ...
    if (m_supportsTiming && !m_timingEvents.empty()) [[likely]]
    {
      ...
      auto result =
        ::vkGetQueryPoolResults(m_device.handle(), m_currentQueryPool, 0, m_timestamps.size(),
                                m_timestamps.size() * sizeof(UInt64), m_timestamps.data(),
  ...
  void present(UInt64 fence)
  {
    ...
    raiseIfFailed(::vkQueueSubmit(queue.handle(), 1, &submitInfo, VK_NULL_HANDLE),
                  "Unable to submit workload semaphore.");
    ...
    raiseIfFailed(::vkQueuePresentKHR(queue.handle(), &presentInfo),
                  "Unable to present swap chain.");
  }

  const VkQueryPool & currentTimestampQueryPool() { return m_currentQueryPool; }

public:
  Array<Format> getSurfaceFormats(const VkPhysicalDevice adapter,
                                  const VkSurfaceKHR surface) const noexcept
  {
    ...
    return availableFormats |
           std::views::transform([](const VkSurfaceFormatKHR & format)
                                 { return Vk::getFormat(format.format); }) |
           std::ranges::to<Array<Format>>();
  }

  VkColorSpaceKHR findColorSpace(const VkPhysicalDevice adapter, const VkSurfaceKHR surface,
                                 Format format) const noexcept
  {
    ...
    if (auto match =
          std::ranges::find_if(availableFormats, [&format](const VkSurfaceFormatKHR & surfaceFormat)
                               { return surfaceFormat.format == Vk::getFormat(format); });
        match != availableFormats.end()) [[likely]]
...
#warning                                                                                           \
  "Vulkan SDK version is below 1.3.268.0, which will cause false validation errors about invalid command buffer resets. This bug has been fixed in later versions."
#else
#pragma message(                                                                                   \
  "Note: Vulkan SDK version is below 1.3.268.0, which will cause false validation errors about invalid command buffer resets. This bug has been fixed in later versions.")
#endif
#endif
    ...
    , m_device(device)
  {
    ...
    if (importSemaphoreWin32HandleKHR == nullptr) [[unlikely]]
      throw RuntimeException(
        "Semaphore importing is not available. Check if all required extensions are available.");
  }
...
public:
  void initialize(Format format, const Size2d & renderArea, UInt32 buffers, bool vsync)
  {
    ...
    // Get the number of images in the swap chain.
    VkSurfaceCapabilitiesKHR deviceCaps;
    ::vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_device.adapter().handle(),
                                                m_device.surface().handle(), &deviceCaps);
    UInt32 images = std::clamp(buffers, deviceCaps.minImageCount, deviceCaps.maxImageCount);
    ...
    Size2d extent(std::max(1u, std::clamp(static_cast<UInt32>(renderArea.width()),
                                          deviceCaps.minImageExtent.width,
                                          deviceCaps.maxImageExtent.width)),
                  std::max(1u, std::clamp(static_cast<UInt32>(renderArea.height()),
                                          deviceCaps.minImageExtent.height,
                                          deviceCaps.maxImageExtent.height)));

    [[unlikely]] if (extent.height() != static_cast<UInt32>(renderArea.height()) ||
                     extent.width() != static_cast<UInt32>(renderArea.width()))
      LITEFX_INFO(VULKAN_LOG, "The render area has been adjusted to {0}x{1} Px (was {2}x{3} Px).",
                  extent.height(), extent.width(), static_cast<UInt32>(renderArea.height()),
                  static_cast<UInt32>(renderArea.width()));
    ...
    m_vsync = vsync;
#ifndef NDEBUG
    D3D::raiseIfFailed(::CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&factory)),
                       "Unable to crate D3D12 factory for interop.");
#else
    D3D::raiseIfFailed(::CreateDXGIFactory2(0, IID_PPV_ARGS(&factory)),
                       "Unable to crate D3D12 factory for interop.");
#endif // NDEBUG

    if (FAILED(factory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &tearingSupport,
                                            sizeof(tearingSupport)))) [[unlikely]]
      LITEFX_WARNING(
        VULKAN_LOG,
        "Unable to check support for variable refresh rates. Feature will be disabled.");

    // Query the DXGI adapter.
    ComPtr<IDXGIAdapter1> adapter;
    auto adapterId = m_device.adapter().uniqueId();
    D3D::raiseIfFailed(factory->EnumAdapterByLuid(*reinterpret_cast<LUID *>(&adapterId),
                                                  IID_PPV_ARGS(&adapter)),
                       "Unable to query adapter \"{0:#x}\".", adapterId);

    // Create a D3D device.
    D3D::raiseIfFailed(::D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_11_1,
    ...
    if (FAILED(m_d3dDevice.As(&infoQueue)))
      LITEFX_WARNING(
        VULKAN_LOG,
        "Unable to query info queue on D3D interop device. Debugger support will be disabled disabled.");
    else
    {
      ...
      D3D12_MESSAGE_SEVERITY severities[] = {
        D3D12_MESSAGE_SEVERITY_INFO}; // Somehow it is required to deny info-level messages. Otherwise strange pointer issues are occurring.
      ...
      D3D::raiseIfFailed(infoQueue->PushStorageFilter(&infoQueueFilter),
                         "Unable to push message filter to info queue of D3D interop device.");
    }
#endif // NDEBUG
    ...
    D3D::raiseIfFailed(m_d3dDevice->CreateCommandQueue(&presentQueueDesc,
                                                       IID_PPV_ARGS(&m_presentQueue)),
                       "Unable to create present queue.");

    // Create the swap chain instance.
    LITEFX_TRACE(
      VULKAN_LOG,
      "Creating swap chain for device {0} {{ Images: {1}, Extent: {2}x{3} Px, Format: {4}, VSync: {5} }}...",
      reinterpret_cast<const void *>(&m_device), images, extent.width(), extent.height(),
      selectedFormat, vsync);

    DXGI_SWAP_CHAIN_DESC1 swapChainDesc{.Width = static_cast<UInt32>(extent.width()),
                                        ...
                                        .Flags =
                                          (m_supportsTearing = static_cast<bool>(tearingSupport))
                                            ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING
                                            : (UInt32)0};

    ComPtr<IDXGISwapChain1> swapChain;
    auto hwnd = m_device.surface().windowHandle();
    D3D::raiseIfFailed(factory->CreateSwapChainForHwnd(m_presentQueue.Get(), hwnd, &swapChainDesc,
                                                       nullptr, nullptr, &swapChain),
                       "Unable to create interop swap chain.");
    D3D::raiseIfFailed(swapChain.As(&m_swapChain),
    ...
    if (FAILED(factory->MakeWindowAssociation(hwnd, DXGI_MWA_NO_ALT_ENTER))) [[unlikely]]
      LITEFX_WARNING(VULKAN_LOG,
                     "Unable disable keyboard control sequence for full-screen switching.");

    // Initialize the present fences array.
    m_presentFences.resize(images, 0ul);

    // Create fences for synchronization.
    D3D::raiseIfFailed(m_d3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                                IID_PPV_ARGS(&m_presentationFence)),
                       "Unable to create presentation synchronization fence for swap chain.");
    D3D::raiseIfFailed(m_d3dDevice->CreateFence(0, D3D12_FENCE_FLAG_SHARED,
                                                IID_PPV_ARGS(&m_workloadFence)),
                       "Unable to create interop synchronization fence for swap chain.");
    D3D::
      raiseIfFailed(m_d3dDevice->CreateSharedHandle(m_workloadFence.Get(), nullptr, GENERIC_ALL,
                                                    L"", &m_fenceHandle),
                    "Unable to create shared handle for swap chain interop synchronization fence.");
    ...
    raiseIfFailed(importSemaphoreWin32HandleKHR(m_device.handle(), &fenceImportInfo),
                  "Unable to import interop synchronization fence for swap chain.");
    ...
    {
      D3D::raiseIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                             IID_PPV_ARGS(
                                                               &m_presentCommandAllocators[i])),
                         "Unable to create command allocator for present queue commands.");
      D3D::raiseIfFailed(m_d3dDevice->CreateCommandList1(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                         D3D12_COMMAND_LIST_FLAG_NONE,
                                                         IID_PPV_ARGS(&m_presentCommandLists[i])),
                         "Unable to create command list for present queue commands.");
    }
  }
  ...
  void reset(Format format, const Size2d & renderArea, UInt32 buffers, bool vsync)
  {
    ...
    std::ranges::for_each(m_presentImages,
                          [this](const auto & image) {
                            ::vkDestroyImage(m_device.handle(), std::as_const(*image).handle(),
                                             nullptr);
                          });
    ...
    LITEFX_TRACE(
      VULKAN_LOG,
      "Resetting swap chain for device {0} {{ Images: {1}, Extent: {2}x{3} Px, Format: {4}, VSync: {5} }}...",
      reinterpret_cast<const void *>(&m_device), images, extent.width(), extent.height(),
      selectedFormat, vsync);
    ...
    D3D::raiseIfFailed(m_swapChain->ResizeBuffers(buffers, static_cast<UInt32>(extent.width()),
                                                  static_cast<UInt32>(extent.height()),
                                                  DX12::getFormat(format),
                                                  m_supportsTearing
                                                    ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING
                                                    : 0),
  ...
  void createImages(Format format, const Size2d & renderArea, UInt32 buffers)
  {
    ...
    std::ranges::generate(
      m_presentImages,
      [this, &renderArea, &format, i = 0]() mutable
      {
        ...
        D3D12_RESOURCE_DESC imageDesc = {.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D,
        ...
        D3D::raiseIfFailed(m_d3dDevice->CreateCommittedResource(&heapInfo, D3D12_HEAP_FLAG_SHARED,
                                                                &imageDesc,
                                                                D3D12_RESOURCE_STATE_COMMON,
                                                                nullptr, IID_PPV_ARGS(&resource)),
                           "Unable to create image resource to interop back buffer.");
        D3D::raiseIfFailed(m_d3dDevice->CreateSharedHandle(resource.Get(), nullptr, GENERIC_ALL,
                                                           nullptr, &resourceHandle),
                           "Unable to create shared handle for interop back buffer.");

        // Wrap the back buffer images in an vulkan image.
        const VkExternalMemoryImageCreateInfo
          wrapperInfo{.sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO,
        ...
        const VkImageCreateInfo imageInfo{.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
                                          ...
                                          .extent = {std::max<UInt32>(1, renderArea.width()),
                                                     std::max<UInt32>(1, renderArea.height()), 1},
                                          .mipLevels = 1,
                                          .arrayLayers = 1,
        ...
        // Create the image.
        VkImage backBuffer;
        raiseIfFailed(::vkCreateImage(m_device.handle(), &imageInfo, nullptr, &backBuffer),
                      "Unable to create swap-chain image.");
        ...
        {
          if ((memoryRequirements.memoryTypeBits & (1 << m)) &&
              memoryProperties.memoryTypes[m].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
          {
            memoryType = m;
            break;
          }
        ...
        VkMemoryDedicatedAllocateInfo memoryInfo{.sType =
        ...
        VkMemoryAllocateInfo allocationInfo{.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
                                            .pNext = &importInfo,
                                            .allocationSize = memoryRequirements.size,
                                            .memoryTypeIndex = memoryType};

        VkDeviceMemory imageMemory;
        raiseIfFailed(::vkAllocateMemory(m_device.handle(), &allocationInfo, nullptr, &imageMemory),
                      "Unable to allocate memory for imported interop swap chain buffer.");
        raiseIfFailed(::vkBindImageMemory(m_device.handle(), backBuffer, imageMemory, 0),
                      "Unable to bind back-buffer.");
        ...
        return makeUnique<VulkanImage>(m_device, backBuffer,
                                       Size3d{imageInfo.extent.width, imageInfo.extent.height,
                                              imageInfo.extent.depth},
                                       format, ImageDimensions::DIM_2, 1, 1, MultiSamplingLevel::x1,
                                       ResourceUsage::TransferDestination);
      });
  ...
  UInt32 swapBackBuffer()
  {
    ...
    if (m_presentationFence->GetCompletedValue() < m_presentFences[m_currentImage])
    {
      ...
      HRESULT hr =
        m_presentationFence->SetEventOnCompletion(m_presentFences[m_currentImage], eventHandle);
    ...
    {
      auto result =
        ::vkGetQueryPoolResults(m_device.handle(), m_timingQueryPools[m_currentImage], 0,
                                m_timestamps.size(), m_timestamps.size() * sizeof(UInt64),
                                m_timestamps.data(), sizeof(UInt64), VK_QUERY_RESULT_64_BIT);

      if (result != VK_NOT_READY) // Initial frames do not yet contain query results.
        raiseIfFailed(result, "Unable to query timing events.");

      // Reset the query pool.
      ::vkResetQueryPool(m_device.handle(), m_timingQueryPools[m_currentImage], 0,
                         m_timestamps.size());
    }
  ...
  void present(UInt64 fence)
  {
    ...
    ComPtr<ID3D12Resource> resource;
    D3D::raiseIfFailed(m_swapChain->GetBuffer(m_currentImage, IID_PPV_ARGS(&resource)),
                       "Unable to acquire image resource from swap chain back buffer {0}.",
                       m_currentImage);

    auto & allocator = m_presentCommandAllocators[m_currentImage];
    auto & commandList = m_presentCommandLists[m_currentImage];
    D3D::raiseIfFailed(allocator->Reset(),
                       "Unable to reset command allocator before presentation.");
    D3D::raiseIfFailed(commandList->Reset(allocator.Get(), nullptr),
                       "Unable to reset command list before presentation.");

    // Transition into copy destination state and copy the resource.
    D3D12_TEXTURE_BARRIER barrier = {.SyncBefore = D3D12_BARRIER_SYNC_NONE,
    ...
    D3D12_BARRIER_GROUP barrierGroup = {.Type = D3D12_BARRIER_TYPE_TEXTURE,
                                        .NumBarriers = 1,
    ...
    // Do the presentation.
    if (m_vsync)
      D3D::raiseIfFailed(m_swapChain->Present(1, 0),
                         "Unable to queue present event on swap chain.");
    else
      D3D::raiseIfFailed(m_swapChain->Present(0,
                                              m_supportsTearing ? DXGI_PRESENT_ALLOW_TEARING : 0),
                         "Unable to queue present event on swap chain.");

    D3D::raiseIfFailed(m_presentQueue->Signal(m_presentationFence.Get(),
                                              m_presentFences[m_currentImage]),
                       "Unable to signal presentation fence.");
  }

  const VkQueryPool & currentTimestampQueryPool() { return m_timingQueryPools[m_currentImage]; }

public:
  Enumerable<Format> getSurfaceFormats(const VkPhysicalDevice adapter,
                                       const VkSurfaceKHR surface) const noexcept
  {
    uint32_t formats;
    ::vkGetPhysicalDeviceSurfaceFormatsKHR(adapter, surface, &formats, nullptr);
    ...
    return availableFormats | std::views::transform([](const VkSurfaceFormatKHR & format)
                                                    { return Vk::getFormat(format.format); });
  }
...
private:
  void waitForInteropDevice()
  {
    ...
    D3D::raiseIfFailed(m_d3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence)),
                       "Unable to create queue synchronization fence on interop device.");
...
VulkanSwapChain::VulkanSwapChain(const VulkanDevice & device, Format surfaceFormat,
                                 const Size2d & renderArea, UInt32 buffers, bool enableVsync)
...
SharedPtr<TimingEvent> VulkanSwapChain::timingEvent(UInt32 queryId) const
{
  if (queryId >= m_impl->m_timingEvents.size())
    throw ArgumentOutOfRangeException("queryId", 0u,
                                      static_cast<UInt32>(m_impl->m_timingEvents.size()), queryId,
                                      "No timing event has been registered for query ID {0}.",
                                      queryId);
...
UInt64 VulkanSwapChain::readTimingEvent(SharedPtr<const TimingEvent> timingEvent) const
{
  ...
  if (auto match =
        std::find(m_impl->m_timingEvents.begin(), m_impl->m_timingEvents.end(), timingEvent);
      match != m_impl->m_timingEvents.end()) [[likely]]
    return m_impl->m_timestamps[std::distance(m_impl->m_timingEvents.begin(), match)];

  throw InvalidArgumentException("timingEvent",
                                 "The timing event is not registered on the swap chain.");
}
...
Format VulkanSwapChain::surfaceFormat() const noexcept { return m_impl->m_format; }

UInt32 VulkanSwapChain::buffers() const noexcept { return m_impl->m_buffers; }

const Size2d & VulkanSwapChain::renderArea() const noexcept { return m_impl->m_renderArea; }

bool VulkanSwapChain::verticalSynchronization() const noexcept { return m_impl->m_vsync; }

IVulkanImage * VulkanSwapChain::image(UInt32 backBuffer) const
{
  if (backBuffer >= m_impl->m_presentImages.size()) [[unlikely]]
    throw ArgumentOutOfRangeException("backBuffer", 0u,
                                      static_cast<UInt32>(m_impl->m_presentImages.size()),
                                      backBuffer, "The back buffer must be a valid index.");
...
Enumerable<IVulkanImage *> VulkanSwapChain::images() const noexcept
{
  return m_impl->m_presentImages |
         std::views::transform([](UniquePtr<IVulkanImage> & image) { return image.get(); });
}

void VulkanSwapChain::present(UInt64 fence) const { m_impl->present(fence); }

Enumerable<Format> VulkanSwapChain::getSurfaceFormats() const noexcept
{
  return m_impl->getSurfaceFormats(m_impl->m_device.adapter().handle(),
                                   m_impl->m_device.surface().handle());
}
