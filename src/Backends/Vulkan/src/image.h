...
/// </summary>
class VulkanImage : public virtual IVulkanImage,
...
public:
  explicit VulkanImage(const VulkanDevice & device, VkImage image, const Size3d & extent,
                       Format format, ImageDimensions dimensions, UInt32 levels, UInt32 layers,
                       MultiSamplingLevel samples, ResourceUsage usage,
                       VmaAllocator allocator = nullptr, VmaAllocation allocation = nullptr,
                       const String & name = "");
...
public:
  static UniquePtr<VulkanImage> allocate(const VulkanDevice & device, const Size3d & extent,
                                         Format format, ImageDimensions dimensions, UInt32 levels,
                                         UInt32 layers, MultiSamplingLevel samples,
                                         ResourceUsage usage, VmaAllocator & allocator,
                                         const VkImageCreateInfo & createInfo,
                                         const VmaAllocationCreateInfo & allocationInfo,
                                         VmaAllocationInfo * allocationResult = nullptr);
  static UniquePtr<VulkanImage> allocate(
    ...
    const VmaAllocationCreateInfo & allocationInfo, VmaAllocationInfo * allocationResult = nullptr);
};

/// <summary>
/// Implements a Vulkan <see cref="ISampler" />.
/// </summary>
class VulkanSampler : public virtual IVulkanSampler,
...
public:
  ...
  explicit VulkanSampler(const VulkanDevice & device, FilterMode magFilter = FilterMode::Nearest,
                         ...
                         Float minLod = 0.f, Float maxLod = std::numeric_limits<Float>::max(),
                         Float anisotropy = 0.f, const String & name = "");
