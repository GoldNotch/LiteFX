...
public:
  VulkanQueueImpl(VulkanQueue * parent, const VulkanDevice & device, QueueType type,
                  QueuePriority priority, UInt32 familyId, UInt32 queueId)
    : base(parent)
    , m_type(type)
    , m_priority(priority)
    , m_familyId(familyId)
...
public:
  ...
  VkQueue initialize()
  {
    ...
    VkSemaphoreTypeCreateInfo timelineCreateInfo = {.sType =
    ...
    raiseIfFailed(::vkCreateSemaphore(m_device.handle(), &createInfo, nullptr,
                                      &m_timelineSemaphore),
                  "Unable to create queue synchronization semaphore.");
  ...
  void releaseCommandBuffers(UInt64 beforeFence)
  {
    ...
    const auto [from, to] =
      std::ranges::remove_if(m_submittedCommandBuffers,
                             [this, &beforeFence](auto & pair)
                             {
...
VulkanQueue::VulkanQueue(const VulkanDevice & device, QueueType type, QueuePriority priority,
...
const VulkanDevice & VulkanQueue::device() const noexcept { return m_impl->m_device; }

UInt32 VulkanQueue::familyId() const noexcept { return m_impl->m_familyId; }

UInt32 VulkanQueue::queueId() const noexcept { return m_impl->m_queueId; }
...
QueueType VulkanQueue::type() const noexcept { return m_impl->m_type; }

#ifdef LITEFX_BUILD_SUPPORT_DEBUG_MARKERS
void VulkanQueue::beginDebugRegion(const String & label,
...
QueuePriority VulkanQueue::priority() const noexcept { return m_impl->m_priority; }

SharedPtr<VulkanCommandBuffer> VulkanQueue::createCommandBuffer(bool beginRecording,
...
UInt64 VulkanQueue::submit(SharedPtr<const VulkanCommandBuffer> commandBuffer) const
{
  ...
  if (commandBuffer->isSecondary()) [[unlikely]]
    throw InvalidArgumentException("commandBuffer",
  ...
  ::vkGetSemaphoreCounterValue(m_impl->m_device.handle(), m_impl->m_timelineSemaphore,
                               &completedValue);
  ...
  VkCommandBufferSubmitInfo commandBufferInfo = {.sType =
...
UInt64 VulkanQueue::submit(
  const Enumerable<SharedPtr<const VulkanCommandBuffer>> & commandBuffers) const
{
  if (!std::ranges::all_of(commandBuffers, [](const auto & buffer) { return buffer != nullptr; }))
    [[unlikely]]
    throw InvalidArgumentException("commandBuffers",
                                   "At least one command buffer is not initialized.");

  if (!std::ranges::all_of(commandBuffers,
                           [](const auto & buffer) { return !buffer->isSecondary(); })) [[unlikely]]
    throw InvalidArgumentException(
      "commandBuffers",
      "At least one command buffer is a secondary command buffer, which is not allowed to be submitted to a command queue.");
  ...
  auto commandBufferInfos = [&commandBuffers]() -> std::generator<VkCommandBufferSubmitInfo>
  {
    for (auto buffer = commandBuffers.begin(); buffer != commandBuffers.end(); ++buffer)
    {
      ...
      co_yield VkCommandBufferSubmitInfo{.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_SUBMIT_INFO,
  ...
  VkSubmitInfo2 submitInfo = {.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO_2,
                              .commandBufferInfoCount =
                                static_cast<UInt32>(commandBufferInfos.size()),
  ...
  std::ranges::for_each(commandBuffers,
                        [this, &fence](auto & buffer) {
                          m_impl->m_submittedCommandBuffers.push_back({fence, buffer});
                        });
