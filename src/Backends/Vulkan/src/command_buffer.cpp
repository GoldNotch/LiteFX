...
public:
  ...
  VkCommandBuffer initialize()
  {
    ...
    raiseIfFailed(::vkCreateCommandPool(m_queue.device().handle(), &poolInfo, nullptr,
                                        &m_commandPool),
                  "Unable to create command pool.");

    // Create the command buffer.
    VkCommandBufferAllocateInfo bufferInfo = {.sType =
    ...
    raiseIfFailed(::vkAllocateCommandBuffers(m_queue.device().handle(), &bufferInfo, &buffer),
                  "Unable to allocate command buffer.");

    return buffer;
  }

  inline void buildAccelerationStructure(VulkanBottomLevelAccelerationStructure & blas,
                                         const SharedPtr<const IVulkanBuffer> scratchBuffer,
                                         const IVulkanBuffer & buffer, UInt64 offset, bool update)
  {
    ...
    raiseIfFailed(::vkCreateAccelerationStructure(device.handle(), &info, nullptr, &handle),
                  "Unable to update acceleration structure handle.");

    auto buildInfo = blas.buildInfo();
    auto descriptions = buildInfo | std::views::values |
                        std::ranges::to<Array<VkAccelerationStructureGeometryKHR>>();
    auto ranges = buildInfo | std::views::keys |
                  std::views::transform(
                    [](UInt32 primitives) {
                      return VkAccelerationStructureBuildRangeInfoKHR{.primitiveCount = primitives};
                    }) |
  ...
  inline void buildAccelerationStructure(VulkanTopLevelAccelerationStructure & tlas,
                                         const SharedPtr<const IVulkanBuffer> scratchBuffer,
                                         const IVulkanBuffer & buffer, UInt64 offset, bool update)
  {
    if (scratchBuffer == nullptr) [[unlikely]]
    ...
    auto instanceBuffer =
      device.factory().createBuffer(BufferType::Storage, ResourceHeap::Dynamic,
                                    sizeof(VkAccelerationStructureInstanceKHR) * buildInfo.size(),
                                    1, ResourceUsage::AccelerationStructureBuildInput);

    // Map the instance buffer.
    instanceBuffer->map(buildInfo.data(),
                        sizeof(VkAccelerationStructureInstanceKHR) * buildInfo.size());
    ...
    VkAccelerationStructureGeometryInstancesDataKHR instanceInfo =
      {.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_INSTANCES_DATA_KHR,
       ...
       .data = {.deviceAddress = instanceBuffer->virtualAddress()}};

    VkAccelerationStructureGeometryKHR geometryInfo =
      {.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR,
       .geometryType = VK_GEOMETRY_TYPE_INSTANCES_KHR,
       .geometry = {.instances = instanceInfo}};
...
const ICommandQueue & VulkanCommandBuffer::queue() const noexcept { return m_impl->m_queue; }
...
void VulkanCommandBuffer::begin(const VulkanRenderPass & renderPass) const
{
  ...
  auto formats =
    renderTargets |
    std::views::filter([](auto & renderTarget)
                       { return renderTarget.type() != RenderTargetType::DepthStencil; }) |
    std::views::transform([](auto & renderTarget)
                          { return Vk::getFormat(renderTarget.format()); }) |
    std::ranges::to<Array<VkFormat>>();
  auto depthStencilFormats =
    renderTargets |
    std::views::filter([](auto & renderTarget)
                       { return renderTarget.type() == RenderTargetType::DepthStencil; }) |
    std::views::transform([](auto & renderTarget) { return renderTarget.format(); }) |
    std::ranges::to<Array<Format>>();
  auto depthFormat = depthStencilFormats.size() > 0 && ::hasDepth(depthStencilFormats.front())
                     ? Vk::getFormat(depthStencilFormats.front())
                     : VK_FORMAT_UNDEFINED;
  auto stencilFormat = depthStencilFormats.size() > 0 && ::hasStencil(depthStencilFormats.front())
                       ? Vk::getFormat(depthStencilFormats.front())
                       : VK_FORMAT_UNDEFINED;
  ...
  auto samples =
    renderTargets |
    std::views::transform([&frameBuffer](auto & renderTarget)
                          { return Vk::getSamples(frameBuffer[renderTarget].samples()); }) |
    std::ranges::to<Array<VkSampleCountFlagBits>>();

  if (std::ranges::adjacent_find(samples, std::not_equal_to{}) != samples.end()) [[unlikely]]
    throw RuntimeException(
      "All render targets of the current render pass must use the multi sampling level.");
...
bool VulkanCommandBuffer::isSecondary() const noexcept { return m_impl->m_secondary; }
...
void VulkanCommandBuffer::setViewports(Span<const IViewport *> viewports) const noexcept
{
  auto vps = viewports |
             std::views::transform(
               [](const auto & viewport)
               {
                 return VkViewport{.x = viewport->getRectangle().x(),
                                   .y = viewport->getRectangle().y(),
                                   .width = viewport->getRectangle().width(),
...
{
  auto vp = VkViewport{.x = viewport->getRectangle().x(),
                       .y = viewport->getRectangle().y(),
                       .width = viewport->getRectangle().width(),
                       .height = viewport->getRectangle().height(),
                       .minDepth = viewport->getMinDepth(),
...
void VulkanCommandBuffer::setScissors(Span<const IScissor *> scissors) const noexcept
{
  auto scs = scissors |
             std::views::transform(
               [](const auto & scissor)
               {
                 return VkRect2D{{.x = static_cast<Int32>(scissor->getRectangle().x()),
...
{
  auto s = VkRect2D{{.x = static_cast<Int32>(scissor->getRectangle().x()),
                     .y = static_cast<Int32>(scissor->getRectangle().y())},
                    {.width = static_cast<UInt32>(scissor->getRectangle().width()),
                     .height = static_cast<UInt32>(scissor->getRectangle().height())}};
  ...
  for (UInt32 layer(0); layer < image.layers(); ++layer)
  {
    ...
    for (UInt32 level(1); level < image.levels(); ++level)
    {
      ...
      VkImageBlit blit{.srcSubresource = VkImageSubresourceLayers{.aspectMask = image.aspectMask(),
                                                                  ...
                                                                  .layerCount = 1},
                       .dstSubresource = VkImageSubresourceLayers{.aspectMask = image.aspectMask(),
                                                                  .mipLevel = level,
                                                                  .baseArrayLayer = layer,
                                                                  .layerCount = 1}};
    ...
    VulkanBarrier subBarrier(PipelineStage::Transfer, PipelineStage::Transfer);
    subBarrier.transition(image, image.levels() - 1, 1, layer, 1, 0, ResourceAccess::TransferWrite,
                          ResourceAccess::TransferRead, ImageLayout::CopySource);
...
void VulkanCommandBuffer::transfer(const IVulkanBuffer & source, const IVulkanBuffer & target,
                                   ...
                                   UInt32 elements) const
{
  if (source.elements() < sourceElement + elements) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "sourceElement",
      "The source buffer has only {0} elements, but a transfer for {1} elements starting from element {2} has been requested.",
      source.elements(), elements, sourceElement);
  ...
  VkBufferCopy copyInfo{.srcOffset = sourceElement * source.alignedElementSize(),
...
void VulkanCommandBuffer::transfer(const void * const data, size_t size,
                                   const IVulkanBuffer & target, UInt32 targetElement,
                                   UInt32 elements) const
{
  auto stagingBuffer = asShared(
    std::move(m_impl->m_queue.device().factory().createBuffer(target.type(), ResourceHeap::Staging,
...
void VulkanCommandBuffer::transfer(Span<const void * const> data, size_t elementSize,
                                   const IVulkanBuffer & target, UInt32 firstElement) const
{
                                   ...
                                   UInt32 elements) const
{
  ...
  if (target.elements() < firstSubresource + elements) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "targetElement",
      "The target image has only {0} sub-resources, but a transfer for {1} elements starting from element {2} has been requested.",
      target.elements(), elements, firstSubresource);

  Array<VkBufferImageCopy> copyInfos(elements);
  std::ranges::generate(copyInfos,
                        [&, this, i = firstSubresource]() mutable
                        {
                          UInt32 subresource = i++, layer = 0, level = 0, plane = 0;
                          target.resolveSubresource(subresource, plane, layer, level);

                          return VkBufferImageCopy{.bufferOffset =
                                                     source.alignedElementSize() * sourceElement,
                                                   .bufferRowLength = 0,
                                                   .bufferImageHeight = 0,
                                                   .imageSubresource =
                                                   ...
                                                   .imageOffset = {0, 0, 0},
                                                   .imageExtent =
                                                     {static_cast<UInt32>(target.extent().width()),
                                                      static_cast<UInt32>(target.extent().height()),
                                                      static_cast<UInt32>(
                                                        target.extent().depth())}};
                        });
...
void VulkanCommandBuffer::transfer(const void * const data, size_t size,
                                   const IVulkanImage & target, UInt32 subresource) const
{
  auto stagingBuffer = asShared(
    std::move(m_impl->m_queue.device().factory().createBuffer(BufferType::Other,
...
void VulkanCommandBuffer::transfer(Span<const void * const> data, size_t elementSize,
                                   const IVulkanImage & target, UInt32 firstSubresource,
                                   UInt32 subresources) const
{
  auto elements = static_cast<UInt32>(data.size());
  auto stagingBuffer = asShared(std::move(
    m_impl->m_queue.device().factory().createBuffer(BufferType::Other, ResourceHeap::Staging,
                                   ...
                                   UInt32 subresources) const
{
  if (source.elements() < sourceSubresource + subresources) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "sourceElement",
      "The source image has only {0} sub-resources, but a transfer for {1} sub-resources starting from sub-resource {2} has been requested.",
      source.elements(), subresources, sourceSubresource);

  if (target.elements() < targetSubresource + subresources) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "targetElement",
      "The target image has only {0} sub-resources, but a transfer for {1} sub-resources starting from sub-resources {2} has been requested.",
      target.elements(), subresources, targetSubresource);
  ...
  std::ranges::
    generate(copyInfos,
             [&, this, i = 0]() mutable
             {
               UInt32 sourceRsc = sourceSubresource + i, sourceLayer = 0, sourceLevel = 0,
                      sourcePlane = 0;
               UInt32 targetRsc = targetSubresource + i, targetLayer = 0, targetLevel = 0,
                      targetPlane = 0;
               ...
               return VkImageCopy{.srcSubresource =
                                    VkImageSubresourceLayers{.aspectMask =
                                                               source.aspectMask(sourcePlane),
                                  ...
                                  .dstSubresource =
                                    VkImageSubresourceLayers{.aspectMask =
                                                               target.aspectMask(targetPlane),
                                                             .mipLevel = targetLevel,
                                                             .baseArrayLayer = targetLayer,
                                                             .layerCount = 1},
                                  .dstOffset = {0, 0, 0},
                                  .extent = {static_cast<UInt32>(source.extent().width()),
                                             static_cast<UInt32>(source.extent().height()),
  ...
  std::ranges::generate(copyInfos,
                        [&, this, i = targetElement]() mutable
                        {
                          ...
                          return VkBufferImageCopy{.bufferOffset =
                                                   ...
                                                   .imageOffset = {0, 0, 0},
                                                   .imageExtent =
                                                     {static_cast<UInt32>(source.extent().width()),
                                                      static_cast<UInt32>(source.extent().height()),
                                                      static_cast<UInt32>(
                                                        source.extent().depth())}};
                        });
...
void VulkanCommandBuffer::transfer(SharedPtr<const IVulkanBuffer> source,
...
void VulkanCommandBuffer::transfer(SharedPtr<const IVulkanImage> source,
...
void VulkanCommandBuffer::bind(const VulkanDescriptorSet & descriptorSet) const
{
  ...
  if (m_impl->m_lastPipeline) [[likely]]
    ...
    throw RuntimeException(
      "No pipeline has been used on the command buffer before attempting to bind the descriptor set.");
}
...
void VulkanCommandBuffer::bind(const VulkanDescriptorSet & descriptorSet,
                               const VulkanPipelineState & pipeline) const noexcept
{
  auto set = &descriptorSet;
  pipeline.bind(*this, {std::addressof(set), 1});
}

void VulkanCommandBuffer::bind(Span<const VulkanDescriptorSet *> descriptorSets,
                               const VulkanPipelineState & pipeline) const noexcept
{
...
void VulkanCommandBuffer::dispatchMeshIndirect(const IVulkanBuffer & batchBuffer,
...
void VulkanCommandBuffer::drawIndirect(const IVulkanBuffer & batchBuffer,
                                       const IVulkanBuffer & countBuffer, UInt64 offset,
                                       UInt64 countOffset, UInt32 maxBatches) const noexcept
{
...
void VulkanCommandBuffer::drawIndexedIndirect(const IVulkanBuffer & batchBuffer,
                                              const IVulkanBuffer & countBuffer, UInt64 offset,
                                              UInt64 countOffset, UInt32 maxBatches) const noexcept
{
...
void VulkanCommandBuffer::pushConstants(const VulkanPushConstantsLayout & layout,
...
void VulkanCommandBuffer::execute(
  Enumerable<SharedPtr<const VulkanCommandBuffer>> commandBuffers) const
{
  auto secondaryHandles =
    commandBuffers |
...
void VulkanCommandBuffer::releaseSharedState() const { m_impl->m_sharedResources.clear(); }

void VulkanCommandBuffer::buildAccelerationStructure(
  VulkanBottomLevelAccelerationStructure & blas, const SharedPtr<const IVulkanBuffer> scratchBuffer,
  const IVulkanBuffer & buffer, UInt64 offset) const
{
  m_impl->buildAccelerationStructure(blas, scratchBuffer, buffer, offset, false);
}

void VulkanCommandBuffer::buildAccelerationStructure(
  VulkanTopLevelAccelerationStructure & tlas, const SharedPtr<const IVulkanBuffer> scratchBuffer,
  const IVulkanBuffer & buffer, UInt64 offset) const
{
  m_impl->buildAccelerationStructure(tlas, scratchBuffer, buffer, offset, false);
}

void VulkanCommandBuffer::updateAccelerationStructure(
  VulkanBottomLevelAccelerationStructure & blas, const SharedPtr<const IVulkanBuffer> scratchBuffer,
  const IVulkanBuffer & buffer, UInt64 offset) const
{
...
void VulkanCommandBuffer::copyAccelerationStructure(
  const VulkanBottomLevelAccelerationStructure & from,
  const VulkanBottomLevelAccelerationStructure & to, bool compress) const noexcept
{
...
void VulkanCommandBuffer::copyAccelerationStructure(
  const VulkanTopLevelAccelerationStructure & from, const VulkanTopLevelAccelerationStructure & to,
...
void VulkanCommandBuffer::traceRays(UInt32 width, UInt32 height, UInt32 depth,
                                    ...
                                    const IVulkanBuffer * callableShaderBindingTable) const noexcept
{
  VkStridedDeviceAddressRegionKHR raygen = {.deviceAddress =
                                              rayGenerationShaderBindingTable.virtualAddress() +
                                              offsets.RayGenerationGroupOffset,
  ...
  {
    callable.deviceAddress =
      callableShaderBindingTable->virtualAddress() + offsets.CallableGroupOffset;
