...
class VulkanRayTracingPipeline::VulkanRayTracingPipelineImpl
  : public Implement<VulkanRayTracingPipeline>
{
public:
...
public:
  VulkanRayTracingPipelineImpl(VulkanRayTracingPipeline * parent, const VulkanDevice & device,
  ...
  {
    m_program =
      std::dynamic_pointer_cast<const VulkanShaderProgram>(m_shaderRecordCollection.program());
  }
...
public:
  VkPipeline initialize()
  {
    ...
    LITEFX_TRACE(VULKAN_LOG, "Using shader program {0} with {1} modules...",
                 reinterpret_cast<const void *>(m_program.get()), modules.size());

    Array<VkPipelineShaderStageCreateInfo> shaderStages =
      modules |
      std::views::transform([](const VulkanShaderModule * shaderModule)
                            { return shaderModule->shaderStageDefinition(); }) |
    ...
    auto moduleIds =
      modules | std::views::enumerate |
      std::views::transform(
        [](const auto & tuple)
        {
          return std::make_tuple(static_cast<const IShaderModule *>(std::get<1>(tuple)),
                                 std::get<0>(tuple));
        }) |
        ...
        [&moduleIds](auto & record)
        {
          ...
          switch (record->type())
          {
            ...
            case ShaderRecordType::Callable:
              ...
              group.generalShader =
                moduleIds.at(std::get<const IShaderModule *>(record->shaderGroup()));
            ...
            case ShaderRecordType::Intersection:
              ...
              group.intersectionShader =
                moduleIds.at(std::get<const IShaderModule *>(record->shaderGroup()));
              group.anyHitShader = group.closestHitShader = group.generalShader =
                VK_SHADER_UNUSED_KHR;
              break;
            case ShaderRecordType::HitGroup:
            {
              const auto & hitGroup =
                std::get<IShaderRecord::MeshGeometryHitGroup>(record->shaderGroup());
              group.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR;
              group.closestHitShader = hitGroup.ClosestHitShader != nullptr
                                       ? moduleIds.at(hitGroup.ClosestHitShader)
                                       : VK_SHADER_UNUSED_KHR;
              group.anyHitShader = hitGroup.AnyHitShader != nullptr
                                   ? moduleIds.at(hitGroup.AnyHitShader)
                                   : VK_SHADER_UNUSED_KHR;
            ...
            default:
              throw InvalidArgumentException(
                "shaderRecords",
                "At least one record in the shader record collection is not a valid ray-tracing shader.");
          }
    ...
    VkPipeline pipeline;
    raiseIfFailed(::vkCreateRayTracingPipelines(m_device.handle(), VK_NULL_HANDLE, VK_NULL_HANDLE,
                                                1, &pipelineInfo, nullptr, &pipeline),
                  "Unable to create render pipeline.");

#ifndef NDEBUG
    m_device.setDebugName(*reinterpret_cast<const UInt64 *>(&pipeline),
                          VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT, m_parent->name());
#endif

    return pipeline;
  }

  UniquePtr<IVulkanBuffer> allocateShaderBindingTable(ShaderBindingTableOffsets & offsets,
                                                      ShaderBindingGroup groups)
  {
    ...
    VkPhysicalDeviceProperties2 deviceProperties{.sType =
    ...
    auto recordSize =
      Math::align<UInt64>(rayTracingProperties.shaderGroupHandleSize + localDataSize,
    ...
    auto shaderRecordIds = m_shaderRecordCollection.shaderRecords() | std::views::enumerate |
                           std::views::transform(
                             [](const auto & tuple) {
                               return std::make_tuple(std::get<1>(tuple).get(), std::get<0>(tuple));
                             }) |
                           std::ranges::to<std::map>();
                       ...
                       ShaderBindingGroup::Callable, ShaderBindingGroup::HitGroup})
    {
      ...
      if (LITEFX_FLAG_IS_SET(groups, group))
      {
        ...
        for (auto & currentRecord : filteredRecords)
        {
          ...
          raiseIfFailed(::vkGetRayTracingShaderGroupHandles(m_device.handle(), m_parent->handle(),
                                                            id, 1,
                                                            rayTracingProperties
                                                              .shaderGroupHandleSize,
                                                            recordData.data()),
                        "Unable to query shader record handle.");

          // Write the payload and map everything into the buffer.
          std::memcpy(recordData.data() + rayTracingProperties.shaderGroupHandleSize,
...
VulkanRayTracingPipeline::VulkanRayTracingPipeline(const VulkanDevice & device,
  ...
  : m_impl(makePimpl<VulkanRayTracingPipelineImpl>(this, device, std::move(shaderRecords)))
  , VulkanPipelineState(VK_NULL_HANDLE)
{
}
...
UniquePtr<IVulkanBuffer> VulkanRayTracingPipeline::allocateShaderBindingTable(
...
void VulkanRayTracingPipeline::bind(const VulkanCommandBuffer & commandBuffer,
                                    Span<const VulkanDescriptorSet *> descriptorSets) const noexcept
{
  // Filter out uninitialized sets.
  auto sets = descriptorSets | std::views::filter([](auto set) { return set != nullptr; }) |
              std::ranges::to<Array<const VulkanDescriptorSet *>>();

  if (sets.empty()) [[unlikely]]
    return; // Nothing to do on empty sets.
  else if (sets.size() == 1)
    ::vkCmdBindDescriptorSets(commandBuffer.handle(), VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
                              std::as_const(*m_impl->m_layout).handle(),
                              sets.front()->layout().space(), 1, &sets.front()->handle(), 0,
                              nullptr);
  else
  {
    ...
    std::ranges::sort(sets, [](auto lhs, auto rhs)
                      { return lhs->layout().space() > rhs->layout().space(); });
      ...
      std::ranges::for_each(sets,
                            [&](auto set)
                            {
                              ::vkCmdBindDescriptorSets(commandBuffer.handle(),
                                                        ...
                                                        std::as_const(*m_impl->m_layout).handle(),
                                                        set->layout().space(), 1, &set->handle(), 0,
                                                        nullptr);
                            });
    else
    {
      // Obtain the handles and bind the sets.
      auto handles = sets | std::views::transform([](auto set) { return set->handle(); }) |
                     std::ranges::to<Array<VkDescriptorSet>>();
      ::vkCmdBindDescriptorSets(commandBuffer.handle(), VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
                                std::as_const(*m_impl->m_layout).handle(), startSpace,
                                static_cast<UInt32>(handles.size()), handles.data(), 0, nullptr);
    }
  }
}
...
VulkanRayTracingPipelineBuilder::VulkanRayTracingPipelineBuilder(
  const VulkanDevice & device, ShaderRecordCollection && shaderRecords, const String & name)
