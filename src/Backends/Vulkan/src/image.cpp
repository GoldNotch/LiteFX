...
public:
  VulkanImageImpl(VulkanImage * parent, const VulkanDevice & device, const Size3d & extent,
...
UInt32 VulkanImage::elements() const noexcept { return m_impl->m_elements; }
...
size_t VulkanImage::size() const noexcept
{
  ...
  {
    auto elementSize = ::getSize(m_impl->m_format) * m_impl->m_extent.width() *
                       m_impl->m_extent.height() * m_impl->m_extent.depth() * m_impl->m_layers;
...
Format VulkanImage::format() const noexcept { return m_impl->m_format; }

ImageDimensions VulkanImage::dimensions() const noexcept { return m_impl->m_dimensions; }

UInt32 VulkanImage::levels() const noexcept { return m_impl->m_levels; }

UInt32 VulkanImage::layers() const noexcept { return m_impl->m_layers; }

UInt32 VulkanImage::planes() const noexcept { return m_impl->m_planes; }

MultiSamplingLevel VulkanImage::samples() const noexcept { return m_impl->m_samples; }
...
VkImageAspectFlags VulkanImage::aspectMask() const noexcept
{
  ...
  else if (m_impl->m_planes > 1)
  {
    ...
    if (m_impl->m_planes > 3) [[unlikely]]
      LITEFX_ERROR(
        VULKAN_LOG,
        "An image resource with a multi-planar format has {0} planes, which is not supported (maximum is {1}).",
        m_impl->m_planes, 3);
...
VkImageAspectFlags VulkanImage::aspectMask(UInt32 plane) const
{
  if (::hasDepth(m_impl->m_format) && ::hasStencil(m_impl->m_format))
  {
    if (plane > 2) [[unlikely]] // Should actually never happen.
      throw RuntimeException(
        "An image resource with a depth/stencil format has more than two planes, which is not supported.");
  ...
  else if (::hasDepth(m_impl->m_format))
  {
    if (plane > 1) [[unlikely]] // Should actually never happen.
      throw RuntimeException(
        "An image resource with a depth-only format has more than one planes, which is not supported.");
  ...
  else if (::hasStencil(m_impl->m_format))
  {
    if (plane > 1) [[unlikely]] // Should actually never happen.
      throw RuntimeException(
        "An image resource with a stencil-only format has more than one planes, which is not supported.");
  ...
  else if (m_impl->m_planes > 1)
  {
    if (plane == 0)
      ...
      throw RuntimeException(
        "An image resource with a multi-planar format has more than three planes, which is not supported.");
  }
  else [[likely]]
  {
    return VK_IMAGE_ASPECT_COLOR_BIT;
...
VmaAllocator & VulkanImage::allocator() const noexcept { return m_impl->m_allocator; }

VmaAllocation & VulkanImage::allocationInfo() const noexcept { return m_impl->m_allocationInfo; }

UniquePtr<VulkanImage> VulkanImage::allocate(
  const VulkanDevice & device, const Size3d & extent, Format format, ImageDimensions dimensions,
  UInt32 levels, UInt32 layers, MultiSamplingLevel samples, ResourceUsage usage,
  VmaAllocator & allocator, const VkImageCreateInfo & createInfo,
  const VmaAllocationCreateInfo & allocationInfo, VmaAllocationInfo * allocationResult)
{
  return VulkanImage::allocate("", device, extent, format, dimensions, levels, layers, samples,
                               usage, allocator, createInfo, allocationInfo, allocationResult);
}

UniquePtr<VulkanImage> VulkanImage::allocate(
  const String & name, const VulkanDevice & device, const Size3d & extent, Format format,
  ImageDimensions dimensions, UInt32 levels, UInt32 layers, MultiSamplingLevel samples,
  ResourceUsage usage, VmaAllocator & allocator, const VkImageCreateInfo & createInfo,
  const VmaAllocationCreateInfo & allocationInfo, VmaAllocationInfo * allocationResult)
{
  VkImage image;
  VmaAllocation allocation;

  raiseIfFailed(::vmaCreateImage(allocator, &createInfo, &allocationInfo, &image, &allocation,
                                 allocationResult),
                "Unable to allocate texture.");
  LITEFX_DEBUG(
    VULKAN_LOG,
    "Allocated image {0} with {1} bytes {{ Extent: {2}x{3} Px, Format: {4}, Levels: {5}, Layers: {6}, Samples: {8}, Usage: {7} }}",
    name.empty() ? std::format("{0}", reinterpret_cast<void *>(image)) : name,
    ::getSize(format) * extent.width() * extent.height(), extent.width(), extent.height(), format,
    levels, layers, usage, samples);

  return makeUnique<VulkanImage>(device, image, extent, format, dimensions, levels, layers, samples,
                                 usage, allocator, allocation, name);
}
...
public:
  VulkanSamplerImpl(VulkanSampler * parent, const VulkanDevice & device, FilterMode magFilter,
                    FilterMode minFilter, BorderMode borderU, BorderMode borderV,
                    BorderMode borderW, MipMapMode mipMapMode, Float mipMapBias, Float minLod,
...
public:
  VkSampler initialize()
  {
    ...
    VkSampler sampler;
    raiseIfFailed(::vkCreateSampler(m_device.handle(), &samplerInfo, nullptr, &sampler),
...
FilterMode VulkanSampler::getMagnifyingFilter() const noexcept { return m_impl->m_magFilter; }

BorderMode VulkanSampler::getBorderModeU() const noexcept { return m_impl->m_borderU; }

BorderMode VulkanSampler::getBorderModeV() const noexcept { return m_impl->m_borderV; }

BorderMode VulkanSampler::getBorderModeW() const noexcept { return m_impl->m_borderW; }

Float VulkanSampler::getAnisotropy() const noexcept { return m_impl->m_anisotropy; }

MipMapMode VulkanSampler::getMipMapMode() const noexcept { return m_impl->m_mipMapMode; }

Float VulkanSampler::getMipMapBias() const noexcept { return m_impl->m_mipMapBias; }

Float VulkanSampler::getMaxLOD() const noexcept { return m_impl->m_maxLod; }

Float VulkanSampler::getMinLOD() const noexcept { return m_impl->m_minLod; }
