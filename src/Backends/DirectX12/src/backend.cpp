...
public:
  [[nodiscard]] ComPtr<IDXGIFactory7> initialize()
  {
    ComPtr<IDXGIFactory7> factory;

#ifndef NDEBUG
    raiseIfFailed(::CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&factory)),
                  "Unable to create DirectX 12 factory instance.");

    if (SUCCEEDED(::D3D12GetDebugInterface(IID_PPV_ARGS(&m_debugInterface))))
      m_debugInterface->EnableDebugLayer();
#else
    raiseIfFailed(::CreateDXGIFactory2(0, IID_PPV_ARGS(&factory)),
                  "Unable to create DirectX 12 factory instance.");
#endif
  ...
  void loadAdapters(bool enableWarp = false)
  {
    ...
    if (enableWarp)
    {
      raiseIfFailed(m_parent->handle()->EnumWarpAdapter(IID_PPV_ARGS(&adapterInterface)),
                    "Unable to iterate advanced software rasterizer adapters.");
      raiseIfFailed(
        adapterInterface.As(&adapterInstance),
        "The advanced software rasterizer adapter is not a valid IDXGIAdapter4 instance.");
      m_adapters.push_back(makeUnique<DirectX12GraphicsAdapter>(adapterInstance));
    }
    else
    {
      for (UInt32 i(0);
           m_parent->handle()->EnumAdapters1(i, &adapterInterface) != DXGI_ERROR_NOT_FOUND; ++i)
      {
        ...
        raiseIfFailed(adapterInterface.As(&adapterInstance),
                      "The hardware adapter is not a valid IDXGIAdapter4 instance.");
        m_adapters.push_back(makeUnique<DirectX12GraphicsAdapter>(adapterInstance));
      }
    }
  }
...
BackendType DirectX12Backend::type() const noexcept { return BackendType::Rendering; }
...
Enumerable<const DirectX12GraphicsAdapter *> DirectX12Backend::listAdapters() const
{
  return m_impl->m_adapters |
         std::views::transform([](const UniquePtr<DirectX12GraphicsAdapter> & adapter)
                               { return adapter.get(); });
}

const DirectX12GraphicsAdapter * DirectX12Backend::findAdapter(
  const Optional<UInt64> & adapterId) const
{
