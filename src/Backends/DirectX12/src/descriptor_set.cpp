...
public:
  DirectX12DescriptorSetImpl(DirectX12DescriptorSet * parent,
                             const DirectX12DescriptorSetLayout & layout,
                             ComPtr<ID3D12DescriptorHeap> && bufferHeap,
                             ComPtr<ID3D12DescriptorHeap> && samplerHeap)
    : base(parent)
    , m_layout(layout)
    , m_bufferHeap(std::move(bufferHeap))
    , m_samplerHeap(std::move(samplerHeap))
  {
    auto buffers =
      m_layout.uniforms() + m_layout.images() + m_layout.storages() + m_layout.buffers();

    if (buffers > 0 && m_bufferHeap == nullptr)
      throw ArgumentNotInitializedException(
        "bufferHeap",
        "The buffer descriptor heap handle must be initialized, if the descriptor set layout contains uniform buffers, storage buffers or images.");

    if (m_layout.samplers() > 0 && m_samplerHeap == nullptr)
      throw ArgumentNotInitializedException(
        "samplerHeap",
        "The sampler descriptor heap handle must be initialized, if the descriptor set layout contains samplers.");
  }

public:
  D3D12_FILTER getFilterMode(FilterMode minFilter, FilterMode magFilter, MipMapMode mipFilter,
                             Float anisotropy = 0.f)
  {
    ...
    {
      D3D12_FILTER_TYPE minType = minFilter == FilterMode::Nearest ? D3D12_FILTER_TYPE_POINT
                                                                   : D3D12_FILTER_TYPE_LINEAR;
      D3D12_FILTER_TYPE magType = magFilter == FilterMode::Nearest ? D3D12_FILTER_TYPE_POINT
                                                                   : D3D12_FILTER_TYPE_LINEAR;
      D3D12_FILTER_TYPE mipType = mipFilter == MipMapMode::Nearest ? D3D12_FILTER_TYPE_POINT
                                                                   : D3D12_FILTER_TYPE_LINEAR;

      return D3D12_ENCODE_BASIC_FILTER(minType, magType, mipType,
...
DirectX12DescriptorSet::DirectX12DescriptorSet(const DirectX12DescriptorSetLayout & layout,
                                               ComPtr<ID3D12DescriptorHeap> && bufferHeap,
                                               ComPtr<ID3D12DescriptorHeap> && samplerHeap)
  : m_impl(makePimpl<DirectX12DescriptorSetImpl>(this, layout, std::move(bufferHeap),
                                                 std::move(samplerHeap)))
{
...
void DirectX12DescriptorSet::update(UInt32 binding, const IDirectX12Buffer & buffer,
                                    UInt32 bufferElement, UInt32 elements,
                                    UInt32 firstDescriptor) const
{
  ...
  if (bufferElement + elementCount > buffer.elements()) [[unlikely]]
    LITEFX_WARNING(
      DIRECTX12_LOG,
      "The buffer only has {0} elements, however there are {1} elements starting at element {2} specified.",
      buffer.elements(), elementCount, bufferElement);

  // Find the descriptor.
  auto descriptors = m_impl->m_layout.descriptors();
  auto match = std::ranges::find_if(descriptors,
                                    [&binding](const DirectX12DescriptorLayout * layout)
                                    { return layout->binding() == binding; });

  if (match == descriptors.end()) [[unlikely]]
  {
    LITEFX_WARNING(DIRECTX12_LOG,
                   "The descriptor set {0} does not contain a descriptor at binding {1}.",
                   m_impl->m_layout.space(), binding);
    return;
  }

  const auto & descriptorLayout = *(*match);
  auto offset = m_impl->m_layout.descriptorOffsetForBinding(binding) + firstDescriptor;
  auto descriptorSize = m_impl->m_layout.device().handle()->GetDescriptorHandleIncrementSize(
    D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
  CD3DX12_CPU_DESCRIPTOR_HANDLE descriptorHandle(m_impl->m_bufferHeap
  ...
  switch (descriptorLayout.descriptorType())
  {
      ...
      for (UInt32 i(0); i < elementCount; ++i)
      {
        D3D12_CONSTANT_BUFFER_VIEW_DESC constantBufferView =
          {.BufferLocation = buffer.handle()->GetGPUVirtualAddress() +
                             static_cast<size_t>(bufferElement + i) * buffer.alignedElementSize(),
           .SizeInBytes = static_cast<UInt32>(buffer.alignedElementSize())};

        m_impl->m_layout.device().handle()->CreateConstantBufferView(&constantBufferView,
                                                                     descriptorHandle);
      ...
      for (UInt32 i(0); i < elementCount; ++i)
      {
        D3D12_SHADER_RESOURCE_VIEW_DESC bufferView =
          {.Format = DXGI_FORMAT_UNKNOWN,
           ...
           .Buffer = {.FirstElement = bufferElement + i,
                      .NumElements = 1,
                      .StructureByteStride = static_cast<UInt32>(buffer.alignedElementSize()),
                      .Flags = D3D12_BUFFER_SRV_FLAG_NONE}};

        m_impl->m_layout.device().handle()->CreateShaderResourceView(buffer.handle().Get(),
                                                                     &bufferView, descriptorHandle);
        descriptorHandle = descriptorHandle.Offset(descriptorSize);
      }
    ...
    case DescriptorType::RWStructuredBuffer:
    {
      for (UInt32 i(0); i < elementCount; ++i)
      {
        ...
        m_impl->m_layout.device().handle()->CreateUnorderedAccessView(buffer.handle().Get(),
                                                                      nullptr, &bufferView,
                                                                      descriptorHandle);
        descriptorHandle = descriptorHandle.Offset(descriptorSize);
      }
      ...
      for (UInt32 i(0); i < elementCount; ++i)
      {
        ...
        D3D12_SHADER_RESOURCE_VIEW_DESC bufferView =
          {.Format = DXGI_FORMAT_R32_TYPELESS,
           .ViewDimension = D3D12_SRV_DIMENSION_BUFFER,
           .Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING,
           .Buffer = {.FirstElement = (bufferElement + i) * sizeof(DWORD),
                      .NumElements =
                        static_cast<UInt32>(buffer.alignedElementSize() / sizeof(DWORD)),
    ...
    default:
      [[unlikely]] throw InvalidArgumentException(
        "binding",
        "The descriptor at binding point {0} does not reference a buffer, uniform or storage resource.",
        binding);
  }
...
void DirectX12DescriptorSet::update(UInt32 binding, const IDirectX12Image & texture,
                                    UInt32 descriptor, UInt32 firstLevel, UInt32 levels,
                                    UInt32 firstLayer, UInt32 layers) const
{
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE
    descriptorHandle(m_impl->m_bufferHeap->GetCPUDescriptorHandleForHeapStart(),
                     offset + descriptor,
                     m_impl->m_layout.device().handle()->GetDescriptorHandleIncrementSize(
                       D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
  ...
  if (descriptorLayout.descriptorType() == DescriptorType::Texture ||
      descriptorLayout.descriptorType() == DescriptorType::InputAttachment)
  {
    ...
    if (::hasDepth(texture.format()))
    {
      switch (texture.format())
      {
        ...
        case Format::D24_UNORM_S8_UINT:
          textureView.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
          break;
        case Format::D32_SFLOAT_S8_UINT:
          textureView.Format = DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS;
          break;
    ...
    switch (texture.dimensions())
    {
      case ImageDimensions::DIM_1:
        if (texture.layers() == 1)
        {
          ...
          textureView.Texture1D = {.MostDetailedMip = firstLevel,
                                   .MipLevels = numLevels,
        ...
        else
        {
          ...
          textureView.Texture1DArray = {.MostDetailedMip = firstLevel,
                                        .MipLevels = numLevels,
                                        .FirstArraySlice = firstLayer,
                                        .ArraySize = numLayers,
                                        .ResourceMinLODClamp = 0};
        }
      ...
      case ImageDimensions::DIM_2:
        if (texture.samples() == MultiSamplingLevel::x1)
        {
          if (texture.layers() == 1)
          {
            ...
            textureView.Texture2D = {.MostDetailedMip = firstLevel,
                                     .MipLevels = numLevels,
                                     .PlaneSlice = 0,
                                     .ResourceMinLODClamp = 0};
          }
          else
          {
            ...
            textureView.Texture2DArray = {.MostDetailedMip = firstLevel,
                                          .MipLevels = numLevels,
                                          .FirstArraySlice = firstLayer,
                                          .ArraySize = numLayers,
                                          .PlaneSlice = 0,
      ...
      case ImageDimensions::DIM_3:
        ...
        textureView.Texture3D = {.MostDetailedMip = firstLevel,
                                 .MipLevels = numLevels,
                                 .ResourceMinLODClamp = 0};
        break;
      case ImageDimensions::CUBE:
        textureView.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
        textureView.TextureCube = {.MostDetailedMip = firstLevel,
                                   .MipLevels = numLevels,
                                   .ResourceMinLODClamp = 0};
        break;
    }

    m_impl->m_layout.device().handle()->CreateShaderResourceView(texture.handle().Get(),
                                                                 &textureView, descriptorHandle);
  }
  else if (descriptorLayout.descriptorType() == DescriptorType::RWTexture)
  {
    if (!texture.writable())
      throw InvalidArgumentException(
        "binding",
        "The provided texture is not writable and cannot be bound to a read/write descriptor.");

    D3D12_UNORDERED_ACCESS_VIEW_DESC textureView = {.Format = DX12::getFormat(texture.format())};
    ...
    switch (texture.dimensions())
    {
      case ImageDimensions::DIM_1:
        ...
        else
        {
          ...
          textureView.Texture1DArray = {.MipSlice = firstLevel,
                                        .FirstArraySlice = firstLayer,
                                        .ArraySize = numLayers};
        }
      ...
      case ImageDimensions::DIM_2:
        if (texture.samples() == MultiSamplingLevel::x1)
        {
          ...
          else
          {
            ...
            textureView.Texture2DArray = {.MipSlice = firstLevel,
                                          .FirstArraySlice = firstLayer,
                                          .ArraySize = numLayers,
                                          .PlaneSlice = 0};
          }
      ...
      case ImageDimensions::DIM_3:
        ...
        textureView.Texture3D = {.MipSlice = firstLevel,
                                 .FirstWSlice = firstLayer,
                                 .WSize = layers == 0
                                          ? static_cast<UInt32>(texture.extent().depth())
                                          : layers};
        break;
      case ImageDimensions::CUBE:
        textureView.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2DARRAY;
    ...
    m_impl->m_layout.device().handle()->CreateUnorderedAccessView(texture.handle().Get(), nullptr,
                                                                  &textureView, descriptorHandle);
  }
  else [[unlikely]]
  {
    throw InvalidArgumentException(
      "binding",
      "The provided texture is bound to a descriptor that is does neither describe a `Texture`, nor a `WritableTexture`.");
  }

  m_impl->updateGlobalBuffers(offset, 1);
}

void DirectX12DescriptorSet::update(UInt32 binding, const IDirectX12Sampler & sampler,
                                    UInt32 descriptor) const
{
  ...
  D3D12_SAMPLER_DESC samplerInfo = {.Filter = m_impl->getFilterMode(sampler.getMinifyingFilter(),
                                                                    sampler.getMagnifyingFilter(),
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE
    descriptorHandle(m_impl->m_samplerHeap->GetCPUDescriptorHandleForHeapStart(),
                     offset + descriptor,
                     m_impl->m_layout.device().handle()->GetDescriptorHandleIncrementSize(
...
void DirectX12DescriptorSet::update(UInt32 binding,
                                    const IDirectX12AccelerationStructure & accelerationStructure,
                                    UInt32 descriptor) const
{
  // Find the descriptor.
  auto descriptors = m_impl->m_layout.descriptors();
  ...
  if (layout.descriptorType() != DescriptorType::AccelerationStructure) [[unlikely]]
    throw InvalidArgumentException(
      "binding",
      "Invalid descriptor type. The binding {0} does not point to an acceleration structure descriptor.",
      binding);
  ...
  if (buffer == nullptr) [[unlikely]]
    throw InvalidArgumentException("accelerationStructure",
                                   "The acceleration structure buffer has not yet been allocated.");
  ...
  m_impl->m_layout.device().handle()->CreateShaderResourceView(nullptr, &bufferView,
                                                               descriptorHandle);
  descriptorHandle = descriptorHandle.Offset(descriptorSize);
...
UInt32 DirectX12DescriptorSet::bufferOffset() const noexcept { return m_impl->m_bufferOffset; }
...
UInt32 DirectX12DescriptorSet::samplerOffset() const noexcept { return m_impl->m_samplerOffset; }
