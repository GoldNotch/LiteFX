...
private:
  ...
  UInt32 m_globalBufferHeapSize, m_globalSamplerHeapSize, m_bufferDescriptorIncrement,
    m_samplerDescriptorIncrement, m_bufferOffset{0}, m_samplerOffset{0};
  ...
  ComPtr<ID3D12CommandSignature> m_dispatchSignature, m_drawSignature, m_drawIndexedSignature,
    m_dispatchMeshSignature;

public:
  DirectX12DeviceImpl(DirectX12Device * parent, const DirectX12GraphicsAdapter & adapter,
                      UniquePtr<DirectX12Surface> && surface, const DirectX12Backend & backend,
                      UInt32 globalBufferHeapSize, UInt32 globalSamplerHeapSize)
    : base(parent)
    , m_adapter(adapter)
    , m_surface(std::move(surface))
    , m_backend(backend)
    , m_globalBufferHeapSize(globalBufferHeapSize)
    , m_globalSamplerHeapSize(globalSamplerHeapSize)
  {
    ...
    if (globalSamplerHeapSize > 2048) [[unlikely]]
      throw ArgumentOutOfRangeException(
        "globalSamplerHeapSize", 0u, 2048u, globalSamplerHeapSize,
        "Only 2048 samplers are allowed in the global sampler heap, but {0} have been specified.",
        globalSamplerHeapSize);
  }
...
#ifndef NDEBUG
private:
  static void __stdcall onDebugMessage(D3D12_MESSAGE_CATEGORY category,
                                       ...
                                       LPCSTR description, void * /*context*/)
  {
    ...
    switch (severity)
    {
      case D3D12_MESSAGE_SEVERITY_CORRUPTION:
        LITEFX_FATAL_ERROR(DIRECTX12_LOG, "{1} ({2}): {0}", description, t, id);
        break;
      case D3D12_MESSAGE_SEVERITY_ERROR:
        LITEFX_ERROR(DIRECTX12_LOG, "{1} ({2}): {0}", description, t, id);
        break;
      case D3D12_MESSAGE_SEVERITY_WARNING:
        LITEFX_WARNING(DIRECTX12_LOG, "{1} ({2}): {0}", description, t, id);
        break;
      case D3D12_MESSAGE_SEVERITY_INFO:
        LITEFX_INFO(DIRECTX12_LOG, "{1} ({2}): {0}", description, t, id);
        break;
      default:
      case D3D12_MESSAGE_SEVERITY_MESSAGE:
        LITEFX_TRACE(DIRECTX12_LOG, "{1} ({2}): {0}", description, t, id);
        break;
    }
  }
#endif
...
private:
  void checkRequiredExtensions(ID3D12Device10 * device, const GraphicsDeviceFeatures & features)
  {
    ...
    raiseIfFailed(device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS5, &options5,
                                              sizeof(options5)),
                  "Unable to query device extensions.");
    raiseIfFailed(device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS7, &options7,
                                              sizeof(options7)),
                  "Unable to query device extensions.");
    raiseIfFailed(device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS12, &options12,
                                              sizeof(options12)),
                  "Unable to query device extensions.");
...
public:
  [[nodiscard]] ComPtr<ID3D12Device10> initialize(const GraphicsDeviceFeatures & features)
  {
    ...
    if (FAILED(device.As(&infoQueue)))
      LITEFX_WARNING(DIRECTX12_LOG,
                     "Unable to query info queue. Debugger support will be disabled.");
    else
    {
      ...
      raiseIfFailed(infoQueue->PushStorageFilter(&infoQueueFilter),
                    "Unable to push message filter to info queue.");

      // Try to register event callback.
      if (FAILED(infoQueue.As(&m_eventQueue)))
        LITEFX_WARNING(
          DIRECTX12_LOG,
          "Unable to query debug message callback queue. Native event logging will be disabled. Note that it requires at least Windows 11.");
      else if (FAILED(
                 m_eventQueue
                   ->RegisterMessageCallback(&DirectX12Device::DirectX12DeviceImpl::onDebugMessage,
                                             D3D12_MESSAGE_CALLBACK_FLAGS::
                                               D3D12_MESSAGE_CALLBACK_FLAG_NONE,
                                             nullptr, &m_debugCallbackCookie)))
        LITEFX_WARNING(
          DIRECTX12_LOG,
          "Unable to register debug message callback with info queue. Native event logging will be disabled.");
    }
#endif
    ...
    raiseIfFailed(device->CreateDescriptorHeap(&bufferHeapDesc, IID_PPV_ARGS(&m_globalBufferHeap)),
                  "Unable create global GPU descriptor heap for buffers.");
    m_bufferDescriptorIncrement =
    ...
    raiseIfFailed(device->CreateDescriptorHeap(&samplerHeapDesc,
                                               IID_PPV_ARGS(&m_globalSamplerHeap)),
                  "Unable create global GPU descriptor heap for samplers.");
    ...
    D3D12_COMMAND_SIGNATURE_DESC signatureDesc = {.ByteStride = sizeof(IndirectDispatchBatch),
                                                  .NumArgumentDescs = 1,
                                                  .pArgumentDescs = &argumentDesc,
                                                  .NodeMask = 0x00};
    raiseIfFailed(device->CreateCommandSignature(&signatureDesc, nullptr,
                                                 IID_PPV_ARGS(&m_dispatchSignature)),
                  "Unable to create indirect dispatch command signature.");
    argumentDesc = {.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH_MESH};
    signatureDesc = {.ByteStride = sizeof(IndirectIndexedBatch),
                     .NumArgumentDescs = 1,
                     .pArgumentDescs = &argumentDesc,
                     .NodeMask = 0x00};
    raiseIfFailed(device->CreateCommandSignature(&signatureDesc, nullptr,
                                                 IID_PPV_ARGS(&m_dispatchMeshSignature)),
                  "Unable to create indirect mesh shader dispatch command signature.");
    argumentDesc = {.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW};
    signatureDesc = {.ByteStride = sizeof(IndirectBatch),
                     .NumArgumentDescs = 1,
                     .pArgumentDescs = &argumentDesc,
                     .NodeMask = 0x00};
    raiseIfFailed(device->CreateCommandSignature(&signatureDesc, nullptr,
                                                 IID_PPV_ARGS(&m_drawSignature)),
                  "Unable to create indirect draw command signature.");
    ...
    raiseIfFailed(device->CreateCommandSignature(&signatureDesc, nullptr,
                  ...
                  "Unable to create indirect indexed draw command signature.");

    return device;
  }

  void createFactory() { m_factory = makeUnique<DirectX12GraphicsFactory>(*m_parent); }

  void createSwapChain(Format format, const Size2d & renderArea, UInt32 backBuffers,
                       bool enableVsync)
  {
    m_swapChain =
      makeUnique<DirectX12SwapChain>(*m_parent, format, renderArea, backBuffers, enableVsync);
  }
  ...
  void createBlitPipeline()
  {
    try
    {
      ...
      modules.push_back(std::move(
        makeUnique<DirectX12ShaderModule>(*m_parent, ShaderStage::Compute, blitShader,
                                          LiteFX::Backends::DirectX12::Shaders::blit_dxi::name(),
                                          "main")));
      auto shaderProgram =
        DirectX12ShaderProgram::create(*m_parent, std::move(modules | std::views::as_rvalue));
      ...
      auto descriptorSetLayouts = Enumerable<UniquePtr<DirectX12DescriptorSetLayout>>(
        makeUnique<DirectX12DescriptorSetLayout>(*m_parent, std::move(bufferLayouts), 0,
                                                 ShaderStage::Compute),
        makeUnique<DirectX12DescriptorSetLayout>(*m_parent, std::move(samplerLayouts), 1,
                                                 ShaderStage::Compute));

      // Create a pipeline layout.
      auto pipelineLayout = makeShared<DirectX12PipelineLayout>(*m_parent,
                                                                std::move(descriptorSetLayouts),
                                                                std::move(pushConstantsLayout));

      // Create the pipeline.
      m_blitPipeline =
        makeUnique<DirectX12ComputePipeline>(*m_parent, pipelineLayout, shaderProgram, "Blit");
    }
    catch (const Exception & ex)
    {
      LITEFX_WARNING(DIRECTX12_LOG,
                     "Unable to create blit pipeline. Blitting will not be available. Error: {0}",
                     ex.what());
    }
  }
...
public:
  Array<Format> getSurfaceFormats() const
  {
    ...
    Array<Format> surfaceFormats = {DX12::getFormat(DXGI_FORMAT_R16G16B16A16_FLOAT),
...
DirectX12Device::DirectX12Device(const DirectX12Backend & backend,
                                 ...
                                 const Size2d & renderArea, UInt32 backBuffers, bool enableVsync,
                                 GraphicsDeviceFeatures features, UInt32 globalBufferHeapSize,
                                 UInt32 globalSamplerHeapSize)
  : ComResource<ID3D12Device10>(nullptr)
  , m_impl(makePimpl<DirectX12DeviceImpl>(this, adapter, std::move(surface), backend,
                                          globalBufferHeapSize, globalSamplerHeapSize))
{
  LITEFX_DEBUG(DIRECTX12_LOG, "Creating DirectX 12 device {{ Surface: {0}, Adapter: {1} }}...",
               reinterpret_cast<void *>(&surface), adapter.deviceId());
  LITEFX_DEBUG(DIRECTX12_LOG,
               "--------------------------------------------------------------------------");
  LITEFX_DEBUG(DIRECTX12_LOG, "Vendor: {0:#0x} (\"{1}\")", adapter.vendorId(),
               DX12::getVendorName(adapter.vendorId()).c_str());
...
const DirectX12Backend & DirectX12Device::backend() const noexcept { return m_impl->m_backend; }
                                                ...
                                                UInt32 & bufferOffset, UInt32 & samplerOffset) const
{
  ...
  else [[unlikely]]
  {
    ...
    if (auto match = std::ranges::find_if(m_impl->m_bufferDescriptorFragments,
                                          [&buffers](const auto & pair)
                                          { return pair.second == buffers; });
        match != m_impl->m_bufferDescriptorFragments.end())
    {
      bufferOffset = match->first;
      m_impl->m_bufferDescriptorFragments.erase(match);
    }
    else if (auto match = std::ranges::find_if(m_impl->m_bufferDescriptorFragments,
                                               [&buffers](const auto & pair)
                                               { return pair.second > buffers; });
             match != m_impl->m_bufferDescriptorFragments.end())
    {
  ...
  else [[unlikely]]
  {
    ...
    // Find a fitting offset from the fragment heap.
    if (auto match = std::ranges::find_if(m_impl->m_samplerDescriptorFragments,
                                          [&samplers](const auto & pair)
                                          { return pair.second == samplers; });
        match != m_impl->m_samplerDescriptorFragments.end())
    {
      samplerOffset = match->first;
      m_impl->m_samplerDescriptorFragments.erase(match);
    }
    else if (auto match = std::ranges::find_if(m_impl->m_samplerDescriptorFragments,
                                               [&samplers](const auto & pair)
                                               { return pair.second > samplers; });
             match != m_impl->m_samplerDescriptorFragments.end())
    {
...
void DirectX12Device::releaseGlobalDescriptors(
  const DirectX12DescriptorSet & descriptorSet) const noexcept
{
  std::lock_guard<std::mutex> lock(m_impl->m_bufferBindMutex);

  if (descriptorSet.bufferHeap() != nullptr)
    m_impl->m_bufferDescriptorFragments.push_back(
      std::make_pair(descriptorSet.bufferOffset(),
                     descriptorSet.bufferHeap()->GetDesc().NumDescriptors));

  if (descriptorSet.samplerHeap() != nullptr)
    m_impl->m_samplerDescriptorFragments.push_back(
      std::make_pair(descriptorSet.samplerOffset(),
                     descriptorSet.samplerHeap()->GetDesc().NumDescriptors));
}
                                              ...
                                              UInt32 descriptors) const noexcept
{
  ...
  {
    CD3DX12_CPU_DESCRIPTOR_HANDLE targetHandle(m_impl->m_globalBufferHeap
                                                 ->GetCPUDescriptorHandleForHeapStart(),
                                               descriptorSet.bufferOffset() + firstDescriptor,
                                               m_impl->m_bufferDescriptorIncrement);
                                               ...
                                               UInt32 descriptors) const noexcept
{
  if (descriptors > 0) [[likely]]
  {
    CD3DX12_CPU_DESCRIPTOR_HANDLE targetHandle(m_impl->m_globalSamplerHeap
                                                 ->GetCPUDescriptorHandleForHeapStart(),
                                               descriptorSet.samplerOffset() + firstDescriptor,
                                               m_impl->m_samplerDescriptorIncrement);
    CD3DX12_CPU_DESCRIPTOR_HANDLE
                                        ...
                                        const DirectX12PipelineState & pipeline) const noexcept
{
  ...
  if (buffers > 0)
  {
    CD3DX12_GPU_DESCRIPTOR_HANDLE
      targetGpuHandle(m_impl->m_globalBufferHeap->GetGPUDescriptorHandleForHeapStart(),
                      descriptorSet.bufferOffset(), m_impl->m_bufferDescriptorIncrement);

    if (isGraphicsSet)
      commandBuffer.handle()
        ->SetGraphicsRootDescriptorTable(descriptorSet.layout().rootParameterIndex(),
                                         targetGpuHandle);
    else
      commandBuffer.handle()
        ->SetComputeRootDescriptorTable(descriptorSet.layout().rootParameterIndex(),
                                        targetGpuHandle);
  }
  ...
  if (samplers > 0)
  {
    ...
    CD3DX12_GPU_DESCRIPTOR_HANDLE
      targetGpuHandle(m_impl->m_globalSamplerHeap->GetGPUDescriptorHandleForHeapStart(),
                      descriptorSet.samplerOffset(), m_impl->m_samplerDescriptorIncrement);
...
void DirectX12Device::bindGlobalDescriptorHeaps(
  const DirectX12CommandBuffer & commandBuffer) const noexcept
{
  const std::array<ID3D12DescriptorHeap *, 2> globalHeaps{m_impl->m_globalBufferHeap.Get(),
                                                          m_impl->m_globalSamplerHeap.Get()};
...
void DirectX12Device::indirectDrawSignatures(
...
DirectX12RenderPassBuilder DirectX12Device::buildRenderPass(const String & name,
                                                            UInt32 commandBuffers) const
{
  return DirectX12RenderPassBuilder(*this, commandBuffers, name);
}

DirectX12RenderPipelineBuilder DirectX12Device::buildRenderPipeline(
  const DirectX12RenderPass & renderPass, const String & name) const
{
...
DirectX12RayTracingPipelineBuilder DirectX12Device::buildRayTracingPipeline(
  const String & name, ShaderRecordCollection && shaderRecords) const
{
...
DirectX12BarrierBuilder DirectX12Device::buildBarrier() const { return DirectX12BarrierBuilder(); }
#endif // defined(LITEFX_BUILD_DEFINE_BUILDERS)

DirectX12SwapChain & DirectX12Device::swapChain() noexcept { return *m_impl->m_swapChain; }

DeviceState & DirectX12Device::state() const noexcept { return m_impl->m_deviceState; }
...
const DirectX12Surface & DirectX12Device::surface() const noexcept { return *m_impl->m_surface; }
...
const DirectX12Queue & DirectX12Device::defaultQueue(QueueType type) const
{
  ...
  if (LITEFX_FLAG_IS_SET(type, QueueType::Graphics))
    ...
    throw InvalidArgumentException("type",
                                   "No default queue for the provided queue type has was found.");
}
...
UniquePtr<DirectX12Barrier> DirectX12Device::makeBarrier(PipelineStage syncBefore,
                                                         PipelineStage syncAfter) const noexcept
{
  return makeUnique<DirectX12Barrier>(syncBefore, syncAfter);
}

UniquePtr<DirectX12FrameBuffer> DirectX12Device::makeFrameBuffer(
...
{
  constexpr std::array<MultiSamplingLevel, 7> allLevels = {MultiSamplingLevel::x64,
                                                           MultiSamplingLevel::x32,
                                                           MultiSamplingLevel::x16,
                                                           MultiSamplingLevel::x8,
                                                           MultiSamplingLevel::x4,
                                                           MultiSamplingLevel::x2,
  ...
  LITEFX_WARNING(
    DIRECTX12_LOG,
    "No supported multi-sampling levels could be queried. Assuming that multi-sampling is disabled.");
    ...
    generate(events,
             [this, i = 0]() mutable -> event_type
             {
               ...
               if (FAILED(hr))
               {
                 ...
                 raiseIfFailed(hr,
                               "Unable to register queue synchronization fence completion event.");
               }
...
void DirectX12Device::computeAccelerationStructureSizes(
  const DirectX12BottomLevelAccelerationStructure & blas, UInt64 & bufferSize, UInt64 & scratchSize,
  bool forUpdate) const
{
  ...
  if (forUpdate)
    ...
    scratchSize =
      (prebuildInfo.ScratchDataSizeInBytes + D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1) &
      ~(D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1);
}

void DirectX12Device::computeAccelerationStructureSizes(
  const DirectX12TopLevelAccelerationStructure & tlas, UInt64 & bufferSize, UInt64 & scratchSize,
  bool forUpdate) const
{
