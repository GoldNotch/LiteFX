...
public:
  DirectX12BufferImpl(DirectX12Buffer * parent, BufferType type, UInt32 elements,
...
BufferType DirectX12Buffer::type() const noexcept { return m_impl->m_type; }

UInt32 DirectX12Buffer::elements() const noexcept { return m_impl->m_elements; }
...
size_t DirectX12Buffer::elementAlignment() const noexcept { return m_impl->m_alignment; }

size_t DirectX12Buffer::alignedElementSize() const noexcept
{
  return m_impl->m_alignment == 0
         ? m_impl->m_elementSize
         : (m_impl->m_elementSize + m_impl->m_alignment - 1) & ~(m_impl->m_alignment - 1);
}
...
void DirectX12Buffer::map(const void * const data, size_t size, UInt32 element)
{
  if (element >= m_impl->m_elements) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "element", 0u, m_impl->m_elements, element,
      "The element {0} is out of range. The buffer only contains {1} elements.", element,
      m_impl->m_elements);
...
UniquePtr<IDirectX12Buffer> DirectX12Buffer::allocate(
  AllocatorPtr allocator, BufferType type, UInt32 elements, size_t elementSize, size_t alignment,
  ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  return DirectX12Buffer::allocate("", allocator, type, elements, elementSize, alignment, usage,
                                   resourceDesc, allocationDesc);
}

UniquePtr<IDirectX12Buffer> DirectX12Buffer::allocate(
  const String & name, AllocatorPtr allocator, BufferType type, UInt32 elements, size_t elementSize,
  size_t alignment, ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  ...
  LITEFX_DEBUG(
    DIRECTX12_LOG,
    "Allocated buffer {0} with {4} bytes {{ Type: {1}, Elements: {2}, Element Size: {3}, Usage: {5} }}",
    name.empty() ? std::format("{0}", reinterpret_cast<void *>(resource.Get())) : name, type,
    elements, elementSize, elements * elementSize, usage);

  return makeUnique<DirectX12Buffer>(std::move(resource), type, elements, elementSize, alignment,
                                     usage, allocator, AllocationPtr(allocation), name);
}
...
public:
  DirectX12VertexBufferImpl(DirectX12VertexBuffer * parent,
...
public:
  ...
  {
    m_view =
      D3D12_VERTEX_BUFFER_VIEW{.BufferLocation = m_parent->handle()->GetGPUVirtualAddress(),
...
DirectX12VertexBuffer::DirectX12VertexBuffer(ComPtr<ID3D12Resource> && buffer,
                                             const DirectX12VertexBufferLayout & layout,
                                             UInt32 elements, ResourceUsage usage,
                                             AllocatorPtr allocator, AllocationPtr && allocation,
                                             const String & name)
  : m_impl(makePimpl<DirectX12VertexBufferImpl>(this, layout))
  , DirectX12Buffer(std::move(buffer), BufferType::Vertex, elements, layout.elementSize(), 0, usage,
                    allocator, std::move(allocation), name)
{
  m_impl->initialize();
}
...
UniquePtr<IDirectX12VertexBuffer> DirectX12VertexBuffer::allocate(
  const DirectX12VertexBufferLayout & layout, AllocatorPtr allocator, UInt32 elements,
  ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  return DirectX12VertexBuffer::allocate("", layout, allocator, elements, usage, resourceDesc,
                                         allocationDesc);
}

UniquePtr<IDirectX12VertexBuffer> DirectX12VertexBuffer::allocate(
  const String & name, const DirectX12VertexBufferLayout & layout, AllocatorPtr allocator,
  UInt32 elements, ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  if (allocator == nullptr) [[unlikely]]
    throw ArgumentNotInitializedException("allocator", "The allocator must be initialized.");
  ...
  LITEFX_DEBUG(
    ...
    BufferType::Vertex, elements, layout.elementSize(), layout.elementSize() * elements, usage);

  return makeUnique<DirectX12VertexBuffer>(std::move(resource), layout, elements, usage, allocator,
                                           AllocationPtr(allocation), name);
}
...
public:
  void initialize()
  {
    m_view = D3D12_INDEX_BUFFER_VIEW{.BufferLocation = m_parent->handle()->GetGPUVirtualAddress(),
                                     .SizeInBytes = static_cast<UInt32>(m_parent->size()),
                                     .Format = m_parent->layout().indexType() == IndexType::UInt16
...
DirectX12IndexBuffer::DirectX12IndexBuffer(ComPtr<ID3D12Resource> && buffer,
                                           const DirectX12IndexBufferLayout & layout,
                                           UInt32 elements, ResourceUsage usage,
                                           AllocatorPtr allocator, AllocationPtr && allocation,
                                           const String & name)
  : m_impl(makePimpl<DirectX12IndexBufferImpl>(this, layout))
  , DirectX12Buffer(std::move(buffer), BufferType::Index, elements, layout.elementSize(), 0, usage,
                    allocator, std::move(allocation), name)
{
  m_impl->initialize();
}
...
UniquePtr<IDirectX12IndexBuffer> DirectX12IndexBuffer::allocate(
  const DirectX12IndexBufferLayout & layout, AllocatorPtr allocator, UInt32 elements,
  ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  return DirectX12IndexBuffer::allocate("", layout, allocator, elements, usage, resourceDesc,
                                        allocationDesc);
}

UniquePtr<IDirectX12IndexBuffer> DirectX12IndexBuffer::allocate(
  const String & name, const DirectX12IndexBufferLayout & layout, AllocatorPtr allocator,
  UInt32 elements, ResourceUsage usage, const D3D12_RESOURCE_DESC1 & resourceDesc,
  const D3D12MA::ALLOCATION_DESC & allocationDesc)
{
  if (allocator == nullptr) [[unlikely]]
  ...
  LITEFX_DEBUG(
    ...
    "Allocated buffer {0} with {4} bytes {{ Type: {1}, Elements: {2}, Element Size: {3}, Usage: {5} }}",
    name.empty() ? std::format("{0}", reinterpret_cast<void *>(resource.Get())) : name,
    BufferType::Index, elements, layout.elementSize(), layout.elementSize() * elements, usage);

  return makeUnique<DirectX12IndexBuffer>(std::move(resource), layout, elements, usage, allocator,
                                          AllocationPtr(allocation), name);
}
