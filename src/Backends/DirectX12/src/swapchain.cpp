...
private:
  bool supportsVariableRefreshRates(const DirectX12Backend & backend) const
  {
    ...
    if (FAILED(backend.handle()->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING,
...
public:
  [[nodiscard]] ComPtr<IDXGISwapChain4> initialize(Format format, const Size2d & renderArea,
                                                   UInt32 backBuffers, bool enableVsync)
  {
    if (!std::ranges::any_of(m_parent->getSurfaceFormats(),
                             [&format](Format surfaceFormat) { return surfaceFormat == format; }))
    ...
    auto size =
      Size2d{std::max<UInt32>(1, renderArea.width()), std::max<UInt32>(1, renderArea.height())};
    LITEFX_TRACE(
      DIRECTX12_LOG,
      "Creating swap chain for device {0} {{ Images: {1}, Extent: {2}x{3} Px, Format: {4}, VSync: {5} }}...",
      reinterpret_cast<void *>(m_device.handle().Get()), backBuffers, size.width(), size.height(),
      format, enableVsync);
    ...
    swapChainDesc.BufferUsage =
      DXGI_USAGE_RENDER_TARGET_OUTPUT; // NOTE: D3D12 does no longer allow UAV access to back buffers, so binding swap chain images to UAV in compute/ray-tracing shaders does not work.
    swapChainDesc.BufferCount = std::max<UInt32>(2, backBuffers);
    swapChainDesc.Scaling = DXGI_SCALING_STRETCH;
    swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
    swapChainDesc.AlphaMode = DXGI_ALPHA_MODE_UNSPECIFIED;
    swapChainDesc.Flags = (m_supportsVariableRefreshRates = supportsVariableRefreshRates(backend))
    ...
    raiseIfFailed(backend.handle()->CreateSwapChainForHwnd(graphicsQueue.Get(), surface,
                                                           &swapChainDesc, nullptr, nullptr,
                                                           &swapChainBase),
                  "Unable to create swap chain for device.");
    raiseIfFailed(swapChainBase.As(&swapChain),
                  "The swap chain does not implement the IDXGISwapChain4 interface.");
    ...
    std::ranges::
      generate(m_presentImages,
               [this, &size, &format, &swapChain, i = 0]() mutable
               {
                 ComPtr<ID3D12Resource> resource;
                 raiseIfFailed(swapChain->GetBuffer(i++, IID_PPV_ARGS(&resource)),
                               "Unable to acquire image resource from swap chain back buffer {0}.",
                               i);
                 return makeUnique<DirectX12Image>(m_device, std::move(resource), size, format,
                                                   ImageDimensions::DIM_2, 1, 1,
  ...
  void reset(Format format, const Size2d & renderArea, UInt32 backBuffers, bool enableVsync)
  {
    ...
    m_presentFences
      .clear(); // No need to wait on them, since we are not going to hand out those back-buffers again anyway.
    ...
    auto size =
      Size2d{std::max<UInt32>(1, renderArea.width()), std::max<UInt32>(1, renderArea.height())};
    raiseIfFailed(m_parent->handle()->ResizeBuffers(buffers, static_cast<UInt32>(size.width()),
                                                    static_cast<UInt32>(size.height()),
                                                    DX12::getFormat(format),
                                                    m_supportsVariableRefreshRates
                                                      ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING
                                                      : 0),
                  "Unable to resize swap chain back buffers.");
    LITEFX_TRACE(
      DIRECTX12_LOG,
    ...
    std::ranges::
      generate(m_presentImages,
               [this, &size, &format, i = 0]() mutable
               {
                 ComPtr<ID3D12Resource> resource;
                 raiseIfFailed(m_parent->handle()->GetBuffer(i++, IID_PPV_ARGS(&resource)),
                               "Unable to acquire image resource from swap chain back buffer {0}.",
                               i);
                 return makeUnique<DirectX12Image>(m_device, std::move(resource), size, format,
                                                   ImageDimensions::DIM_2, 1, 1,
  ...
  void resetQueryHeaps(const Array<SharedPtr<TimingEvent>> & timingEvents)
  {
    ...
    std::ranges::generate(m_timingQueryHeaps,
                          [this, &timingEvents]()
                          {
                            D3D12_QUERY_HEAP_DESC heapInfo{.Type = D3D12_QUERY_HEAP_TYPE_TIMESTAMP,
                                                           .Count = static_cast<UInt32>(
                                                             timingEvents.size()),
                                                           .NodeMask = 0x01};

                            ComPtr<ID3D12QueryHeap> heap;
                            raiseIfFailed(m_device.handle()->CreateQueryHeap(&heapInfo,
                                                                             IID_PPV_ARGS(&heap)),
                                          "Unable to create timestamp query heap.");
                            return heap;
                          });

    // Create a readback buffer for each heap.
    m_timingQueryReadbackBuffers.resize(m_buffers);
    std::ranges::generate(m_timingQueryReadbackBuffers,
                          [this, &timingEvents]()
                          {
                            return m_device.factory().createBuffer(BufferType::Other,
  ...
  UInt32 swapBackBuffer()
  {
    ...
    if (!m_timestamps.empty())
      m_timingQueryReadbackBuffers[m_currentImage]->map(m_timestamps.data(),
                                                        sizeof(UInt64) * m_timestamps.size(), 0,
                                                        false);
...
DirectX12SwapChain::DirectX12SwapChain(const DirectX12Device & device, Format format,
                                       const Size2d & renderArea, UInt32 backBuffers,
                                       bool enableVsync)
...
Format DirectX12SwapChain::surfaceFormat() const noexcept { return m_impl->m_format; }

UInt32 DirectX12SwapChain::buffers() const noexcept { return m_impl->m_buffers; }

const Size2d & DirectX12SwapChain::renderArea() const noexcept { return m_impl->m_renderArea; }

bool DirectX12SwapChain::verticalSynchronization() const noexcept { return m_impl->m_vsync; }
...
Enumerable<IDirectX12Image *> DirectX12SwapChain::images() const noexcept
{
  return m_impl->m_presentImages |
         std::views::transform([](UniquePtr<IDirectX12Image> & image) { return image.get(); });
}
...
Enumerable<Format> DirectX12SwapChain::getSurfaceFormats() const noexcept
{
  ...
  return Enumerable<Format>{DX12::getFormat(DXGI_FORMAT_R16G16B16A16_FLOAT),
...
void DirectX12SwapChain::resolveQueryHeaps(
  const DirectX12CommandBuffer & commandBuffer) const noexcept
{
  ...
  commandBuffer.handle()
    ->ResolveQueryData(m_impl->m_timingQueryHeaps[m_impl->m_currentImage].Get(),
                       D3D12_QUERY_TYPE_TIMESTAMP, 0, m_impl->m_timestamps.size(),
                       std::as_const(*m_impl->m_timingQueryReadbackBuffers[m_impl->m_currentImage])
                         .handle()
                         .Get(),
                       0);
}
