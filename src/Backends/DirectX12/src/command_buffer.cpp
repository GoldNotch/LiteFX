...
public:
  ComPtr<ID3D12GraphicsCommandList7> initialize(bool begin, bool primary)
  {
    ...
    m_queue.device().indirectDrawSignatures(m_dispatchSignature, m_dispatchMeshSignature,
                                            m_drawSignature, m_drawIndexedSignature);
    ...
    raiseIfFailed(m_queue.device().handle()->CreateCommandAllocator(type, IID_PPV_ARGS(
                                                                            &m_commandAllocator)),
                  "Unable to create command allocator for command buffer.");

    // Create the actual command list.
    ComPtr<ID3D12GraphicsCommandList7> commandList;

    if (m_recording = begin)
      raiseIfFailed(m_queue.device().handle()->CreateCommandList(0, type, m_commandAllocator.Get(),
                                                                 nullptr,
                                                                 IID_PPV_ARGS(&commandList)),
                    "Unable to create command list for command buffer.");
    else
      raiseIfFailed(m_queue.device().handle()->CreateCommandList1(0, type,
  ...
  void reset()
  {
    ...
    raiseIfFailed(m_parent->handle()->Reset(m_commandAllocator.Get(), nullptr),
                  "Unable to reset command list.");
    m_recording = true;
  ...
  inline void buildAccelerationStructure(DirectX12BottomLevelAccelerationStructure & blas,
                                         const SharedPtr<const IDirectX12Buffer> scratchBuffer,
                                         const IDirectX12Buffer & buffer, UInt64 offset,
                                         bool update)
  {
    ...
    D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC blasDesc =
      {.DestAccelerationStructureData = buffer.virtualAddress() + offset,
       .Inputs = {.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL,
                  .Flags = std::bit_cast<D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS>(
                    blas.flags()),
  ...
  inline void buildAccelerationStructure(DirectX12TopLevelAccelerationStructure & tlas,
                                         const SharedPtr<const IDirectX12Buffer> scratchBuffer,
                                         const IDirectX12Buffer & buffer, UInt64 offset,
                                         bool update)
  {
    ...
    auto instanceBuffer =
      m_queue.device().factory().createBuffer(BufferType::Storage, ResourceHeap::Dynamic,
    ...
    instanceBuffer->map(buildInfo.data(),
                        sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * buildInfo.size());
...
DirectX12CommandBuffer::DirectX12CommandBuffer(const DirectX12Queue & queue, bool begin,
                                               bool primary)
...
const ICommandQueue & DirectX12CommandBuffer::queue() const noexcept { return m_impl->m_queue; }
...
bool DirectX12CommandBuffer::isSecondary() const noexcept { return m_impl->m_secondary; }
...
void DirectX12CommandBuffer::setViewports(Span<const IViewport *> viewports) const noexcept
{
  auto vps = viewports |
             std::views::transform(
               [](const auto & viewport)
               {
                 return CD3DX12_VIEWPORT(viewport->getRectangle().x(), viewport->getRectangle().y(),
                                         viewport->getRectangle().width(),
                                         viewport->getRectangle().height(), viewport->getMinDepth(),
...
void DirectX12CommandBuffer::setScissors(Span<const IScissor *> scissors) const noexcept
{
  auto scs = scissors |
             std::views::transform(
               [](const auto & scissor)
               {
                 return CD3DX12_RECT(scissor->getRectangle().x(), scissor->getRectangle().y(),
                                     scissor->getRectangle().width(),
                                     scissor->getRectangle().height());
               }) |
...
void DirectX12CommandBuffer::generateMipMaps(IDirectX12Image & image) noexcept
{
  ...
  std::ranges::generate(parametersData,
                        [this, &image, i = 0]() mutable
                        {
                          auto level = i++;

                          return Parameters{.sizeX =
                                              1.f /
                                              static_cast<Float>(
                                                std::max<size_t>(image.extent(level).width(), 1)),
                                            .sizeY =
                                              1.f /
                                              static_cast<Float>(
                                                std::max<size_t>(image.extent(level).height(), 1)),
                                            .sRGB = DX12::isSRGB(image.format()) ? 1.f : 0.f};
                        });

  auto parametersBlock =
    parametersData |
    std::views::transform([](const Parameters & parameters)
                          { return reinterpret_cast<const void *>(&parameters); }) |
  ...
  auto parameters =
    m_impl->m_queue.device().factory().createBuffer(parametersLayout.type(), ResourceHeap::Dynamic,
                                                    parametersLayout.elementSize(), image.levels());
  ...
  auto sampler = m_impl->m_queue.device().factory().createSampler(FilterMode::Linear,
                                                                  FilterMode::Linear,
...
UniquePtr<DirectX12Barrier> DirectX12CommandBuffer::makeBarrier(
  PipelineStage syncBefore, PipelineStage syncAfter) const noexcept
{
                                      ...
                                      UInt32 targetElement, UInt32 elements) const
{
  ...
  this->handle()->CopyBufferRegion(std::as_const(target).handle().Get(),
                                   ...
                                   elements * source.alignedElementSize());
}

void DirectX12CommandBuffer::transfer(const void * const data, size_t size,
                                      const IDirectX12Buffer & target, UInt32 targetElement,
...
void DirectX12CommandBuffer::transfer(Span<const void * const> data, size_t elementSize,
                                      const IDirectX12Buffer & target, UInt32 firstElement) const
{
                                      ...
                                      UInt32 firstSubresource, UInt32 elements) const
{
  ...
  {
    m_impl->m_queue.device().handle()->GetCopyableFootprints(&targetDesc, sourceElement + sr, 1, 0,
...
void DirectX12CommandBuffer::transfer(const void * const data, size_t size,
                                      const IDirectX12Image & target, UInt32 subresource) const
{
  auto stagingBuffer = asShared(
...
void DirectX12CommandBuffer::transfer(Span<const void * const> data, size_t elementSize,
                                      const IDirectX12Image & target, UInt32 firstSubresource,
                                      ...
                                      UInt32 targetElement, UInt32 subresources) const
{
  ...
  for (int sr(0); sr < subresources; ++sr)
  {
    m_impl->m_queue.device().handle()->GetCopyableFootprints(&targetDesc, firstSubresource + sr, 1,
                                                             0, &footprint, nullptr, nullptr,
                                                             nullptr);
    CD3DX12_TEXTURE_COPY_LOCATION sourceLocation(std::as_const(source).handle().Get(), footprint),
...
void DirectX12CommandBuffer::transfer(SharedPtr<const IDirectX12Buffer> source,
                                      const IDirectX12Buffer & target, UInt32 sourceElement,
                                      UInt32 targetElement, UInt32 elements) const
{
...
void DirectX12CommandBuffer::transfer(SharedPtr<const IDirectX12Image> source,
                                      const IDirectX12Image & target, UInt32 sourceSubresource,
                                      UInt32 targetSubresource, UInt32 subresources) const
{
...
{
  if (m_impl->m_lastPipeline) [[likely]]
    std::ranges::for_each(descriptorSets | std::views::filter([](auto descriptorSet)
                                                              { return descriptorSet != nullptr; }),
                          [this](auto descriptorSet) {
                            m_impl->m_queue.device().bindDescriptorSet(*this, *descriptorSet,
...
void DirectX12CommandBuffer::bind(Span<const DirectX12DescriptorSet *> descriptorSets,
                                  const DirectX12PipelineState & pipeline) const noexcept
{
  std::ranges::for_each(descriptorSets | std::views::filter([](auto descriptorSet)
                                                            { return descriptorSet != nullptr; }),
                                              ...
                                              UInt64 countOffset, UInt32 maxBatches) const noexcept
{
  this->handle()->ExecuteIndirect(m_impl->m_dispatchSignature.Get(),
                                  ...
                                  batchBuffer.handle().Get(), offset, countBuffer.handle().Get(),
                                  countOffset);
}
...
void DirectX12CommandBuffer::dispatchMeshIndirect(const IDirectX12Buffer & batchBuffer,
                                                  const IDirectX12Buffer & countBuffer,
...
void DirectX12CommandBuffer::drawIndirect(const IDirectX12Buffer & batchBuffer,
                                          const IDirectX12Buffer & countBuffer, UInt64 offset,
                                          UInt64 countOffset, UInt32 maxBatches) const noexcept
{
...
void DirectX12CommandBuffer::buildAccelerationStructure(
  DirectX12BottomLevelAccelerationStructure & blas,
  const SharedPtr<const IDirectX12Buffer> scratchBuffer, const IDirectX12Buffer & buffer,
  UInt64 offset) const
{
...
void DirectX12CommandBuffer::updateAccelerationStructure(
  DirectX12BottomLevelAccelerationStructure & blas,
  const SharedPtr<const IDirectX12Buffer> scratchBuffer, const IDirectX12Buffer & buffer,
  UInt64 offset) const
{
...
void DirectX12CommandBuffer::copyAccelerationStructure(
...
void DirectX12CommandBuffer::traceRays(
  UInt32 width, UInt32 height, UInt32 depth, const ShaderBindingTableOffsets & offsets,
  const IDirectX12Buffer & rayGenerationShaderBindingTable,
  const IDirectX12Buffer * missShaderBindingTable, const IDirectX12Buffer * hitShaderBindingTable,
  const IDirectX12Buffer * callableShaderBindingTable) const noexcept
{
