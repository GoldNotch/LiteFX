...
class DirectX12DescriptorSetLayout::DirectX12DescriptorSetLayoutImpl
  : public Implement<DirectX12DescriptorSetLayout>
{
public:
...
public:
  DirectX12DescriptorSetLayoutImpl(
    DirectX12DescriptorSetLayout * parent, const DirectX12Device & device,
...
public:
  void initialize()
  {
    LITEFX_TRACE(DIRECTX12_LOG, "Defining layout for descriptor set {0} {{ Stages: {1} }}...",
                 m_space, m_stages);

    // Sort the layouts by binding.
    std::sort(std::begin(m_layouts), std::end(m_layouts),
              [](const UniquePtr<DirectX12DescriptorLayout> & a,
                 const UniquePtr<DirectX12DescriptorLayout> & b)
              { return a->binding() < b->binding(); });

    // Count the samplers and descriptors.
    std::ranges::for_each(
      m_layouts,
      [&, i = 0](const UniquePtr<DirectX12DescriptorLayout> & layout) mutable
      {
        LITEFX_TRACE(
          DIRECTX12_LOG,
          "\tWith descriptor {0}/{1} {{ Type: {2}, Element size: {3} bytes, Array size: {6}, Offset: {4}, Binding point: {5} }}...",
          ++i, m_layouts.size(), layout->descriptorType(), layout->elementSize(), 0,
        ...
        if (layout->descriptors() == -1)
        {
          if (m_layouts.size() != 1) [[unlikely]]
            throw InvalidArgumentException(
              "descriptorLayouts",
              "If an unbounded runtime array descriptor is used, it must be the only descriptor in the descriptor set, however the current descriptor set specifies {0} descriptors",
              m_layouts.size());
          else
...
public:
  void tryAllocate(ComPtr<ID3D12DescriptorHeap> & bufferHeap,
                   ComPtr<ID3D12DescriptorHeap> & samplerHeap, UInt32 descriptorCount)
  {
    ...
    if (m_descriptors > 0)
    {
      ...
      {
        D3D12_DESCRIPTOR_HEAP_DESC bufferHeapDesc = {.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
                                                     .NumDescriptors = descriptors,
                                                     .Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE};

        raiseIfFailed(
          m_device.handle()->CreateDescriptorHeap(&bufferHeapDesc, IID_PPV_ARGS(&bufferHeap)),
          "Unable create constant CPU descriptor heap for constant buffers and images.");
      }
    }
    ...
    if (m_samplers > 0)
    {
      ...
      {
        D3D12_DESCRIPTOR_HEAP_DESC samplerHeapDesc = {.Type = D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER,
                                                      .NumDescriptors = samplers,
                                                      .Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE};

        raiseIfFailed(m_device.handle()->CreateDescriptorHeap(&samplerHeapDesc,
                                                              IID_PPV_ARGS(&samplerHeap)),
                      "Unable create constant CPU descriptor heap for samplers.");
      }
    }
  }
};
...
DirectX12DescriptorSetLayout::DirectX12DescriptorSetLayout(
  ...
  Enumerable<UniquePtr<DirectX12DescriptorLayout>> && descriptorLayouts, UInt32 space,
  ShaderStage stages)
  : m_impl(makePimpl<DirectX12DescriptorSetLayoutImpl>(this, device, std::move(descriptorLayouts),
                                                       space, stages))
{
  m_impl->initialize();
}
...
UInt32 DirectX12DescriptorSetLayout::descriptorOffsetForBinding(UInt32 binding) const
{
  if (!m_impl->m_bindingToDescriptor.contains(binding)) [[unlikely]]
    throw InvalidArgumentException(
      "binding", "The descriptor set does not contain a descriptor at binding {0}.", binding);
...
Enumerable<const DirectX12DescriptorLayout *> DirectX12DescriptorSetLayout::descriptors()
  const noexcept
{
  return m_impl->m_layouts |
         std::views::transform([](const UniquePtr<DirectX12DescriptorLayout> & layout)
                               { return layout.get(); });
}
...
{
  if (auto match =
        std::ranges::find_if(m_impl->m_layouts,
                             [&binding](const UniquePtr<DirectX12DescriptorLayout> & layout)
                             { return layout->binding() == binding; });
      match != m_impl->m_layouts.end()) [[likely]]
    return *match->get();
...
UInt32 DirectX12DescriptorSetLayout::space() const noexcept { return m_impl->m_space; }

ShaderStage DirectX12DescriptorSetLayout::shaderStages() const noexcept { return m_impl->m_stages; }

UInt32 DirectX12DescriptorSetLayout::uniforms() const noexcept
{
  return std::ranges::count_if(m_impl->m_layouts,
                               [](const UniquePtr<DirectX12DescriptorLayout> & layout) {
                                 return layout->descriptorType() == DescriptorType::ConstantBuffer;
                               });
}
...
UInt32 DirectX12DescriptorSetLayout::buffers() const noexcept
{
  return std::ranges::count_if(m_impl->m_layouts,
                               [](const UniquePtr<DirectX12DescriptorLayout> & layout)
                               {
                                 return layout->descriptorType() == DescriptorType::Buffer ||
                                        layout->descriptorType() == DescriptorType::RWBuffer;
                               });
}
...
UInt32 DirectX12DescriptorSetLayout::images() const noexcept
{
  return std::ranges::count_if(m_impl->m_layouts,
                               [](const UniquePtr<DirectX12DescriptorLayout> & layout)
                               {
                                 return layout->descriptorType() == DescriptorType::Texture ||
                                        layout->descriptorType() == DescriptorType::RWTexture;
                               });
}
...
UInt32 DirectX12DescriptorSetLayout::inputAttachments() const noexcept
{
  return std::ranges::count_if(m_impl->m_layouts,
                               [](const UniquePtr<DirectX12DescriptorLayout> & layout) {
                                 return layout->descriptorType() == DescriptorType::InputAttachment;
                               });
}
  ...
  UInt32 descriptors, const Enumerable<DescriptorBinding> & bindings) const
{
  ...
  auto descriptorSet =
    makeUnique<DirectX12DescriptorSet>(*this, std::move(bufferHeap), std::move(samplerHeap));
...
Enumerable<UniquePtr<DirectX12DescriptorSet>> DirectX12DescriptorSetLayout::allocateMultiple(
  UInt32 count, UInt32 descriptors,
  const Enumerable<Enumerable<DescriptorBinding>> & bindings) const
{
  return [this, descriptors, &bindings,
...
{
  return [this, descriptors, &bindingFactory,
...
DirectX12DescriptorSetLayoutBuilder::DirectX12DescriptorSetLayoutBuilder(
  DirectX12PipelineLayoutBuilder & parent, UInt32 space, ShaderStage stages)
  : DescriptorSetLayoutBuilder(parent, UniquePtr<DirectX12DescriptorSetLayout>(
                                         new DirectX12DescriptorSetLayout(parent.device())))
{
}
