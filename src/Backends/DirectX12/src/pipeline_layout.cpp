...
class DirectX12PipelineLayout::DirectX12PipelineLayoutImpl
  : public Implement<DirectX12PipelineLayout>
{
public:
...
public:
  DirectX12PipelineLayoutImpl(
    DirectX12PipelineLayout * parent, const DirectX12Device & device,
    Enumerable<UniquePtr<DirectX12DescriptorSetLayout>> && descriptorLayouts,
    UniquePtr<DirectX12PushConstantsLayout> && pushConstantsLayout)
    : base(parent)
    , m_pushConstantsLayout(std::move(pushConstantsLayout))
    , m_device(device)
  {
    m_descriptorSetLayouts = descriptorLayouts | std::views::as_rvalue |
...
public:
  ComPtr<ID3D12RootSignature> initialize()
  {
    ...
    std::ranges::sort(m_descriptorSetLayouts, [](const UniquePtr<DirectX12DescriptorSetLayout> & a,
                                                 const UniquePtr<DirectX12DescriptorSetLayout> & b)
                      { return a->space() < b->space(); });

    for (Tuple spaces :
         m_descriptorSetLayouts |
           std::views::transform([](const UniquePtr<DirectX12DescriptorSetLayout> & layout)
                                 { return layout->space(); }) |
           std::views::adjacent_transform<2>([](UInt32 a, UInt32 b)
                                             { return std::make_tuple(a, b); }))
    {
      ...
      if (a == b) [[unlikely]]
        throw InvalidArgumentException(
          ...
          "Two layouts defined for the same descriptor set {}. Each descriptor set must use it's own space.",
          a);
    }
    ...
    LITEFX_TRACE(
      DIRECTX12_LOG,
      "Creating render pipeline layout {0} {{ Descriptor Sets: {1}, Push Constant Ranges: {2} }}...",
      reinterpret_cast<void *>(m_parent), m_descriptorSetLayouts.size(),
      m_pushConstantsLayout == nullptr ? 0 : m_pushConstantsLayout->ranges().size());

    if (m_pushConstantsLayout != nullptr)
    {
      std::ranges::for_each(m_pushConstantsLayout->ranges(),
                            [&](DirectX12PushConstantsRange * range)
                            {
                              ...
                              switch (range->stage())
                              {
                                case ShaderStage::Vertex:
                                  rootParameter.InitAsConstants(range->size() / 4, range->binding(),
                                                                range->space(),
                                ...
                                case ShaderStage::Task:
                                  rootParameter
                                    .InitAsConstants(range->size() / 4, range->binding(),
                                                     range->space(),
    ...
    std::ranges::
      for_each(m_descriptorSetLayouts,
               [&](const UniquePtr<DirectX12DescriptorSetLayout> & layout)
               {
                 ...
                 Array<D3D12_DESCRIPTOR_RANGE1> rangeSet =
                   layouts |
                   std::views::filter(
                     [](const DirectX12DescriptorLayout * range)
                     { return range->staticSampler() == nullptr && !range->local(); }) |
                   std::views::transform(
                     [&](const DirectX12DescriptorLayout * range)
                     {
                       ...
                       switch (range->descriptorType())
                       {
                         case DescriptorType::ConstantBuffer:
                           descriptorRange
                             .Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, range->descriptors(),
                                   range->binding(), space,
                         ...
                         case DescriptorType::Texture:
                           descriptorRange
                             .Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, range->descriptors(),
                                   range->binding(), space,
                                   D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE |
                                     D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE,
                                   D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND);
                         ...
                         case DescriptorType::RWTexture:
                           descriptorRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV,
                                                range->descriptors(), range->binding(), space,
                                                D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE,
                                                D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND);
                           break;
                         case DescriptorType::Sampler:
                           descriptorRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER,
                                                range->descriptors(), range->binding(), space,
                                                D3D12_DESCRIPTOR_RANGE_FLAG_NONE,
                                                D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND);
                           break;
                         default:
                           throw InvalidArgumentException("descriptorSetLayouts",
                                       ...
                                       [&](const DirectX12DescriptorLayout * range)
                                       {
                                         if (range->staticSampler() != nullptr)
                                         {
                                           ...
                                           D3D12_STATIC_SAMPLER_DESC samplerInfo =
                                             {.Filter =
                                                getFilterMode(sampler->getMinifyingFilter(),
                                              ...
                                              .MaxAnisotropy =
                                                static_cast<UInt32>(sampler->getAnisotropy()),
                 ...
                 {
                   CD3DX12_ROOT_PARAMETER1 rootParameter = {};
                   rootParameter.InitAsDescriptorTable(rangeSet.size(), rangeSet.data(),
                                                       static_cast<D3D12_SHADER_VISIBILITY>(
                                                         shaderStages));
    ...
    rootSignatureDesc.Init_1_1(static_cast<UInt32>(descriptorParameters.size()),
                               descriptorParameters.data(),
    ...
    raiseIfFailed(hr, "Unable to serialize root signature to create pipeline layout: {0}",
                  errorString);
    ...
    raiseIfFailed(m_device.handle()->CreateRootSignature(0, signature->GetBufferPointer(),
                  ...
                  "Unable to create root signature for pipeline layout.");
...
DirectX12PipelineLayout::DirectX12PipelineLayout(
  ...
  UniquePtr<DirectX12PushConstantsLayout> && pushConstantsLayout)
  : ComResource<ID3D12RootSignature>(nullptr)
...
DirectX12PipelineLayout::DirectX12PipelineLayout(const DirectX12Device & device) noexcept
  : ComResource<ID3D12RootSignature>(nullptr)
  , m_impl(makePimpl<DirectX12PipelineLayoutImpl>(this, device))
{
}
...
{
  if (auto match =
        std::ranges::find_if(m_impl->m_descriptorSetLayouts,
                             [&space](const UniquePtr<DirectX12DescriptorSetLayout> & layout)
                             { return layout->space() == space; });
      match != m_impl->m_descriptorSetLayouts.end())
    return *match->get();

  throw ArgumentOutOfRangeException("space",
                                    "No descriptor set layout uses the provided space {0}.", space);
}

Enumerable<const DirectX12DescriptorSetLayout *> DirectX12PipelineLayout::descriptorSets()
  const noexcept
{
...
class DirectX12PipelineLayoutBuilder::DirectX12PipelineLayoutBuilderImpl
  : public Implement<DirectX12PipelineLayoutBuilder>
{
public:
  friend class DirectX12PipelineLayoutBuilder;
  friend class DirectX12PipelineLayout;
...
DirectX12PipelineLayoutBuilder::DirectX12PipelineLayoutBuilder(const DirectX12Device & parent)
  : m_impl(makePimpl<DirectX12PipelineLayoutBuilderImpl>(this, parent))
  , PipelineLayoutBuilder(SharedPtr<DirectX12PipelineLayout>(new DirectX12PipelineLayout(parent)))
{
}
