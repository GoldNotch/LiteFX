...
{
  return static_cast<UInt32>((Byte)ch0) | static_cast<UInt32>((Byte)ch1) << 8 |
         static_cast<UInt32>((Byte)ch2) << 16 | static_cast<UInt32>((Byte)ch3) << 24;
}
...
public:
  void validate()
  {
    ...
    if (std::ranges::contains(m_modules, nullptr)) [[unlikely]]
      throw InvalidArgumentException("modules",
                                     "At least one of the shader modules is not initialized.");

    // Check if there are combinations, that are not supported.
    Dictionary<ShaderStage, UInt32> shaders = {{ShaderStage::Compute, 0},
    ...
    bool containsGraphicsGroup = shaders[ShaderStage::Vertex] > 0 ||
                                 shaders[ShaderStage::Geometry] > 0 ||
                                 shaders[ShaderStage::TessellationControl] > 0 ||
                                 shaders[ShaderStage::TessellationEvaluation] > 0;
    ...
    bool containsRaytracingGroup =
      shaders[ShaderStage::RayGeneration] > 0 || shaders[ShaderStage::Miss] > 0 ||
      shaders[ShaderStage::Callable] > 0 || shaders[ShaderStage::AnyHit] > 0 ||
      shaders[ShaderStage::ClosestHit] > 0 || shaders[ShaderStage::Intersection] > 0;

    // Compute groups must be compute only.
    if (containsComputeGroup)
    {
      if (containsGraphicsGroup || containsMeshGroup || containsFragmentGroup ||
          containsRaytracingGroup) [[unlikely]]
        throw InvalidArgumentException(
          "modules", "The provided shader modules mix compute shaders with non-compute shaders.");
      if (shaders[ShaderStage::Compute] > 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules",
          "If a shader program contains a compute shader, it must contain only one shader module.");
    ...
    if (containsRaytracingGroup)
    {
      if (containsGraphicsGroup || containsMeshGroup || containsFragmentGroup) [[unlikely]]
        throw InvalidArgumentException(
          "modules",
          "If a shader program contains ray-tracing shaders, it must only contain ray-tracing shaders.");
      if (containsRaytracingGroup && shaders[ShaderStage::RayGeneration] != 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules",
          "If ray-tracing shaders are present, there must also be exactly one ray generation shader.");
    ...
    if (containsMeshGroup)
    {
      if (containsGraphicsGroup) [[unlikely]]
        throw InvalidArgumentException("modules",
                                       "Mesh shaders must not be combined with graphics shaders.");
      if (shaders[ShaderStage::Fragment] != 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules", "In a mesh shader program, there must be exactly one fragment/pixel shader.");
      if (shaders[ShaderStage::Mesh] != 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules", "In a mesh shader program, there must be exactly one mesh shader.");
      if (shaders[ShaderStage::Task] > 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules", "In a mesh shader program, there must be at most one mesh shader.");
    ...
    if (containsGraphicsGroup)
    {
      if (shaders[ShaderStage::Fragment] != 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules",
          "In a graphics shader program, there must be exactly one fragment/pixel shader.");
      if (shaders[ShaderStage::Vertex] != 1) [[unlikely]]
        throw InvalidArgumentException(
          "modules", "In a graphics shader program, there must be exactly one vertex shader.");
      if (shaders[ShaderStage::TessellationControl] > 1 ||
          shaders[ShaderStage::TessellationEvaluation] > 1 || shaders[ShaderStage::Geometry] > 1)
        [[unlikely]]
        throw InvalidArgumentException(
          "modules",
          "In a graphics shader program, there must be at most one geometry, tessellation control/domain or tessellation evaluation/hull shader.");
    ...
    if (containsFragmentGroup) [[unlikely]]
      throw InvalidArgumentException(
        "modules", "A shader program that contains only a fragment/pixel shader is not valid.");
  }

  void reflectRootSignature(ComPtr<ID3D12RootSignatureDeserializer> deserializer,
                            ...
                            Array<PushConstantRangeInfo> & pushConstantRanges)
  {
    ...
    ShaderStage stages{};
    std::ranges::for_each(m_modules, [&stages](const auto & shaderModule)
                          { stages = stages | shaderModule->type(); });
    ...
    for (int i(0); i < description->NumStaticSamplers; ++i)
    {
      ...
      if (!descriptorSetLayouts.contains(staticSampler.RegisterSpace)) [[unlikely]]
      {
        LITEFX_DEBUG(
          DIRECTX12_LOG,
          "The root signature of the shader defines a descriptor set at space {0}, which the reflection did not find.",
          staticSampler.RegisterSpace);
        descriptorSetLayouts.insert(
          std::make_pair(staticSampler.RegisterSpace,
                         DescriptorSetInfo{.space = staticSampler.RegisterSpace, .stage = stages}));
      }

      // Find the sampler for the register and overwrite it. If it does not exist, create it.
      if (auto match =
            std::ranges::find_if(descriptorSetLayouts[staticSampler.RegisterSpace].descriptors,
                                 [&staticSampler](const auto & descriptor)
                                 {
                                   return descriptor.type == DescriptorType::Sampler &&
                                          descriptor.location == staticSampler.ShaderRegister;
                                 });
          match == descriptorSetLayouts[staticSampler.RegisterSpace].descriptors.end())
        descriptorSetLayouts[staticSampler.RegisterSpace].descriptors.push_back(
          DescriptorInfo{.location = staticSampler.ShaderRegister,
                         .elementSize = 0,
                         .elements = 1,
                         .type = DescriptorType::Sampler,
                         .staticSamplerState = staticSampler});
      else
    ...
    for (int i(0); i < description->NumParameters; ++i)
    {
      ...
      if (!descriptorSetLayouts.contains(rootParameter.Descriptor.RegisterSpace)) [[unlikely]]
      {
        ...
        descriptorSetLayouts.insert(
          std::make_pair(rootParameter.Descriptor.RegisterSpace,
                         DescriptorSetInfo{.space = rootParameter.Descriptor.RegisterSpace,
                                           .stage = stages}));
      }
      ...
      switch (rootParameter.ParameterType)
      {
        case D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS:
          // Find the descriptor.
          if (auto match =
                std::ranges::find_if(descriptorSetLayouts[rootParameter.Descriptor.RegisterSpace]
                                       .descriptors,
                                     [&rootParameter](const auto & descriptor) {
                                       return descriptor.location ==
                                              rootParameter.Descriptor.ShaderRegister;
                                     });
              match ==
              descriptorSetLayouts[rootParameter.Descriptor.RegisterSpace].descriptors.end())
            [[unlikely]]
            LITEFX_WARNING(
              DIRECTX12_LOG,
              "The root signature defines a descriptor at {0} (space {1}), which the shader reflection did not find. The descriptor will be ignored.",
              rootParameter.Descriptor.ShaderRegister, rootParameter.Descriptor.RegisterSpace);
          else
          {
            ...
            switch (rootParameter.ShaderVisibility)
            {
              ...
              case D3D12_SHADER_VISIBILITY_ALL:
                stage = ShaderStage::Any;
                break; // TODO: Might not work as intended.
              default:
                throw InvalidArgumentException(
                  "pushConstantRanges",
                  "The push constants for a shader are defined for invalid or unsupported shader stages. Note that a push constant must only be defined for a single shader stage.");
            }

            pushConstantRanges.push_back(
              PushConstantRangeInfo{.stage = stage,
                                    .offset = pushConstantOffset,
                                    .size = rootParameter.Constants.Num32BitValues * 4,
                                    .location = rootParameter.Descriptor.ShaderRegister,
                                    .space = rootParameter.Descriptor.RegisterSpace});
        ...
        case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE:
          throw InvalidArgumentException(
            "modules",
            "The shader modules root signature defines a descriptor table for parameter {0}, which is currently not supported. Convert each parameter of the table into a separate root parameter.",
            i);
        default:
          throw InvalidArgumentException(
            "modules",
            "The shader modules root signature exposes an unknown root parameter type {1} for parameter {0}.",
            i, rootParameter.ParameterType);
      }
    }
  }

  template<typename TReflection>
  DescriptorInfo getReflectionDescriptorDesc(D3D12_SHADER_INPUT_BIND_DESC inputDesc,
                                             TReflection * shaderReflection)
  {
    ...
    switch (inputDesc.Type)
    {
      case D3D_SIT_CBUFFER:
      {
        ...
        raiseIfFailed(constantBuffer->GetDesc(&bufferDesc),
                      "Unable to query constant buffer \"{0}\" at binding point {1} (space {2}).",
                      inputDesc.Name, inputDesc.BindPoint, inputDesc.Space);
      ...
      {
        type = inputDesc.Dimension == D3D_SRV_DIMENSION_BUFFER ? DescriptorType::Buffer
                                                               : DescriptorType::Texture;
        break;
      }
      case D3D_SIT_UAV_RWTYPED:
      {
        type = inputDesc.Dimension == D3D_SRV_DIMENSION_BUFFER ? DescriptorType::RWBuffer
      ...
      case D3D_SIT_UAV_FEEDBACKTEXTURE:
        throw RuntimeException(
          "The shader exposes an unsupported resource of type {1} at binding point {0} (space {2}).",
          inputDesc.BindPoint, inputDesc.Type, inputDesc.Space);
      default:
        throw RuntimeException(
          "The shader exposes an unknown resource type in binding {0} (space {1}).",
          inputDesc.BindPoint, inputDesc.Space);
    }

    DescriptorInfo descriptor = {.location = inputDesc.BindPoint,
  ...
  SharedPtr<DirectX12PipelineLayout> reflectPipelineLayout()
  {
    ...
    std::ranges::for_each(
      m_modules,
      [this, &descriptorSetLayouts](UniquePtr<DirectX12ShaderModule> & shaderModule)
      {
        // Load the shader reflection.
        ComPtr<IDxcContainerReflection> reflection;
        raiseIfFailed(::DxcCreateInstance(CLSID_DxcContainerReflection, IID_PPV_ARGS(&reflection)),
                      "Unable to access DirectX shader reflection.");
        raiseIfFailed(reflection->Load(std::as_const(*shaderModule).handle().Get()),
                      "Unable to load reflection from shader module.");

        // Callback to register a new descriptor set or merge a descriptor into an existing one.
        auto registerDescriptor = [&descriptorSetLayouts](DescriptorInfo & descriptor,
                                                          D3D12_SHADER_INPUT_BIND_DESC inputDesc,
                                                          const IShaderModule * shaderModule)
        {
          // Mark the descriptor as part of the local root signature, if the shader module has a local descriptor.
          descriptor.local =
            !shaderModule->shaderLocalDescriptor().has_value()
              ? false
              : shaderModule->shaderLocalDescriptor().value().Register == inputDesc.BindPoint &&
                  shaderModule->shaderLocalDescriptor().value().Space == inputDesc.Space;

          // Check if a descriptor set has already been defined for the space.
          if (!descriptorSetLayouts.contains(inputDesc.Space))
            descriptorSetLayouts.insert(
              std::make_pair(inputDesc.Space, DescriptorSetInfo{.space = inputDesc.Space,
                                                                .stage = shaderModule->type(),
                                                                .descriptors = {descriptor}}));
          else
          {
            auto & descriptorSetLayout = descriptorSetLayouts[inputDesc.Space];
            descriptorSetLayout.stage = descriptorSetLayouts[inputDesc.Space].stage |
                                        shaderModule->type();

            // If another descriptor is bound to the same register, check if they are compatible. Otherwise, simply add the descriptor.
            if (auto match =
                  std::ranges::find_if(descriptorSetLayouts[inputDesc.Space].descriptors,
                                       [&](const auto & descriptor)
                                       { return descriptor.location == inputDesc.BindPoint; });
                match == descriptorSetLayouts[inputDesc.Space].descriptors.end())
              descriptorSetLayout.descriptors.push_back(descriptor);
            else if (!match->equals(descriptor)) [[unlikely]]
              LITEFX_WARNING(
                DIRECTX12_LOG,
                "Two incompatible descriptors are bound to the same location ({0} in space {1}) at different shader stages.",
                descriptor.location, inputDesc.Space);
          }
        };
        ...
        if (LITEFX_FLAG_IS_SET(ShaderStage::RayTracingPipeline, shaderModule->type()))
        {
          ...
          raiseIfFailed(reflection->FindFirstPartKind(FOUR_CC('D', 'X', 'I', 'L'), &shaderIdx),
                        "The shader module does not contain a valid DXIL shader.");
          raiseIfFailed(reflection->GetPartReflection(shaderIdx, IID_PPV_ARGS(&shaderReflection)),
                        "Unable to query shader reflection from DXIL module.");

          // Get the shader description from the reflection.
          D3D12_LIBRARY_DESC shaderInfo;
          raiseIfFailed(shaderReflection->GetDesc(&shaderInfo),
                        "Unable to acquire meta-data from shader module.");
    ...
    for (const auto & shaderModule : m_modules)
    {
      ...
      if (SUCCEEDED(::D3D12CreateRootSignatureDeserializer(std::as_const(*shaderModule)
                                                             .handle()
                                                             ->GetBufferPointer(),
                                                           std::as_const(*shaderModule)
                                                             .handle()
                                                             ->GetBufferSize(),
    ...
    if (!hasRootSignature && !SUPPRESS_MISSING_ROOT_SIGNATURE_WARNING)
      LITEFX_WARNING(
        DIRECTX12_LOG,
        "None of the provided shader modules exports a root signature. Descriptor sets will be acquired using reflection. Some features (such as root/push constants) are not supported.");

    // Create the descriptor set layouts.
    auto descriptorSets = [this, &descriptorSetLayouts]()
      -> std::generator<UniquePtr<DirectX12DescriptorSetLayout>>
    {
      for (auto it = descriptorSetLayouts.begin(); it != descriptorSetLayouts.end(); ++it)
      {
        ...
        auto descriptors =
          [this, &descriptorSet]() -> std::generator<UniquePtr<DirectX12DescriptorLayout>>
        {
          for (auto descriptor = descriptorSet.descriptors.begin();
               descriptor != descriptorSet.descriptors.end(); ++descriptor)
            co_yield descriptor->staticSamplerState.has_value()
              ? makeUnique<DirectX12DescriptorLayout>(
                  makeUnique<DirectX12Sampler>(m_device,
                                               D3D12_DECODE_MAG_FILTER(
                                                 descriptor->staticSamplerState->Filter) ==
                                               ...
                                               D3D12_DECODE_MIN_FILTER(
                                                 ...
                                                 : FilterMode::Linear,
                                               DECODE_BORDER_MODE(
                                                 descriptor->staticSamplerState->AddressU),
                                               DECODE_BORDER_MODE(
                                                 descriptor->staticSamplerState->AddressV),
                                               DECODE_BORDER_MODE(
                                                 descriptor->staticSamplerState->AddressW),
                                               D3D12_DECODE_MIP_FILTER(
                                                 descriptor->staticSamplerState->Filter) ==
                                                   D3D12_FILTER_TYPE_POINT
                                                 ? MipMapMode::Nearest
                                                 : MipMapMode::Linear,
                                               descriptor->staticSamplerState->MipLODBias,
                                               descriptor->staticSamplerState->MinLOD,
                                               descriptor->staticSamplerState->MaxLOD,
                                               static_cast<Float>(
                                                 descriptor->staticSamplerState->MaxAnisotropy)),
                  descriptor->location)
              : makeUnique<DirectX12DescriptorLayout>(descriptor->type, descriptor->location,
                                                      descriptor->elementSize, descriptor->elements,
                                                      descriptor->local);
        }() | std::views::as_rvalue;

        co_yield makeUnique<DirectX12DescriptorSetLayout>(m_device, std::move(descriptors),
                                                          descriptorSet.space, descriptorSet.stage);
      }
    }() | std::views::as_rvalue |
           std::ranges::to<Enumerable<UniquePtr<DirectX12DescriptorSetLayout>>>();

    // Create the push constants layout.
    auto pushConstants = [&pushConstantRanges]()
      -> std::generator<UniquePtr<DirectX12PushConstantsRange>>
    {
      for (auto range = pushConstantRanges.begin(); range != pushConstantRanges.end(); ++range)
        co_yield makeUnique<DirectX12PushConstantsRange>(range->stage, range->offset, range->size,
                                                         range->space, range->location);
    }() | std::views::as_rvalue |
           std::ranges::to<Enumerable<UniquePtr<DirectX12PushConstantsRange>>>();

    auto overallSize = std::accumulate(pushConstantRanges.begin(), pushConstantRanges.end(), 0,
                                       [](UInt32 currentSize, const auto & range)
                                       { return currentSize + range.size; });
    auto pushConstantsLayout =
      makeUnique<DirectX12PushConstantsLayout>(std::move(pushConstants), overallSize);

    // Return the pipeline layout.
    return makeShared<DirectX12PipelineLayout>(m_device, std::move(descriptorSets),
...
SharedPtr<DirectX12ShaderProgram> DirectX12ShaderProgram::create(
  const DirectX12Device & device, Enumerable<UniquePtr<DirectX12ShaderModule>> && modules)
{
...
Enumerable<const DirectX12ShaderModule *> DirectX12ShaderProgram::modules() const noexcept
{
  return m_impl->m_modules |
         std::views::transform([](const UniquePtr<DirectX12ShaderModule> & shader)
                               { return shader.get(); });
}
...
class DirectX12ShaderProgramBuilder::DirectX12ShaderProgramBuilderImpl
  : public Implement<DirectX12ShaderProgramBuilder>
{
public:
  friend class DirectX12ShaderProgramBuilder;
...
DirectX12ShaderProgramBuilder::DirectX12ShaderProgramBuilder(const DirectX12Device & device)
  : m_impl(makePimpl<DirectX12ShaderProgramBuilderImpl>(this, device))
  , ShaderProgramBuilder(SharedPtr<DirectX12ShaderProgram>(new DirectX12ShaderProgram(device)))
{
}
