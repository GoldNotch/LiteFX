...
public:
  ...
  ~DirectX12QueueImpl() { m_submittedCommandBuffers.clear(); }

public:
  [[nodiscard]] ComPtr<ID3D12CommandQueue> initialize()
  {
    ...
    if (m_type == QueueType::Graphics)
      ...
      throw InvalidArgumentException(
        "type",
        "Unsupported combination of queue types. Only specify one queue type, even if the queue needs to support other tasks).");

    switch (m_priority)
    {
      default:
      case QueuePriority::Normal: desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL; break;
      case QueuePriority::High: desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_HIGH; break;
      case QueuePriority::Realtime:
        desc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_GLOBAL_REALTIME;
        break;
    }

    raiseIfFailed(m_device.handle()->CreateCommandQueue(&desc, IID_PPV_ARGS(&commandQueue)),
                  "Unable to create command queue of type {0} with priority {1}.", m_type,
                  m_priority);
    raiseIfFailed(m_device.handle()->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence)),
                  "Unable to create command buffer synchronization fence.");
...
const DirectX12Device & DirectX12Queue::device() const noexcept { return m_impl->m_device; }

QueueType DirectX12Queue::type() const noexcept { return m_impl->m_type; }
...
QueuePriority DirectX12Queue::priority() const noexcept { return m_impl->m_priority; }

SharedPtr<DirectX12CommandBuffer> DirectX12Queue::createCommandBuffer(bool beginRecording,
...
UInt64 DirectX12Queue::submit(
  const Enumerable<SharedPtr<const DirectX12CommandBuffer>> & commandBuffers) const
{
  ...
  if (!std::ranges::all_of(commandBuffers,
                           [](const auto & buffer) { return !buffer->isSecondary(); }))
    throw InvalidArgumentException(
      "commandBuffers",
