...
public:
  DirectX12FrameBufferImpl(DirectX12FrameBuffer * parent, const DirectX12Device & device,
...
public:
  void initialize()
  {
    ...
    UInt32 renderTargets = std::ranges::count_if(m_images,
                                                 [](const auto & image) {
                                                   return !::hasDepth(image->format()) &&
                                                          !::hasStencil(image->format());
                                                 });
    UInt32 depthStencilTargets = static_cast<UInt32>(m_images.size()) - renderTargets;

    D3D12_DESCRIPTOR_HEAP_DESC renderTargetHeapDesc = {.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
                                                       .NumDescriptors = renderTargets};

    D3D12_DESCRIPTOR_HEAP_DESC depthStencilHeapDesc = {.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV,
                                                       .NumDescriptors = depthStencilTargets};

    m_renderTargetDescriptorSize =
      m_device.handle()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    m_depthStencilDescriptorSize =
      m_device.handle()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);

    raiseIfFailed(m_device.handle()->CreateDescriptorHeap(&renderTargetHeapDesc,
                                                          IID_PPV_ARGS(&m_renderTargetHeap)),
                  "Unable to create render target descriptor heap.");
    CD3DX12_CPU_DESCRIPTOR_HANDLE renderTargetViewDescriptor(
      m_renderTargetHeap->GetCPUDescriptorHandleForHeapStart());
    raiseIfFailed(m_device.handle()->CreateDescriptorHeap(&depthStencilHeapDesc,
                                                          IID_PPV_ARGS(&m_depthStencilHeap)),
                  "Unable to create depth/stencil descriptor heap.");
    CD3DX12_CPU_DESCRIPTOR_HANDLE depthStencilViewDescriptor(
    ...
    std::ranges::for_each(
      m_images,
      [&, i = 0](const UniquePtr<IDirectX12Image> & image) mutable
      {
        ...
        if (m_device.maximumMultiSamplingLevel(format) < samples)
          throw RuntimeException("The image {0} with format {1} does not support {2} samples.", i,
                                 format, std::to_underlying(samples));

        if (::hasDepth(format) || ::hasStencil(format))
        {
          D3D12_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc =
            {.Format = DX12::getFormat(format),
             .ViewDimension = samples == MultiSamplingLevel::x1 ? D3D12_DSV_DIMENSION_TEXTURE2D
                                                                : D3D12_DSV_DIMENSION_TEXTURE2DMS,
             .Texture2D = {.MipSlice = 0}};

          m_device.handle()->CreateDepthStencilView(std::as_const(*image).handle().Get(),
        ...
        else
        {
          D3D12_RENDER_TARGET_VIEW_DESC renderTargetViewDesc =
            {.Format = DX12::getFormat(format),
             .ViewDimension = samples == MultiSamplingLevel::x1 ? D3D12_RTV_DIMENSION_TEXTURE2D
                                                                : D3D12_RTV_DIMENSION_TEXTURE2DMS,
             .Texture2D = {.MipSlice = 0, .PlaneSlice = 0}};

          m_device.handle()->CreateRenderTargetView(std::as_const(*image).handle().Get(),
  ...
  void resize(const Size2d & renderArea)
  {
    ...
    auto images = m_images |
                  std::views::transform(
                    [&](const UniquePtr<IDirectX12Image> & image)
                    {
                      auto newImage =
                        m_device.factory().createTexture(image->name(), image->format(), renderArea,
                                                         image->dimensions(), image->levels(),
                                                         image->layers(), image->samples(),
                                                         image->usage());
    ...
    // Update the mappings.
    std::ranges::for_each(m_mappedRenderTargets | std::views::values,
                          [&imageReplacements](auto & image) { image = imageReplacements[image]; });
...
DirectX12FrameBuffer::DirectX12FrameBuffer(const DirectX12Device & device,
                                           const Size2d & renderArea, StringView name)
...
D3D12_CPU_DESCRIPTOR_HANDLE DirectX12FrameBuffer::descriptorHandle(UInt32 imageIndex) const
{
  if (imageIndex >= m_impl->m_images.size()) [[unlikely]]
    throw ArgumentOutOfRangeException("imageIndex", 0u,
                                      static_cast<UInt32>(m_impl->m_images.size()), imageIndex,
                                      "The frame buffer does not contain an image at index {0}.",
                                      imageIndex);
...
D3D12_CPU_DESCRIPTOR_HANDLE DirectX12FrameBuffer::descriptorHandle(StringView imageName) const
{
  ...
  if (auto match = std::ranges::find_if(m_impl->m_images,
    ...
    throw InvalidArgumentException(
      "imageName", "The frame buffer does not contain an image with the name \"{0}\".", imageName);
}

D3D12_CPU_DESCRIPTOR_HANDLE DirectX12FrameBuffer::descriptorHandle(
  const RenderTarget & renderTarget) const
{
  if (!m_impl->m_mappedRenderTargets.contains(renderTarget.identifier())) [[unlikely]]
    throw InvalidArgumentException(
      "renderTarget",
      "The frame buffer does not map an image to the provided render target \"{0}\".",
      renderTarget.name());

  return m_impl->m_renderTargetHandles.at(m_impl->m_mappedRenderTargets[renderTarget.identifier()]);
}

const Size2d & DirectX12FrameBuffer::size() const noexcept { return m_impl->m_size; }

size_t DirectX12FrameBuffer::getWidth() const noexcept { return m_impl->m_size.width(); }

size_t DirectX12FrameBuffer::getHeight() const noexcept { return m_impl->m_size.height(); }
...
void DirectX12FrameBuffer::mapRenderTarget(const RenderTarget & renderTarget, UInt32 index)
{
  if (index >= m_impl->m_images.size()) [[unlikely]]
    throw ArgumentOutOfRangeException("index", 0u, static_cast<UInt32>(m_impl->m_images.size()),
                                      index,
                                      "The frame buffer does not contain an image at index {0}.",
                                      index);
  ...
  if (m_impl->m_images[index]->format() != renderTarget.format()) [[unlikely]]
    LITEFX_WARNING(DIRECTX12_LOG,
                   "The render target format {0} does not match the image format {1} for image {2}.",
                   renderTarget.format(), m_impl->m_images[index]->format(), index);
...
const IDirectX12Image & DirectX12FrameBuffer::resolveImage(UInt64 hash) const
{
  if (!m_impl->m_mappedRenderTargets.contains(hash)) [[unlikely]]
    throw InvalidArgumentException(
      "renderTarget",
      "The frame buffer does not map an image to the provided render target name hash \"0x{0:016X}\".",
      hash);

  return *m_impl->m_mappedRenderTargets[hash];
}

void DirectX12FrameBuffer::addImage(const String & name, Format format, MultiSamplingLevel samples,
                                    ResourceUsage usage)
{
  // Check if there's already another image with the same name.
  auto nameHash = hash(name);

  if (auto match = std::ranges::find_if(m_impl->m_images, [nameHash](auto & image)
                                        { return hash(image->name()) == nameHash; });
      match != m_impl->m_images.end()) [[unlikely]]
    throw InvalidArgumentException(
      "name", "Another image with the name {0} does already exist within the frame buffer.", name);

  // Add a new image.
  m_impl->m_images.push_back(std::move(
    m_impl->m_device.factory().createTexture(name, format, m_impl->m_size, ImageDimensions::DIM_2,
                                             1u, 1u, samples, usage)));
...
void DirectX12FrameBuffer::addImage(const String & name, const RenderTarget & renderTarget,
                                    MultiSamplingLevel samples, ResourceUsage usage)
{
  // Check if there's already another image with the same name.
  auto nameHash = hash(name);
  ...
  m_impl->m_images.push_back(std::move(
    m_impl->m_device.factory().createTexture(name, renderTarget.format(), m_impl->m_size,
                                             ImageDimensions::DIM_2, 1u, 1u, samples, usage)));
