...
class DirectX12GraphicsAdapter::DirectX12GraphicsAdapterImpl
  : public Implement<DirectX12GraphicsAdapter>
{
public:
...
DirectX12GraphicsAdapter::DirectX12GraphicsAdapter(ComPtr<IDXGIAdapter4> adapter)
  : ComResource<IDXGIAdapter4>(adapter)
  , m_impl(makePimpl<DirectX12GraphicsAdapterImpl>(this))
{
}
...
{
  return (static_cast<UInt64>(m_impl->m_properties.AdapterLuid.HighPart) << 0x20) |
         static_cast<UInt64>(m_impl->m_properties.AdapterLuid.LowPart);
}

UInt32 DirectX12GraphicsAdapter::vendorId() const noexcept { return m_impl->m_properties.VendorId; }

UInt32 DirectX12GraphicsAdapter::deviceId() const noexcept { return m_impl->m_properties.DeviceId; }

GraphicsAdapterType DirectX12GraphicsAdapter::type() const noexcept
{
  return m_impl->m_properties.Flags & DXGI_ADAPTER_FLAG3_SOFTWARE ? GraphicsAdapterType::CPU
                                                                  : GraphicsAdapterType::GPU;
}

UInt64 DirectX12GraphicsAdapter::driverVersion() const noexcept { return m_impl->m_driverVersion; }

UInt32 DirectX12GraphicsAdapter::apiVersion() const noexcept { return m_impl->m_apiVersion; }
