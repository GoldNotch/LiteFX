...
public:
  ...
  using RenderPassContext = Tuple<Array<D3D12_RENDER_PASS_RENDER_TARGET_DESC>,
                                  Optional<D3D12_RENDER_PASS_DEPTH_STENCIL_DESC>>;
...
private:
  ...
  Dictionary<const IFrameBuffer *, Array<SharedPtr<DirectX12CommandBuffer>>>
...
public:
  DirectX12RenderPassImpl(DirectX12RenderPass * parent, const DirectX12Device & device,
                          const DirectX12Queue & queue, Span<RenderTarget> renderTargets,
    ...
    , m_secondaryCommandBufferCount(secondaryCommandBuffers)
  {
    ...
    if (secondaryCommandBuffers == 0) [[unlikely]]
      LITEFX_WARNING(
        DIRECTX12_LOG,
        "Secondary command buffer count for this render pass is 0, which makes it prevents recording draw commands to this render pass.");
  }

  DirectX12RenderPassImpl(DirectX12RenderPass * parent, const DirectX12Device & device)
    : base(parent)
    , m_device(device)
    , m_queue(&device.defaultQueue(QueueType::Graphics))
  {
  }
...
public:
  void mapRenderTargets(Span<RenderTarget> renderTargets)
  {
    ...
    std::ranges::sort(m_renderTargets, [this](const auto & a, const auto & b)
                      { return a.location() < b.location(); });

    if (auto match =
          std::ranges::find_if(m_renderTargets, [](const RenderTarget & renderTarget)
                               { return renderTarget.type() == RenderTargetType::Present; });
        match != m_renderTargets.end())
    ...
    if (m_presentTarget != nullptr &&
      ...
      throw InvalidArgumentException(
        "renderTargets",
        "A render pass with a present target must be executed on the default graphics queue.");
  }
  ...
  void registerFrameBuffer(const DirectX12FrameBuffer & frameBuffer)
  {
    ...
    {
      m_frameBufferTokens[interfacePointer] =
        frameBuffer.released.add(std::bind(&DirectX12RenderPassImpl::onFrameBufferRelease, this,
      ...
      // Create begin command buffers.
      {
        ...
        std::as_const(*commandBuffer)
          .handle()
          ->SetName(Widen(std::format("{0} Begin Commands {1}", m_parent->name(),
                                      m_beginCommandBuffers.size()))
                      .c_str());
#endif
      ...
      // Create end command buffers.
      {
        ...
        std::as_const(*commandBuffer)
          .handle()
          ->SetName(
            Widen(std::format("{0} End Commands {1}", m_parent->name(), m_endCommandBuffers.size()))
              .c_str());
#endif
      ...
      m_secondaryCommandBuffers[interfacePointer] =
        std::views::iota(0u, m_secondaryCommandBufferCount) |
        std::views::transform(
          [this](UInt32 i)
          {
            auto commandBuffer = m_queue->createCommandBuffer(false);
            ...
            std::as_const(*commandBuffer)
              .handle()
              ->SetName(
                Widen(std::format("{0} Secondary Commands {1}", m_parent->name(), i)).c_str());
#endif
  ...
  void onFrameBufferRelease(const void * sender, IFrameBuffer::ReleasedEventArgs args)
  {
    ...
    if (static_cast<const IFrameBuffer *>(m_activeFrameBuffer) == interfacePointer) [[unlikely]]
      throw RuntimeException(
        "A frame buffer that is currently in use on a render pass cannot be released.");
  ...
  {
    std::get<0>(m_activeContext) =
      m_renderTargets |
      std::views::filter([](const RenderTarget & renderTarget)
                         { return renderTarget.type() != RenderTargetType::DepthStencil; }) |
      std::views::transform(
        [&frameBuffer](const RenderTarget & renderTarget)
        {
          Float clearColor[4] = {renderTarget.clearValues().x(), renderTarget.clearValues().y(),
                                 renderTarget.clearValues().z(), renderTarget.clearValues().w()};
          ...
          D3D12_RENDER_PASS_BEGINNING_ACCESS beginAccess =
            renderTarget.clearBuffer()
              ? D3D12_RENDER_PASS_BEGINNING_ACCESS{D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR,
                                                   {clearValue}}
          ...
          D3D12_RENDER_PASS_RENDER_TARGET_DESC renderTargetDesc{frameBuffer.descriptorHandle(
                                                                  renderTarget),
    ...
    {
      CD3DX12_CLEAR_VALUE clearValue{DX12::getFormat(m_depthStencilTarget->format()),
                                     m_depthStencilTarget->clearValues().x(),
                                     static_cast<Byte>(m_depthStencilTarget->clearValues().y())};

      D3D12_RENDER_PASS_ENDING_ACCESS depthEndAccess, stencilEndAccess;
      D3D12_RENDER_PASS_BEGINNING_ACCESS depthBeginAccess =
        m_depthStencilTarget->clearBuffer() && ::hasDepth(m_depthStencilTarget->format())
          ? D3D12_RENDER_PASS_BEGINNING_ACCESS{D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR,
                                               {clearValue}}
          : D3D12_RENDER_PASS_BEGINNING_ACCESS{D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_PRESERVE,
                                               {}};
      ...
      depthEndAccess =
        m_depthStencilTarget->isVolatile()
          ? D3D12_RENDER_PASS_ENDING_ACCESS{D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_DISCARD, {}}
          : D3D12_RENDER_PASS_ENDING_ACCESS{D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_PRESERVE, {}};

      D3D12_RENDER_PASS_BEGINNING_ACCESS stencilBeginAccess =
        m_depthStencilTarget->clearStencil() && ::hasStencil(m_depthStencilTarget->format())
          ? D3D12_RENDER_PASS_BEGINNING_ACCESS{D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR,
                                               {clearValue}}
          : D3D12_RENDER_PASS_BEGINNING_ACCESS{D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_PRESERVE,
                                               {}};
      ...
      std::get<1>(m_activeContext) =
        D3D12_RENDER_PASS_DEPTH_STENCIL_DESC{frameBuffer.descriptorHandle(*m_depthStencilTarget),
                                             depthBeginAccess, stencilBeginAccess, depthEndAccess,
                                             stencilEndAccess};
    }
  ...
  inline SharedPtr<DirectX12CommandBuffer> getBeginCommandBuffer(
    const DirectX12FrameBuffer & frameBuffer)
  {
    return m_beginCommandBuffers.at(static_cast<const IFrameBuffer *>(&frameBuffer));
  }

  inline SharedPtr<DirectX12CommandBuffer> getEndCommandBuffer(
    const DirectX12FrameBuffer & frameBuffer)
  {
    return m_endCommandBuffers.at(static_cast<const IFrameBuffer *>(&frameBuffer));
  }

  inline Array<SharedPtr<DirectX12CommandBuffer>> & getSecondaryCommandBuffers(
    const DirectX12FrameBuffer & frameBuffer)
  {
...
DirectX12RenderPass::DirectX12RenderPass(
  ...
  Span<RenderPassDependency> inputAttachments,
  Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding, UInt32 secondaryCommandBuffers)
  : DirectX12RenderPass(device, device.defaultQueue(QueueType::Graphics), renderTargets,
                        inputAttachments, inputAttachmentSamplerBinding, secondaryCommandBuffers)
{
}
...
DirectX12RenderPass::DirectX12RenderPass(
  ...
  Span<RenderPassDependency> inputAttachments,
  Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding, UInt32 secondaryCommandBuffers)
  : DirectX12RenderPass(device, name, device.defaultQueue(QueueType::Graphics), renderTargets,
                        inputAttachments, inputAttachmentSamplerBinding, secondaryCommandBuffers)
{
}
...
DirectX12RenderPass::DirectX12RenderPass(
  ...
  Span<RenderPassDependency> inputAttachments,
  Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding, UInt32 secondaryCommandBuffers)
  : m_impl(makePimpl<DirectX12RenderPassImpl>(this, device, queue, renderTargets, inputAttachments,
...
DirectX12RenderPass::DirectX12RenderPass(
  const DirectX12Device & device, const String & name_, const DirectX12Queue & queue,
  Span<RenderTarget> renderTargets, Span<RenderPassDependency> inputAttachments,
  Optional<DescriptorBindingPoint> inputAttachmentSamplerBinding, UInt32 secondaryCommandBuffers)
  : DirectX12RenderPass(device, queue, renderTargets, inputAttachments,
                        inputAttachmentSamplerBinding, secondaryCommandBuffers)
{
  if (!name_.empty())
    name() = name_;
}
...
const DirectX12Device & DirectX12RenderPass::device() const noexcept { return m_impl->m_device; }
...
SharedPtr<const DirectX12CommandBuffer> DirectX12RenderPass::commandBuffer(UInt32 index) const
{
  if (m_impl->m_activeFrameBuffer == nullptr) [[unlikely]]
    throw RuntimeException(
      "Unable to lookup command buffers on a render pass that has not been begun.");

  if (index >= m_impl->m_secondaryCommandBufferCount) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "index", 0u, m_impl->m_secondaryCommandBufferCount, index,
      "The render pass only contains {0} command buffers, but an index of {1} has been provided.",
      m_impl->m_secondaryCommandBufferCount, index);

  return m_impl->m_secondaryCommandBuffers[m_impl->m_activeFrameBuffer][index];
}

Enumerable<SharedPtr<const DirectX12CommandBuffer>> DirectX12RenderPass::commandBuffers()
  const noexcept
{
...
{
  if (auto match = std::ranges::find_if(m_impl->m_renderTargets,
                                        [&location](const RenderTarget & renderTarget)
                                        { return renderTarget.location() == location; });
      match != m_impl->m_renderTargets.end())
    return *match;

  throw InvalidArgumentException("location",
                                 "No render target is mapped to location {0} in this render pass.",
                                 location);
}
...
const RenderPassDependency & DirectX12RenderPass::inputAttachment(UInt32 location) const
{
  if (location >= m_impl->m_inputAttachments.size()) [[unlikely]]
    throw ArgumentOutOfRangeException(
      "location", 0u, static_cast<UInt32>(m_impl->m_inputAttachments.size()), location,
      "The render pass does not contain an input attachment at location {0}.", location);

  return m_impl->m_inputAttachments[location];
}

const Optional<DescriptorBindingPoint> & DirectX12RenderPass::inputAttachmentSamplerBinding()
  const noexcept
{
...
void DirectX12RenderPass::begin(const DirectX12FrameBuffer & frameBuffer) const
{
  ...
  if (m_impl->m_activeFrameBuffer != nullptr)
    throw RuntimeException(
      "Unable to begin a render pass, that is already running. End the current pass first.");
  ...
  std::ranges::for_each(
    m_impl->m_renderTargets,
    [&renderTargetBarrier, &depthStencilBarrier, &frameBuffer](const RenderTarget & renderTarget)
    {
      auto & image = frameBuffer[renderTarget];

      if (renderTarget.type() == RenderTargetType::DepthStencil)
        depthStencilBarrier.transition(image, ResourceAccess::None,
                                       ResourceAccess::DepthStencilWrite, ImageLayout::Undefined,
                                       ImageLayout::DepthWrite);
      else //if (!renderTarget.multiQueueAccess())
        renderTargetBarrier.transition(image, ResourceAccess::None, ResourceAccess::RenderTarget,
                                       ImageLayout::Undefined, ImageLayout::RenderTarget);
  ...
  std::ranges::for_each(m_impl->m_inputAttachments,
                        [&inputAttachmentBarrier,
                         &frameBuffer](const RenderPassDependency & dependency)
                        {
                          inputAttachmentBarrier.transition(frameBuffer[dependency.renderTarget()],
                                                            ResourceAccess::None,
  ...
  std::as_const(*beginCommandBuffer)
    .handle()
    ->BeginRenderPass(std::get<0>(context).size(), std::get<0>(context).data(),
                      std::get<1>(context).has_value() ? &std::get<1>(context).value() : nullptr,
                      D3D12_RENDER_PASS_FLAG_SUSPENDING_PASS);
  std::as_const(*beginCommandBuffer).handle()->EndRenderPass();
  std::ranges::for_each(m_impl->getSecondaryCommandBuffers(frameBuffer),
                        [&context](auto commandBuffer)
                        {
                          commandBuffer->begin();
                          std::as_const(*commandBuffer)
                            .handle()
                            ->BeginRenderPass(std::get<0>(context).size(),
                                              std::get<0>(context).data(),
                                              std::get<1>(context).has_value()
                                                ? &std::get<1>(context).value()
                                                : nullptr,
                                              D3D12_RENDER_PASS_FLAG_SUSPENDING_PASS |
...
UInt64 DirectX12RenderPass::end() const
{
  ...
  std::ranges::for_each(m_impl->getSecondaryCommandBuffers(frameBuffer),
                        [&context](auto commandBuffer)
                        { std::as_const(*commandBuffer).handle()->EndRenderPass(); });
  endCommandBuffer->begin();
  std::as_const(*endCommandBuffer)
    .handle()
    ->BeginRenderPass(std::get<0>(context).size(), std::get<0>(context).data(),
                      std::get<1>(context).has_value() ? &std::get<1>(context).value() : nullptr,
                      D3D12_RENDER_PASS_FLAG_RESUMING_PASS);
    ...
    [&](const RenderTarget & renderTarget)
    {
      ...
      switch (renderTarget.type())
      {
        ...
        case RenderTargetType::Color:
          renderTargetBarrier.transition(frameBuffer[renderTarget], ResourceAccess::RenderTarget,
                                         ResourceAccess::None, ImageLayout::RenderTarget,
                                         ImageLayout::ShaderResource);
          break;
        case RenderTargetType::DepthStencil:
          depthStencilBarrier.transition(frameBuffer[renderTarget],
                                         ResourceAccess::DepthStencilWrite, ResourceAccess::None,
                                         ImageLayout::DepthWrite, ImageLayout::DepthRead);
          break;
        case RenderTargetType::Present:
          if (requiresResolve)
            resolveBarrier.transition(frameBuffer[renderTarget], ResourceAccess::RenderTarget,
                                      ResourceAccess::ResolveRead, ImageLayout::RenderTarget,
                                      ImageLayout::ResolveSource);
          else
            presentBarrier.transition(frameBuffer[renderTarget], ResourceAccess::RenderTarget,
                                      ResourceAccess::TransferRead, ImageLayout::RenderTarget,
                                      ImageLayout::CopySource);
  ...
  if (requiresResolve)
  {
    resolveBarrier.transition(backBufferImage, ResourceAccess::Common, ResourceAccess::ResolveWrite,
                              ImageLayout::Common, ImageLayout::ResolveDestination);
    endCommandBuffer->barrier(resolveBarrier);

    auto & multiSampledImage = frameBuffer[*m_impl->m_presentTarget];
    std::as_const(*endCommandBuffer)
      .handle()
      ->ResolveSubresource(backBufferImage.handle().Get(), 0, multiSampledImage.handle().Get(), 0,
                           DX12::getFormat(multiSampledImage.format()));
    ...
    presentBarrier.transition(backBufferImage, ResourceAccess::ResolveWrite, ResourceAccess::Common,
                              ImageLayout::ResolveDestination, ImageLayout::Present);
    presentBarrier.transition(multiSampledImage, ResourceAccess::ResolveRead,
                              ResourceAccess::Common, ImageLayout::ResolveSource,
                              ImageLayout::Common);
    endCommandBuffer->barrier(presentBarrier);
  }
  else if (hasPresentTarget())
  {
    ...
    beginPresentBarrier.transition(backBufferImage, ResourceAccess::None,
                                   ResourceAccess::TransferWrite, ImageLayout::Undefined,
                                   ImageLayout::CopyDestination);
    ...
    endPresentBarrier.transition(backBufferImage, ResourceAccess::TransferWrite,
                                 ResourceAccess::None, ImageLayout::CopyDestination,
                                 ImageLayout::Present);
  ...
  auto fence = m_impl->m_queue->submit(
    commandBuffers | std::ranges::to<Enumerable<SharedPtr<const DirectX12CommandBuffer>>>());
...
DirectX12RenderPassBuilder::DirectX12RenderPassBuilder(const DirectX12Device & device,
...
RenderPassDependency DirectX12RenderPassBuilder::makeInputAttachment(
  DescriptorBindingPoint binding, const RenderTarget & renderTarget)
{
