...
{
  void operator()(auto * ptr) noexcept { ptr->Release(); }
};
...
/// </summary>
class DirectX12Buffer : public virtual IDirectX12Buffer,
                        public ComResource<ID3D12Resource>,
                        public virtual StateResource
{
  LITEFX_IMPLEMENTATION(DirectX12BufferImpl);

public:
  explicit DirectX12Buffer(ComPtr<ID3D12Resource> && buffer, BufferType type, UInt32 elements,
                           size_t elementSize, size_t alignment, ResourceUsage usage,
                           AllocatorPtr allocator = nullptr, AllocationPtr && allocation = nullptr,
                           const String & name = "");
...
public:
  ...
  /// <inheritdoc />
  void map(Span<void *> data, size_t elementSize, UInt32 firstElement = 0,
...
public:
  static UniquePtr<IDirectX12Buffer> allocate(AllocatorPtr allocator, BufferType type,
                                              UInt32 elements, size_t elementSize, size_t alignment,
                                              ResourceUsage usage,
                                              const D3D12_RESOURCE_DESC1 & resourceDesc,
                                              const D3D12MA::ALLOCATION_DESC & allocationDesc);
  static UniquePtr<IDirectX12Buffer> allocate(const String & name, AllocatorPtr allocator,
                                              BufferType type, UInt32 elements, size_t elementSize,
                                              size_t alignment, ResourceUsage usage,
                                              const D3D12_RESOURCE_DESC1 & resourceDesc,
                                              const D3D12MA::ALLOCATION_DESC & allocationDesc);
};
...
/// </summary>
class DirectX12VertexBuffer : public DirectX12Buffer,
                              public virtual IDirectX12VertexBuffer
{
  LITEFX_IMPLEMENTATION(DirectX12VertexBufferImpl);

public:
  explicit DirectX12VertexBuffer(ComPtr<ID3D12Resource> && buffer,
                                 const DirectX12VertexBufferLayout & layout, UInt32 elements,
...
public:
  static UniquePtr<IDirectX12VertexBuffer> allocate(
...
/// </summary>
class DirectX12IndexBuffer : public DirectX12Buffer,
                             public virtual IDirectX12IndexBuffer
{
  LITEFX_IMPLEMENTATION(DirectX12IndexBufferImpl);

public:
  explicit DirectX12IndexBuffer(ComPtr<ID3D12Resource> && buffer,
                                const DirectX12IndexBufferLayout & layout, UInt32 elements,
                                ResourceUsage usage, AllocatorPtr allocator,
                                AllocationPtr && allocation, const String & name = "");
...
public:
  static UniquePtr<IDirectX12IndexBuffer> allocate(const DirectX12IndexBufferLayout & layout,
                                                   AllocatorPtr allocator, UInt32 elements,
                                                   ResourceUsage usage,
                                                   const D3D12_RESOURCE_DESC1 & resourceDesc,
                                                   const D3D12MA::ALLOCATION_DESC & allocationDesc);
  static UniquePtr<IDirectX12IndexBuffer> allocate(const String & name,
