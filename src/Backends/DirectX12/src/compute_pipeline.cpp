...
class DirectX12ComputePipeline::DirectX12ComputePipelineImpl
  : public Implement<DirectX12ComputePipeline>
{
public:
...
public:
  ComPtr<ID3D12PipelineState> initialize()
  {
               ...
               [&, i = 0](const DirectX12ShaderModule * shaderModule) mutable
               {
                 ...
                 switch (shaderModule->type())
                 {
                   case ShaderStage::Compute:
                     pipelineStateDescription.CS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.CS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   default:
                     throw InvalidArgumentException(
                       "shaderModule", "Trying to bind shader to unsupported shader stage '{0}'.",
                       shaderModule->type());
                 }
               });
    ...
    raiseIfFailed(m_device.handle()->CreateComputePipelineState(&pipelineStateDescription,
                                                                IID_PPV_ARGS(&pipelineState)),
                  "Unable to create compute pipeline state.");
...
DirectX12ComputePipeline::DirectX12ComputePipeline(const DirectX12Device & device) noexcept
  : m_impl(makePimpl<DirectX12ComputePipelineImpl>(this, device))
  , DirectX12PipelineState(nullptr)
{
}
...
DirectX12ComputePipelineBuilder::DirectX12ComputePipelineBuilder(const DirectX12Device & device,
                                                                 const String & name)
