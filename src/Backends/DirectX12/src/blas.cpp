...
class DirectX12BottomLevelAccelerationStructure::DirectX12BottomLevelAccelerationStructureImpl
  : public Implement<DirectX12BottomLevelAccelerationStructure>
{
public:
...
public:
  DirectX12BottomLevelAccelerationStructureImpl(DirectX12BottomLevelAccelerationStructure * parent,
...
public:
    ...
    return [this]() -> std::generator<D3D12_RAYTRACING_GEOMETRY_DESC>
    {
      ...
      for (auto & mesh : m_triangleMeshes)
      {
        ...
        co_yield D3D12_RAYTRACING_GEOMETRY_DESC{
          ...
          .Triangles = D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC{
            .Transform3x4 =
              mesh.TransformBuffer == nullptr ? 0 : mesh.TransformBuffer->virtualAddress(),
            .IndexFormat = mesh.IndexBuffer == nullptr
                           ? DXGI_FORMAT_UNKNOWN
            ...
            .VertexBuffer = {mesh.VertexBuffer->virtualAddress(),
      ...
      for (auto & bb : m_boundingBoxes)
      {
        ...
        co_yield D3D12_RAYTRACING_GEOMETRY_DESC{
          ...
          .AABBs =
            D3D12_RAYTRACING_GEOMETRY_AABBS_DESC{.AABBCount = bb.Buffer->elements(),
                                                 .AABBs = {bb.Buffer->virtualAddress(),
                                                           bb.Buffer->alignedElementSize()}}};
      }
    }() | std::ranges::to<Array<D3D12_RAYTRACING_GEOMETRY_DESC>>();
  }

  inline void queuePostbuildInfoCommands(const DirectX12CommandBuffer & commandBuffer,
                                         bool afterCopy = false)
  {
    ...
    {
      m_postBuildBuffer =
        device.factory().createBuffer(BufferType::Storage, ResourceHeap::Resource, sizeof(UInt64),
                                      1, ResourceUsage::TransferSource | ResourceUsage::AllowWrite);

      auto barrier =
        device.makeBarrier(PipelineStage::None, PipelineStage::AccelerationStructureCopy);
      barrier->transition(*m_postBuildBuffer, ResourceAccess::None,
                          ResourceAccess::ShaderReadWrite);
      commandBuffer.barrier(*barrier);
    }

    if (m_postBuildResults == nullptr) [[unlikely]]
      m_postBuildResults = device.factory().createBuffer(BufferType::Storage,
                                                         ResourceHeap::Readback, sizeof(UInt64), 1,
    ...
    CD3DX12_BUFFER_BARRIER preBarrier[1] = {
      CD3DX12_BUFFER_BARRIER(
        afterCopy ? D3D12_BARRIER_SYNC_COPY_RAYTRACING_ACCELERATION_STRUCTURE
                  : D3D12_BARRIER_SYNC_BUILD_RAYTRACING_ACCELERATION_STRUCTURE,
        D3D12_BARRIER_SYNC_EMIT_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO,
        D3D12_BARRIER_ACCESS_RAYTRACING_ACCELERATION_STRUCTURE_WRITE,
        D3D12_BARRIER_ACCESS_RAYTRACING_ACCELERATION_STRUCTURE_READ,
        std::as_const(*m_buffer).handle().Get()),
    ...
    commandBuffer.handle()->EmitRaytracingAccelerationStructurePostbuildInfo(&postBuildInfo, 1,
                                                                             &address);

    // Transition the buffer into copy state.
    auto postBarrier = CD3DX12_BUFFER_BARRIER(
      D3D12_BARRIER_SYNC_EMIT_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO,
      D3D12_BARRIER_SYNC_COPY, D3D12_BARRIER_ACCESS_UNORDERED_ACCESS,
      D3D12_BARRIER_ACCESS_COPY_SOURCE, std::as_const(*m_postBuildBuffer).handle().Get());
...
DirectX12BottomLevelAccelerationStructure::DirectX12BottomLevelAccelerationStructure(
...
void DirectX12BottomLevelAccelerationStructure::build(
  const DirectX12CommandBuffer & commandBuffer, SharedPtr<const IDirectX12Buffer> scratchBuffer,
  SharedPtr<const IDirectX12Buffer> buffer, UInt64 offset, UInt64 maxSize)
{
  ...
  if ((offset % D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT) != 0) [[unlikely]]
    throw InvalidArgumentException("offset", "The offset must be aligned to {0} bytes",
...
void DirectX12BottomLevelAccelerationStructure::update(
  const DirectX12CommandBuffer & commandBuffer, SharedPtr<const IDirectX12Buffer> scratchBuffer,
  SharedPtr<const IDirectX12Buffer> buffer, UInt64 offset, UInt64 maxSize)
{
  // Validate the state.
...
void DirectX12BottomLevelAccelerationStructure::copy(
  const DirectX12CommandBuffer & commandBuffer,
  DirectX12BottomLevelAccelerationStructure & destination, bool compress,
  SharedPtr<const IDirectX12Buffer> buffer, UInt64 offset, bool copyBuildInfo) const
{
  // Validate the state.
  if (m_impl->m_buffer == nullptr) [[unlikely]]
    throw RuntimeException(
  ...
  if (buffer == nullptr)
    buffer = destination.m_impl->m_buffer->size() >= requiredMemory
...
const Array<TriangleMesh> & DirectX12BottomLevelAccelerationStructure::triangleMeshes()
  const noexcept
{
...
const Array<BoundingBoxes> & DirectX12BottomLevelAccelerationStructure::boundingBoxes()
  const noexcept
{
...
void DirectX12BottomLevelAccelerationStructure::doBuild(const ICommandBuffer & commandBuffer,
...
void DirectX12BottomLevelAccelerationStructure::doUpdate(const ICommandBuffer & commandBuffer,
                                                         SharedPtr<const IBuffer> scratchBuffer,
                                                         SharedPtr<const IBuffer> buffer,
                                                         UInt64 offset, UInt64 maxSize)
{
...
void DirectX12BottomLevelAccelerationStructure::doCopy(
  const ICommandBuffer & commandBuffer, IBottomLevelAccelerationStructure & destination,
  bool compress, SharedPtr<const IBuffer> buffer, UInt64 offset, bool copyBuildInfo) const
{
  copy(dynamic_cast<const DirectX12CommandBuffer &>(commandBuffer),
             dynamic_cast<DirectX12BottomLevelAccelerationStructure &>(destination), compress,
