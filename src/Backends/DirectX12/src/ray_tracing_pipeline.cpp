...
class DirectX12RayTracingPipeline::DirectX12RayTracingPipelineImpl
  : public Implement<DirectX12RayTracingPipeline>
{
public:
...
public:
  DirectX12RayTracingPipelineImpl(DirectX12RayTracingPipeline * parent,
                                  ...
                                  UInt32 maxAttributeSize, ShaderRecordCollection && shaderRecords)
    : base(parent)
    , m_device(device)
    , m_layout(layout)
    , m_program(shaderProgram)
    , m_shaderRecordCollection(std::move(shaderRecords))
    ...
    , m_maxAttributeSize(maxAttributeSize)
  {
    if (maxRecursionDepth > D3D12_RAYTRACING_MAX_DECLARABLE_TRACE_RECURSION_DEPTH) [[unlikely]]
      throw ArgumentOutOfRangeException("maxRecursionDepth", 0u,
                                        static_cast<UInt32>(
                                          D3D12_RAYTRACING_MAX_DECLARABLE_TRACE_RECURSION_DEPTH),
                                        maxRecursionDepth,
                                        "The specified ray tracing recursion depth too large.");

    if (maxAttributeSize > D3D12_RAYTRACING_MAX_ATTRIBUTE_SIZE_IN_BYTES) [[unlikely]]
      throw ArgumentOutOfRangeException("maxAttributeSize", 0u,
                                        static_cast<UInt32>(
                                          D3D12_RAYTRACING_MAX_ATTRIBUTE_SIZE_IN_BYTES),
                                        maxAttributeSize,
                                        "The specified ray attribute size was too large.");
  }
  ...
  {
    m_program =
      std::dynamic_pointer_cast<const DirectX12ShaderProgram>(m_shaderRecordCollection.program());
  }
...
public:
  void initialize()
  {
    if (m_program == nullptr) [[unlikely]]
      throw ArgumentNotInitializedException("shaderProgram",
                                            "The shader program must be initialized.");
    ...
    if (m_program != m_shaderRecordCollection.program()) [[unlikely]]
      throw InvalidArgumentException(
        "shaderRecords",
        "The ray tracing pipeline shader program must be the same as used to build the shader record collection.");

    LITEFX_TRACE(DIRECTX12_LOG,
                 "Creating ray-tracing pipeline (\"{1}\") for layout {0} (records: {2})...",
                 reinterpret_cast<void *>(m_layout.get()), m_parent->name(),
                 m_shaderRecordCollection.shaderRecords().size());

    // Validate shader stage usage.
    auto modules = m_program->modules();
    bool hasComputeShaders =
      std::ranges::find_if(modules, [](const auto & module)
                           { return LITEFX_FLAG_IS_SET(ShaderStage::Compute, module->type()); }) !=
      modules.end();
    bool hasRayTracingShaders =
      std::ranges::find_if(modules,
                           [](const auto & module) {
                             return LITEFX_FLAG_IS_SET(ShaderStage::RayTracingPipeline,
                                                       module->type());
                           }) != modules.end();
    bool hasMeshShaders =
      std::ranges::find_if(modules,
                           [](const auto & module) {
                             return LITEFX_FLAG_IS_SET(ShaderStage::MeshPipeline, module->type());
                           }) != modules.end();
    bool hasDirectShaders =
      std::ranges::find_if(modules,
                           [](const auto & module) {
                             return LITEFX_FLAG_IS_SET(ShaderStage::RasterizationPipeline,
                                                       module->type());
                           }) != modules.end();
    ...
    if (hasComputeShaders) [[unlikely]]
      throw InvalidArgumentException(
        "shaderProgram",
        "The shader program contains a compute shader, which is not supported in a ray-tracing pipeline.");
    else if (hasDirectShaders) [[unlikely]]
      throw InvalidArgumentException(
        "shaderProgram",
        "The shader program contains a graphics shader, which is not supported in a ray-tracing pipeline");
    else if (hasMeshShaders) [[unlikely]]
      throw InvalidArgumentException(
        "shaderProgram",
        "The shader program contains a mesh shader, which is not supported in a ray-tracing pipeline");

    LITEFX_TRACE(DIRECTX12_LOG, "Using shader program {0} with {1} modules...",
                 reinterpret_cast<const void *>(m_program.get()), modules.size());
    ...
    auto shaderModuleSubobjects =
      m_program->modules() |
      std::views::transform(
        [](auto module)
        {
          ShaderModuleSubobjectData data = {.Name = Widen(module->fileName()),
                                            .EntryPoint = Widen(module->entryPoint()),
                                            .Type = module->type(),
                                            .Module = module};

          data.LibraryDesc = {
            .DXILLibrary = {.pShaderBytecode = module->handle()->GetBufferPointer(),
    ...
    auto subobjects =
      ...
      std::views::transform(
        [](auto & subobjectData)
        {
          ...
          return D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY,
                                       .pDesc = &subobjectData.LibraryDesc};
        }) |
    ...
    auto hitGroupSubobjects =
      m_shaderRecordCollection.shaderRecords() |
      std::views::filter(
        [](const UniquePtr<const IShaderRecord> & record)
        {
          return record->type() == ShaderRecordType::HitGroup ||
                 record->type() == ShaderRecordType::Intersection;
        }) |
      std::views::transform(
        [i = 0](const UniquePtr<const IShaderRecord> & record) mutable
        {
          ...
          if (record->type() == ShaderRecordType::Intersection)
          {
            ...
            hitGroup.HitGroupDesc =
              D3D12_HIT_GROUP_DESC{.Type = D3D12_HIT_GROUP_TYPE_PROCEDURAL_PRIMITIVE};
    ...
    subobjects.append_range(
      ...
      std::views::transform(
        [](auto & subobjectData)
        {
          ...
          subobjectData.HitGroupDesc.IntersectionShaderImport =
            subobjectData.IntersectionShaderName.empty()
              ? nullptr
              : subobjectData.IntersectionShaderName.c_str();
          subobjectData.HitGroupDesc.ClosestHitShaderImport =
            subobjectData.ClosestHitShaderName.empty() ? nullptr
                                                       : subobjectData.ClosestHitShaderName.c_str();
          subobjectData.HitGroupDesc.AnyHitShaderImport = subobjectData.AnyHitShaderName.empty()
                                                          ? nullptr
                                                          : subobjectData.AnyHitShaderName.c_str();

          return D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP,
    ...
    std::ranges::for_each(
      ...
      [&](auto & subobject)
      {
        ...
        LocalDescriptorBindingPoint binding = {.BindingPoint =
                                                 subobject.Module->shaderLocalDescriptor().value()};
        ...
        if (!descriptor.local()) [[unlikely]]
          throw RuntimeException(
            "The descriptor at a shader-local binding point must also be declared local, since it will be part of the global root signature otherwise.");
        ...
        switch (descriptor.descriptorType())
        {
          ...
          case DescriptorType::Sampler:
            throw RuntimeException("Shader-local samplers are not supported.");
          case DescriptorType::InputAttachment:
            throw RuntimeException("Shader-local input attachments are not supported.");
        }
        ...
        if (!rootSignatures.contains(binding))
        {
          ...
          switch (binding.Type)
          {
            case D3D12_DESCRIPTOR_RANGE_TYPE_CBV:
              ...
              rootParameter.InitAsConstants(descriptor.elementSize() / 4,
                                            binding.BindingPoint.Register,
                                            binding.BindingPoint.Space);
              break;
            case D3D12_DESCRIPTOR_RANGE_TYPE_SRV:
              // NOTE: SRVs and UAVs must be passed as GPU-virtual addresses to the shader-local data.
              rootParameter.InitAsShaderResourceView(binding.BindingPoint.Register,
                                                     binding.BindingPoint.Space);
              break;
            case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
              // NOTE: SRVs and UAVs must be passed as GPU-virtual addresses to the shader-local data.
              rootParameter.InitAsUnorderedAccessView(binding.BindingPoint.Register,
                                                      binding.BindingPoint.Space);
              break;
          ...
          rootSignatureDesc.Init_1_1(1, &rootParameter, 0, nullptr,
          ...
          if (error != nullptr)
            errorString =
              String(reinterpret_cast<TCHAR *>(error->GetBufferPointer()), error->GetBufferSize());

          raiseIfFailed(hr, "Unable to serialize shader-local root signature: {0}", errorString);

          // Create the root signature.
          ComPtr<ID3D12RootSignature> rootSignature;
          raiseIfFailed(m_device.handle()->CreateRootSignature(0, signature->GetBufferPointer(),
                                                               signature->GetBufferSize(),
                                                               IID_PPV_ARGS(&rootSignature)),
                        "Unable to create root signature for shader-local payload.");

          // Add the root signature to the binding associations set.
          rootSignatures[binding].RootSignature = rootSignature;
          rootSignatures[binding].StateDesc = {.pLocalRootSignature = rootSignature.Get()};
        }
        ...
        rootSignatures[binding].AssocDesc.NumExports =
          static_cast<UInt32>(rootSignatures[binding].ModuleNames.size());
    ...
    subobjects.reserve(
      subobjects.size() + (rootSignatures.size() * 2) +
      4); // 4 subobjects for shader + pipeline config, global root signature and shader associations.

    // Define local root signature associations.
    for (auto & rootSignature : rootSignatures | std::views::values)
    {
      subobjects.push_back(
        D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_LOCAL_ROOT_SIGNATURE,
                              .pDesc = &rootSignature.StateDesc});

      rootSignature.AssocDesc.pSubobjectToAssociate = &subobjects.back();

      subobjects.push_back(
        D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION,
    ...
    D3D12_RAYTRACING_SHADER_CONFIG shaderConfig = {.MaxPayloadSizeInBytes = m_maxPayloadSize,
                                                   .MaxAttributeSizeInBytes = m_maxAttributeSize};

    subobjects.push_back(
      D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG,
                            .pDesc = &shaderConfig});
    ...
    //       more involved.
    auto shaderGroupNames =
      shaderModuleSubobjects |
      std::views::filter(
        [](auto & moduleDesc)
        {
          return LITEFX_FLAG_IS_SET(ShaderStage::RayGeneration | ShaderStage::Callable |
                                      ShaderStage::Miss,
                                    moduleDesc.Type);
        }) |
      std::views::transform([](auto & moduleDesc) { return moduleDesc.Name.c_str(); }) |
      std::ranges::to<Array<LPCWSTR>>();

    shaderGroupNames.append_range(
      hitGroupSubobjects |
      std::views::transform([](auto & hitGroup) { return hitGroup.Name.c_str(); }));
    ...
    D3D12_RAYTRACING_PIPELINE_CONFIG pipelineConfig = {.MaxTraceRecursionDepth =
                                                         m_maxRecursionDepth};

    subobjects.push_back(
      D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_PIPELINE_CONFIG,
                            .pDesc = &pipelineConfig});

    // Finally, add the global root signature.
    D3D12_GLOBAL_ROOT_SIGNATURE globalRootSignature{.pGlobalRootSignature =
                                                      std::as_const(*m_layout).handle().Get()};

    subobjects.push_back(
      D3D12_STATE_SUBOBJECT{.Type = D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE,
                            .pDesc = &globalRootSignature});

    // Define pipeline description from sub-objects.
    D3D12_STATE_OBJECT_DESC pipelineDesc = {.Type = D3D12_STATE_OBJECT_TYPE_RAYTRACING_PIPELINE,
                                            .NumSubobjects = static_cast<UInt32>(subobjects.size()),
                                            .pSubobjects = subobjects.data()};

    // Create the pipeline.
    ComPtr<ID3D12StateObject> pipeline;
    raiseIfFailed(m_device.handle()->CreateStateObject(&pipelineDesc, IID_PPV_ARGS(&pipeline)),
                  "Unable to create ray tracing pipeline state.");
  ...
  UniquePtr<IDirectX12Buffer> allocateShaderBindingTable(ShaderBindingTableOffsets & offsets,
                                                         ShaderBindingGroup groups)
  {
    ...
    raiseIfFailed(m_pipelineState.As(&stateProperties),
                  "Unable to query ray tracing pipeline state properties.");
    ...
    auto filterByGroupType = [groups](auto & record) -> bool
    {
      switch (record->type())
      {
        case ShaderRecordType::RayGeneration:
          return LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::RayGeneration);
        case ShaderRecordType::Miss: return LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::Miss);
        case ShaderRecordType::Callable:
          return LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::Callable);
        case ShaderRecordType::Intersection:
        case ShaderRecordType::HitGroup:
          return LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::HitGroup);
        default: std::unreachable(); // Must be caught during pipeline creation!
      }
    };

    auto localDataSize = std::ranges::max(
      m_shaderRecordCollection.shaderRecords() | std::views::filter(filterByGroupType) |
      std::views::transform([](auto & record) { return record->localDataSize(); }));

    // Compute the record size by aligning the handle and payload sizes.
    auto recordSize = Math::align<UInt64>(D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES + localDataSize,
    ...
    alignmentRecords[ShaderBindingGroup::RayGeneration] =
      LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::RayGeneration)
        ? (recordSize *
           std::ranges::count_if(m_shaderRecordCollection.shaderRecords(),
                                 [](auto & record) {
                                   return LITEFX_FLAG_IS_SET(record->type(),
                                                             ShaderRecordType::RayGeneration);
                                 }) %
           D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT) /
            D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT
        : 0u;
    alignmentRecords[ShaderBindingGroup::Miss] =
      LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::Miss)
        ? (recordSize *
           std::ranges::count_if(m_shaderRecordCollection.shaderRecords(),
                                 [](auto & record) {
                                   return LITEFX_FLAG_IS_SET(record->type(),
                                                             ShaderRecordType::Miss);
                                 }) %
           D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT) /
            D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT
        : 0u;
    alignmentRecords[ShaderBindingGroup::Callable] =
      LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::Callable)
        ? (recordSize *
           std::ranges::count_if(m_shaderRecordCollection.shaderRecords(),
                                 [](auto & record) {
                                   return LITEFX_FLAG_IS_SET(record->type(),
                                                             ShaderRecordType::Callable);
                                 }) %
           D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT) /
            D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT
        : 0u;
    alignmentRecords[ShaderBindingGroup::HitGroup] =
      LITEFX_FLAG_IS_SET(groups, ShaderBindingGroup::HitGroup)
        ? (recordSize *
           std::ranges::count_if(m_shaderRecordCollection.shaderRecords(),
                                 [](auto & record)
                                 {
                                   return LITEFX_FLAG_IS_SET(record->type(),
                                                             ShaderRecordType::HitGroup) ||
                                          LITEFX_FLAG_IS_SET(record->type(),
                                                             ShaderRecordType::Intersection);
                                 }) %
           D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT) /
            D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT
        : 0u;
    ...
    auto totalRecordCount = std::ranges::distance(m_shaderRecordCollection.shaderRecords() |
                                                  std::views::filter(filterByGroupType)) +
                            alignmentRecords[ShaderBindingGroup::RayGeneration] +
                            alignmentRecords[ShaderBindingGroup::Miss] +
                            alignmentRecords[ShaderBindingGroup::Callable] +
                            alignmentRecords[ShaderBindingGroup::HitGroup];
    ...
    auto result = m_device.factory().createBuffer(BufferType::ShaderBindingTable,
                                                  ResourceHeap::Dynamic, recordSize,
                                                  totalRecordCount, ResourceUsage::TransferSource);
    ...
    for (auto group : {ShaderBindingGroup::RayGeneration, ShaderBindingGroup::Miss,
                       ShaderBindingGroup::Callable, ShaderBindingGroup::HitGroup})
    {
      auto groupFilter = [group](auto & record) -> bool
      {
        switch (group)
        {
          case ShaderBindingGroup::RayGeneration:
            return record->type() == ShaderRecordType::RayGeneration;
      ...
      auto getRecordIdentifier = [&stateProperties, group,
                                  hitGroupIndex = 0](auto & record) mutable -> const void *
      {
        switch (group)
        {
          case ShaderBindingGroup::RayGeneration:
          ...
          {
            WString identifier =
              Widen(std::get<const IShaderModule *>(record->shaderGroup())->fileName());
      ...
      if (LITEFX_FLAG_IS_SET(groups, group))
      {
        ...
        auto filteredRecords = m_shaderRecordCollection.shaderRecords() |
                               std::views::filter(groupFilter);
        ...
        for (auto & currentRecord : filteredRecords)
        {
          ...
          std::memcpy(recordData.data(), getRecordIdentifier(currentRecord),
                      D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

          // Write the payload and map everything into the buffer.
          std::memcpy(recordData.data() + D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES,
                      currentRecord->localData(), currentRecord->localDataSize());
...
DirectX12RayTracingPipeline::DirectX12RayTracingPipeline(
  const DirectX12Device & device, ShaderRecordCollection && shaderRecords) noexcept
  : m_impl(makePimpl<DirectX12RayTracingPipelineImpl>(this, device, std::move(shaderRecords)))
  , DirectX12PipelineState(nullptr)
{
}
...
UniquePtr<IDirectX12Buffer> DirectX12RayTracingPipeline::allocateShaderBindingTable(
  ShaderBindingTableOffsets & offsets, ShaderBindingGroup groups) const noexcept
{
...
DirectX12RayTracingPipelineBuilder::DirectX12RayTracingPipelineBuilder(
  const DirectX12Device & device, ShaderRecordCollection && shaderRecords, const String & name)
