...
class DirectX12RenderPipeline::DirectX12RenderPipelineImpl
  : public Implement<DirectX12RenderPipeline>
{
public:
...
private:
  ...
  Dictionary<const IFrameBuffer *, Array<UniquePtr<DirectX12DescriptorSet>>>
    m_inputAttachmentBindings;
...
public:
  DirectX12RenderPipelineImpl(DirectX12RenderPipeline * parent,
                              const DirectX12RenderPass & renderPass, bool alphaToCoverage,
    ...
    , m_inputAssembler(inputAssembler)
...
public:
  ...
  {
    LITEFX_TRACE(DIRECTX12_LOG, "Creating render pipeline \"{1}\" for layout {0}...",
                 reinterpret_cast<void *>(m_layout.get()), m_parent->name());
    m_samples = samples;
    ...
    bool hasMeshShaders =
      std::ranges::find_if(modules,
                           [](const auto & module) {
                             return LITEFX_FLAG_IS_SET(ShaderStage::Mesh | ShaderStage::Task,
                                                       module->type());
                           }) != modules.end();
    ...
    LITEFX_TRACE(
      DIRECTX12_LOG,
      "Rasterizer state: {{ PolygonMode: {0}, CullMode: {1}, CullOrder: {2}, LineWidth: {3} }}",
      rasterizer.polygonMode(), rasterizer.cullMode(), rasterizer.cullOrder(),
    ...
    else
    {
      LITEFX_TRACE(
        DIRECTX12_LOG,
        "\tRasterizer depth bias: {{ Clamp: {0}, ConstantFactor: {1}, SlopeFactor: {2} }}",
      ...
      rasterizerState.DepthBias =
        static_cast<Int32>(rasterizer.depthStencilState().depthBias().ConstantFactor);
    ...
    LITEFX_TRACE(DIRECTX12_LOG, "Input assembler state: {{ PrimitiveTopology: {0} }}",
                 m_inputAssembler->topology());
    D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType =
      DX12::getPrimitiveTopologyType(m_inputAssembler->topology());

    auto vertexLayouts = m_inputAssembler->vertexBufferLayouts();

    std::ranges::for_each(
      m_inputAssembler->vertexBufferLayouts(),
      [&, l = 0](const DirectX12VertexBufferLayout * layout) mutable
      {
        ...
        LITEFX_TRACE(
          DIRECTX12_LOG,
          "Defining vertex buffer layout {0}/{1} {{ Attributes: {2}, Size: {3} bytes, Binding: {4} }}...",
          ++l, vertexLayouts.size(), bufferAttributes.size(), layout->elementSize(), bindingPoint);
        ...
        std::ranges::for_each(
          ...
          [&](const BufferAttribute * attribute)
          {
            ...
            elementDescriptor.InputSlotClass =
              D3D12_INPUT_CLASSIFICATION::D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA;
            elementDescriptor.AlignedByteOffset =
              attribute
                ->offset(); // TODO: May not include packing, but packing is required - need to test this!
    ...
    DXGI_SAMPLE_DESC multisamplingState =
      samples == MultiSamplingLevel::x1
        ? DXGI_SAMPLE_DESC{1, 0}
        : DXGI_SAMPLE_DESC{std::to_underlying(samples), DXGI_STANDARD_MULTISAMPLE_QUALITY_PATTERN};
    ...
    UInt32 renderTargets =
      std::ranges::count_if(targets, [](auto & renderTarget)
                            { return renderTarget.type() != RenderTargetType::DepthStencil; });
    ...
    if (renderTargets > D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT) [[unlikely]]
      throw RuntimeException(
        "You have specified too many render targets: only 8 render targets and 1 depth/stencil target are allowed, but {0} have been specified.",
        renderTargets);

    // Only one DSV is allowed.
    if (depthStencilTargets > 1) [[unlikely]]
      throw RuntimeException(
        "You have specified too many render targets: only 1 depth/stencil target is allowed, but {0} have been specified.",
        depthStencilTargets);

    std::ranges::
      for_each(targets,
               [&, i = 0](auto & renderTarget) mutable
               {
                 if (renderTarget.type() == RenderTargetType::DepthStencil)
                 {
                   ...
                   depthStencilState.DepthEnable =
                     rasterizer.depthStencilState().depthState().Enable;
                   depthStencilState.DepthWriteMask =
                     rasterizer.depthStencilState().depthState().Write
                   ...
                   depthStencilState.DepthFunc =
                     DX12::getCompareOp(rasterizer.depthStencilState().depthState().Operation);

                   depthStencilState.StencilEnable =
                     rasterizer.depthStencilState().stencilState().Enable;
                   depthStencilState.StencilReadMask =
                     rasterizer.depthStencilState().stencilState().ReadMask;
                   depthStencilState.StencilWriteMask =
                     rasterizer.depthStencilState().stencilState().WriteMask;
                   depthStencilState.FrontFace.StencilFunc = DX12::getCompareOp(
                     rasterizer.depthStencilState().stencilState().FrontFace.Operation);
                   depthStencilState.FrontFace.StencilDepthFailOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().FrontFace.DepthFailOp);
                   depthStencilState.FrontFace.StencilFailOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().FrontFace.StencilFailOp);
                   depthStencilState.FrontFace.StencilPassOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().FrontFace.StencilPassOp);
                   depthStencilState.BackFace.StencilFunc = DX12::getCompareOp(
                     rasterizer.depthStencilState().stencilState().BackFace.Operation);
                   depthStencilState.BackFace.StencilDepthFailOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().BackFace.DepthFailOp);
                   depthStencilState.BackFace.StencilFailOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().BackFace.StencilFailOp);
                   depthStencilState.BackFace.StencilPassOp = DX12::getStencilOp(
                     rasterizer.depthStencilState().stencilState().BackFace.StencilPassOp);
                 }
                 else
                 {
                   ...
                   targetBlendState.RenderTargetWriteMask =
                     static_cast<D3D12_COLOR_WRITE_ENABLE>(renderTarget.blendState().WriteMask);
                   targetBlendState.SrcBlend =
                     DX12::getBlendFactor(renderTarget.blendState().SourceColor);
                   targetBlendState.SrcBlendAlpha =
                     DX12::getBlendFactor(renderTarget.blendState().SourceAlpha);
                   targetBlendState.DestBlend =
                     DX12::getBlendFactor(renderTarget.blendState().DestinationColor);
                   targetBlendState.DestBlendAlpha =
                     DX12::getBlendFactor(renderTarget.blendState().DestinationAlpha);
                   targetBlendState.BlendOp =
                     DX12::getBlendOperation(renderTarget.blendState().ColorOperation);
                   targetBlendState.BlendOpAlpha =
                     DX12::getBlendOperation(renderTarget.blendState().AlphaOperation);
    ...
    if (hasMeshShaders)
      return initializeMeshPipeline(blendState, rasterizerState, depthStencilState, topologyType,
                                    renderTargets, rtvFormats, dsvFormat, multisamplingState);
    else
      return initializeGraphicsPipeline(blendState, rasterizerState, depthStencilState, inputLayout,
                                        topologyType, renderTargets, rtvFormats, dsvFormat,
                                        multisamplingState);
  }

  ComPtr<ID3D12PipelineState> initializeMeshPipeline(
    const D3D12_BLEND_DESC & blendState, const D3D12_RASTERIZER_DESC & rasterizerState,
    const D3D12_DEPTH_STENCIL_DESC & depthStencilState, D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType,
    UINT renderTargets,
    const std::array<DXGI_FORMAT, D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT> & renderTargetFormats,
    DXGI_FORMAT depthStencilFormat, const DXGI_SAMPLE_DESC & multisamplingState)
  {
    ...
    std::memcpy(&pipelineStateDescription.RTVFormats, renderTargetFormats.data(),
                sizeof(DXGI_FORMAT) * D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT);

    // Setup shader stages.
    auto modules = m_program->modules();
    LITEFX_TRACE(DIRECTX12_LOG, "Using shader program {0} with {1} modules...",
                 reinterpret_cast<void *>(m_program.get()), modules.size());

    std::ranges::
      for_each(modules,
               [&, i = 0](const DirectX12ShaderModule * shaderModule) mutable
               {
                 LITEFX_TRACE(DIRECTX12_LOG,
                              "\tModule {0}/{1} (\"{2}\") state: {{ Type: {3}, EntryPoint: {4} }}",
                              ++i, modules.size(), shaderModule->fileName(), shaderModule->type(),
                              shaderModule->entryPoint());

                 switch (shaderModule->type())
                 {
                   case ShaderStage::Fragment:
                     pipelineStateDescription.PS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.PS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::Task:
                     pipelineStateDescription.AS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.AS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::Mesh:
                     pipelineStateDescription.MS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.MS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   default:
                     throw InvalidArgumentException(
                       "shaderProgram", "Trying to bind shader to unsupported shader stage '{0}'.",
                       shaderModule->type());
                 }
               });

    CD3DX12_PIPELINE_STATE_STREAM2 streamDesc(pipelineStateDescription);
    D3D12_PIPELINE_STATE_STREAM_DESC pipelineDesc = {.SizeInBytes = sizeof(streamDesc),
                                                     .pPipelineStateSubobjectStream = &streamDesc};

    // Create the pipeline state instance.
    ComPtr<ID3D12PipelineState> pipelineState;
    raiseIfFailed(m_renderPass.device().handle()->CreatePipelineState(&pipelineDesc,
  ...
  ComPtr<ID3D12PipelineState> initializeGraphicsPipeline(
    const D3D12_BLEND_DESC & blendState, const D3D12_RASTERIZER_DESC & rasterizerState,
    const D3D12_DEPTH_STENCIL_DESC & depthStencilState, const D3D12_INPUT_LAYOUT_DESC & inputLayout,
    D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType, UINT renderTargets,
    const std::array<DXGI_FORMAT, D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT> & renderTargetFormats,
    DXGI_FORMAT depthStencilFormat, const DXGI_SAMPLE_DESC & multisamplingState)
  {
    // Create a pipeline state description.
               ...
               [&, i = 0](const DirectX12ShaderModule * shaderModule) mutable
               {
                 ...
                 switch (shaderModule->type())
                 {
                   case ShaderStage::Vertex:
                     pipelineStateDescription.VS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.VS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::TessellationControl: // aka. Hull Shader
                     pipelineStateDescription.HS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.HS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::TessellationEvaluation: // aka. Domain Shader
                     pipelineStateDescription.DS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.DS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::Geometry:
                     pipelineStateDescription.GS.pShaderBytecode =
                       shaderModule->handle()->GetBufferPointer();
                     pipelineStateDescription.GS.BytecodeLength =
                       shaderModule->handle()->GetBufferSize();
                     break;
                   case ShaderStage::Fragment:
                     pipelineStateDescription.PS.pShaderBytecode =
    ...
    // Create the pipeline state instance.
    ComPtr<ID3D12PipelineState> pipelineState;
    raiseIfFailed(m_renderPass.device()
                    .handle()
                    ->CreateGraphicsPipelineState(&pipelineStateDescription,
                                                  IID_PPV_ARGS(&pipelineState)),
                  "Unable to create render pipeline state.");
  ...
  void initializeInputAttachmentBindings(const DirectX12FrameBuffer & frameBuffer)
  {
    ...
    std::ranges::for_each(m_renderPass.inputAttachments(),
                          [&descriptorsPerSet](auto & dependency) {
                            descriptorsPerSet[dependency.binding().Space].push_back(
                              dependency.binding().Register);
                          });
    ...
    for (auto & [set, descriptors] : descriptorsPerSet)
    {
      ...
      if (std::ranges::adjacent_find(descriptors) != descriptors.end()) [[unlikely]]
        throw RuntimeException(
          "The descriptor set {0} has input attachment mappings that point to the same descriptor.",
          set);
      ...
      for (auto & descriptor : layout.descriptors())
      {
        if (std::ranges::find(descriptors, descriptor->binding()) == descriptors.end()) [[unlikely]]
        {
          LITEFX_WARNING(
            DIRECTX12_LOG,
            "The descriptor set {0} is not fully mapped by the provided input attachments for the render pass.",
            set);
          break;
        }
      }
    ...
    if (m_renderPass.inputAttachmentSamplerBinding().has_value())
    {
      ...
      if (auto samplerSet = std::ranges::find_if(layouts, [&](auto set)
                                                 { return set->space() == samplerBinding.Space; });
          samplerSet != layouts.end())
      {
        if (descriptorsPerSet.contains(samplerBinding.Space)) [[unlikely]]
          throw RuntimeException(
            "The input attachment sampler is defined in a descriptor set that contains input attachment descriptors. Samplers must be defined within their own space.");
    ...
    allocateInputAttachmentBindings(frameBuffer, descriptorsPerSet | std::views::keys);
  }

  void allocateInputAttachmentBindings(const DirectX12FrameBuffer & frameBuffer,
                                       const std::ranges::input_range auto & descriptorSets)
    requires std::is_convertible_v<std::ranges::range_value_t<decltype(descriptorSets)>, UInt32>
  {
    ...
    bindings.append_range(
      descriptorSets |
      std::views::transform([this](UInt32 set)
                            { return std::move(m_layout->descriptorSet(set).allocate()); }));

    // Listen to frame buffer events and update the bindings or remove the sets (on release).
    m_frameBufferResizeTokens[interfacePointer] =
      frameBuffer.resized.add(std::bind(&DirectX12RenderPipelineImpl::onFrameBufferResize, this,
                                        std::placeholders::_1, std::placeholders::_2));
    m_frameBufferReleaseTokens[interfacePointer] =
      frameBuffer.released.add(std::bind(&DirectX12RenderPipelineImpl::onFrameBufferRelease, this,
                                         std::placeholders::_1, std::placeholders::_2));
  }
  ...
  void updateInputAttachmentBindings(const DirectX12FrameBuffer & frameBuffer)
  {
        ...
        for (auto & binding : bindings)
        {
          if (binding->layout().space() == dependency.binding().Space)
          {
            ...
            if (image.samples() != m_samples) [[unlikely]]
              LITEFX_WARNING(
                DIRECTX12_LOG,
                "The image multi sampling level {0} does not match the render pipeline multi sampling state {1}.",
    ...
    if (m_renderPass.inputAttachmentSamplerBinding().has_value())
    {
      for (auto & binding : bindings)
      {
        ...
        {
          binding->update(m_renderPass.inputAttachmentSamplerBinding().value().Register,
                          *m_inputAttachmentSampler);
          break;
        }
      }
    }
  ...
  void bindInputAttachments(const DirectX12CommandBuffer & commandBuffer)
  {
    ...
    commandBuffer.bind(m_inputAttachmentBindings.at(interfacePointer) |
                       std::views::transform([](auto & set) { return set.get(); }));
  }
...
DirectX12RenderPipeline::DirectX12RenderPipeline(const DirectX12RenderPass & renderPass,
                                                 ...
                                                 bool enableAlphaToCoverage, const String & name_)
  : m_impl(makePimpl<DirectX12RenderPipelineImpl>(this, renderPass, enableAlphaToCoverage, layout,
...
DirectX12RenderPipeline::DirectX12RenderPipeline(const DirectX12RenderPass & renderPass,
  ...
  : m_impl(makePimpl<DirectX12RenderPipelineImpl>(this, renderPass))
  , DirectX12PipelineState(nullptr)
{
  if (!name_.empty())
    name() = name_;
...
bool DirectX12RenderPipeline::alphaToCoverage() const noexcept { return m_impl->m_alphaToCoverage; }

MultiSamplingLevel DirectX12RenderPipeline::samples() const noexcept { return m_impl->m_samples; }
...
void DirectX12RenderPipeline::use(const DirectX12CommandBuffer & commandBuffer) const noexcept
{
  ...
  commandBuffer.handle()->IASetPrimitiveTopology(
    DX12::getPrimitiveTopology(m_impl->m_inputAssembler->topology()));
...
DirectX12RenderPipelineBuilder::DirectX12RenderPipelineBuilder(
  const DirectX12RenderPass & renderPass, const String & name)
  : RenderPipelineBuilder(
      UniquePtr<DirectX12RenderPipeline>(new DirectX12RenderPipeline(renderPass)))
{
  instance()->name() = name;
}
