...
using ImageBarrier =
  Tuple<ResourceAccess, ResourceAccess, const IDirectX12Image &, Optional<ImageLayout>, ImageLayout,
...
PipelineStage DirectX12Barrier::syncBefore() const noexcept { return m_impl->m_syncBefore; }

PipelineStage & DirectX12Barrier::syncBefore() noexcept { return m_impl->m_syncBefore; }

PipelineStage DirectX12Barrier::syncAfter() const noexcept { return m_impl->m_syncAfter; }

PipelineStage & DirectX12Barrier::syncAfter() noexcept { return m_impl->m_syncAfter; }
...
void DirectX12Barrier::transition(const IDirectX12Buffer & buffer, ResourceAccess accessBefore,
                                  ResourceAccess accessAfter)
{
  m_impl->m_bufferBarriers.push_back(
    {accessBefore, accessAfter, buffer, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES});
}

void DirectX12Barrier::transition(const IDirectX12Buffer & buffer, UInt32 element,
...
void DirectX12Barrier::transition(const IDirectX12Image & image, ResourceAccess accessBefore,
                                  ResourceAccess accessAfter, ImageLayout layout)
{
  m_impl->m_imageBarriers.push_back({accessBefore, accessAfter, image, std::nullopt, layout, 0,
                                     image.levels(), 0, image.layers(), 0});
}
...
{
  m_impl->m_imageBarriers.push_back({accessBefore, accessAfter, image, fromLayout, toLayout, 0,
                                     image.levels(), 0, image.layers(), 0});
}

void DirectX12Barrier::transition(const IDirectX12Image & image, UInt32 level, UInt32 levels,
                                  UInt32 layer, UInt32 layers, UInt32 plane,
                                  ResourceAccess accessBefore, ResourceAccess accessAfter,
                                  ImageLayout layout)
{
  m_impl->m_imageBarriers.push_back(
    {accessBefore, accessAfter, image, std::nullopt, layout, level, levels, layer, layers, plane});
}

void DirectX12Barrier::transition(const IDirectX12Image & image, UInt32 level, UInt32 levels,
                                  UInt32 layer, UInt32 layers, UInt32 plane,
                                  ResourceAccess accessBefore, ResourceAccess accessAfter,
                                  ImageLayout fromLayout, ImageLayout toLayout)
{
  m_impl->m_imageBarriers.push_back(
    {accessBefore, accessAfter, image, fromLayout, toLayout, level, levels, layer, layers, plane});
}
...
void DirectX12Barrier::execute(const DirectX12CommandBuffer & commandBuffer) const noexcept
{
  ...
  // Global barriers.
  auto globalBarriers =
    m_impl->m_globalBarriers |
    std::views::transform(
      [this, &syncBefore, &syncAfter](auto & barrier)
      {
        return CD3DX12_GLOBAL_BARRIER(syncBefore, syncAfter,
                                      DX12::getResourceAccess(std::get<0>(barrier)),
                                      DX12::getResourceAccess(std::get<1>(barrier)));
      }) |
    std::ranges::to<Array<D3D12_GLOBAL_BARRIER>>();

  // Buffer barriers.
  auto bufferBarriers =
    m_impl->m_bufferBarriers |
    std::views::transform(
      [this, &syncBefore, &syncAfter](auto & barrier)
      {
        ...
        if (syncBefore == D3D12_BARRIER_SYNC_BUILD_RAYTRACING_ACCELERATION_STRUCTURE &&
            accessBefore == D3D12_BARRIER_ACCESS_RAYTRACING_ACCELERATION_STRUCTURE_WRITE)
          accessBefore = D3D12_BARRIER_ACCESS_UNORDERED_ACCESS;
        if (syncAfter == D3D12_BARRIER_SYNC_BUILD_RAYTRACING_ACCELERATION_STRUCTURE &&
        ...
        return CD3DX12_BUFFER_BARRIER(syncBefore, syncAfter, accessBefore, accessAfter,
                                      std::as_const(std::get<2>(barrier)).handle().Get());
      }) |
    std::ranges::to<Array<D3D12_BUFFER_BARRIER>>();

  // Image barriers.
  auto imageBarriers =
    m_impl->m_imageBarriers |
    std::views::transform(
      [this, &syncBefore, &syncAfter](auto & barrier)
      {
        auto & image = std::get<2>(barrier);
        auto currentLayout =
          DX12::getImageLayout(std::get<3>(barrier).value_or(ImageLayout::Undefined));
        auto targetLayout = DX12::getImageLayout(std::get<4>(barrier));

        for (auto layer = std::get<7>(barrier); layer < std::get<7>(barrier) + std::get<8>(barrier);
             layer++)
          for (auto level = std::get<5>(barrier);
               level < std::get<5>(barrier) + std::get<6>(barrier); level++)
            auto subresource = image.subresourceId(level, layer, std::get<9>(barrier));

        return CD3DX12_TEXTURE_BARRIER(syncBefore, syncAfter,
                                       DX12::getResourceAccess(std::get<0>(barrier)),
                                       DX12::getResourceAccess(std::get<1>(barrier)), currentLayout,
                                       targetLayout, std::as_const(image).handle().Get(),
                                       CD3DX12_BARRIER_SUBRESOURCE_RANGE(std::get<5>(barrier),
                                                                         std::get<6>(barrier),
                                                                         std::get<7>(barrier),
                                                                         std::get<8>(barrier),
                                                                         std::get<9>(barrier)));
...
void DirectX12BarrierBuilder::setupImageBarrier(IImage & image, ResourceAccess before,
                                                ResourceAccess after, ImageLayout layout,
                                                UInt32 level, UInt32 levels, UInt32 layer,
