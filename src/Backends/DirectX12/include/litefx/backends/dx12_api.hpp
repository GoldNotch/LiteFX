...
#elif (defined(LiteFX_Backends_DirectX12_EXPORTS) || defined(__APPLE__)) && defined __GNUC__ &&    \
  __GNUC__ >= 4
...
///
/// </summary>
PrimitiveTopology LITEFX_DIRECTX12_API
getPrimitiveTopology(const D3D12_PRIMITIVE_TOPOLOGY & topology);
...
///
/// </summary>
D3D12_PRIMITIVE_TOPOLOGY_TYPE LITEFX_DIRECTX12_API
getPrimitiveTopologyType(PrimitiveTopology topology);
...
/// </summary>
class LITEFX_DIRECTX12_API DirectX12GraphicsAdapter final : public IGraphicsAdapter,
                                                            public ComResource<IDXGIAdapter4>
{
...
/// </summary>
class LITEFX_DIRECTX12_API DirectX12Surface final : public ISurface,
...
public:
  ...
  explicit DX12PlatformException(HRESULT result) noexcept
    : m_code(result)
    , m_error(result)
    , RuntimeException("{1} (HRESULT 0x{0:08X})", static_cast<unsigned>(result),
  ...
  explicit DX12PlatformException(HRESULT result, StringView message) noexcept
    : m_code(result)
    , m_error(result)
    , RuntimeException("{2} {1} (HRESULT 0x{0:08X})", static_cast<unsigned>(result),
                       _com_error(result).ErrorMessage(), message)
  {
  }
...
public:
  ...
  const _com_error & error() const noexcept { return m_error; }
